...
<div class="course-item-meta">
	<?php do_action( 'learning_online_before_item_meta', $item ); ?>
	...
	if ( $user->can_view_item( $item->ID, $course->id ) !== false ) {
		if ( $item->post_type == 'lp_quiz' ) {
			...
			if ( $course->is( 'final-quiz', $item->ID ) ) {
				?><span class="item-loop-meta-text item-final"><?php _e( 'Final Quiz', 'learningonline' ); ?></span><?php
			}
			if ( $item_status == 'completed' ) {
				$grade = $user->get_quiz_graduation( $course_item->id, $course->id );
				if ( $grade === 'passed' ) {
					?>
				<span class="lp-icon item-status item-status-passed" title="<?php esc_attr_e( 'Passed', 'learningonline' ); ?>"></span><?php
				} else {
					?>
				<span class="lp-icon item-status item-status-failed" title="<?php esc_attr_e( 'Failed', 'learningonline' ); ?>"></span><?php
				}
			} elseif ( $item_status == 'viewed' ) {
				?>
			<span class="lp-icon item-status item-status-viewed" title="<?php esc_attr_e( 'Viewed', 'learningonline' ); ?>"></span><?php
			} elseif ( $item_status == 'started' ) {
				?>
			<span class="lp-icon item-status item-status-started" title="<?php esc_attr_e( 'In Progress', 'learningonline' ); ?>"></span><?php
			}
		} else {
			if ( $item_status == 'completed' ) {
				?>
				<span class="lp-icon item-status item-status-passed" title="<?php esc_attr_e( 'Completed', 'learningonline' ); ?>"></span>
				<?php
			} elseif ( $item_status == 'viewed' ) { ?>
				<span class="lp-icon item-status item-status-viewed" title="<?php esc_attr_e( 'Viewed', 'learningonline' ); ?>"></span>
				<?php
			}
		}
		if ( !$item_status ) {
	...
	?>
	<?php do_action( 'learning_online_after_item_meta', $item ); ?>
</div>
