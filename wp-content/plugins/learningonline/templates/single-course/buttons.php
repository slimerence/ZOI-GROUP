...
<div class="learning-online-course-buttons">
	...
	if ( $external_link = $course->get_external_link() ):
		$external_button_text = apply_filters( 'learning_online_course_external_link_button_text', __( 'Buy this course', 'learningonline' ) );
		?>
		<?php do_action( 'learning_online_before_external_link_buy_course' ); ?>
		<div class="purchase-course">
			<a href="<?php echo esc_url( $external_link ); ?>" class="purchase-button">
				<?php echo $external_button_text; ?>
			</a>
		</div>
		<?php do_action( 'learning_online_after_external_link_buy_course' ); ?>
	<?php else:
		...
		if ( $course->is_required_enroll() ) {
			...
			$purchase_button_text  = apply_filters( 'learning_online_purchase_button_text', __( 'Buy this course', 'learningonline' ) );
			...
			<?php else: ?>
				...
				<?php } elseif ( in_array( $order_status, array( 'lp-processing', 'lp-on-hold' ) ) ) { ?>
					<?php learning_online_display_message( '<p>' . apply_filters( 'learning_online_user_course_pending_message', __( 'Your order is processing. Please wait for approval.', 'learningonline' ), $course, $user ) . '</p>' ); ?>
