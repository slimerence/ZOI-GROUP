...
$students_list_heading     = apply_filters( 'learning_online_students_list_heading', __( 'Students Enrolled', 'leanpress' ) );
$student_limit             = apply_filters( 'learning_online_students_list_limit', - 1 );
$show_avatar               = apply_filters( 'learning_online_students_list_avatar', true );
$students_list_avatar_size = apply_filters( 'learning_online_students_list_avatar_size', 32 );
?>
<?php do_action( 'learning_online_before_student-list' ) ?>
	...
	<?php if ( $students = $course->get_students_list( true, $student_limit ) ): ?>
		...
		<?php if ( is_user_logged_in() ): ?>
            <div class="filter-students">
                <label for="students-list-filter"><?php echo esc_html__( 'Student filter', 'learningonline' ); ?></label>
                <select id="students-list-filter">
					<?php
					$filters = learning_online_get_students_list_filter();
        ...
        <ul class="students">
			<?php foreach ( $students as $student ) {
				...
				if ( is_user_logged_in() ) {
					learning_online_setup_user_course_data( $student->ID, $course->ID, true );
                ...
                <li class="students-enrolled <?php echo ( $result ) ? 'user-login ' . $process : ''; ?>">
                    <div class="user-info">
						...
						<?php endif; ?>
                        <a class="name" href="<?php echo learning_online_user_profile_link( $student->ID ) ?>"
		...
		if ( $other_student ) {
			echo '<p class="additional-students">and ' . sprintf( _n( 'one student enrolled.', '%s students enrolled.', $other_student, 'learningonline' ), $other_student ) . '</p>';
		}
		?>
	<?php else: ?>
        <div class="students empty">
			<?php if ( $course->students ) {
				echo apply_filters( 'learning_online_course_count_student', sprintf( _n( 'One student enrolled.', '%s students enrolled.', $course->students, 'learningonline' ), $course->students ) );
			} else {
				echo apply_filters( 'learning_online_course_no_student', __( 'No student enrolled.', 'learningonline' ) );
			} ?>
        </div>
	<?php endif; ?>
</div>
<?php do_action( 'learning_online_after_student-list' ) ?>
