 ...
 *
 * @author  JwsTheme
 * @package LearningOnline/Templates/Widgets
 * @version 2.1.5
 */
...
<div class="<?php echo 'archive-course-widget-outer ' . esc_attr( $instance["css_class"] ); ?>">
				...
				<div class="course-detail">
					...
					<?php if ( !empty( $instance['desc_length'] ) && intval( $instance['desc_length'] ) > 0 ): ?>
						<div class="course-description"><?php
							...
							$the_content    = wp_trim_words( $the_content, $content_length, __( '...', 'learningonline' ) );
							echo $the_content;
							?></div>
					<?php endif; ?>
					<div class="course-meta-data">
						...
						<?php if ( !empty( $instance['show_enrolled_students'] ) ): ?>
							<div class="course-student-number course-meta-field">
								...
								echo intval( $students ) > 1 ? sprintf( __( '%d students', 'learningonline' ), $students ) : sprintf( __( '%d student', 'learningonline' ), $students );
								?>
							</div>
						<?php endif; ?>
						<?php if ( !empty( $instance['show_lesson'] ) ): ?>
							<div class="course-lesson-number course-meta-field">
								...
								echo intval( $lessons ) > 1 ? sprintf( __( '%d lessons', 'learningonline' ), $lessons ) : sprintf( __( '%d lesson', 'learningonline' ), $lessons );
								?>
							</div>
						<?php endif; ?>
	...
	<div class="widget-footer">
		<?php if ( !empty( $instance['bottom_link_text'] ) ):
			$page_id = get_option( 'learning_online_courses_page_id' );
