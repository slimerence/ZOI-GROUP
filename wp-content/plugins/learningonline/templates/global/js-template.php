...
if ( learning_online_is_course() ) {
	$course = learning_online_get_course( get_the_ID() );
	$user   = learning_online_get_current_user();
	?>
	<script type="text/template" id="learning-online-template-curriculum-popup">
	...
	<script type="text/template" id="learning-online-template-course-prev-item">
		<a class="footer-control prev-item hide-if-js button-load-item" data-id="{{data.id}}" href="{{data.url}}"><span>&larr;</span>{{data.title}}</a>
	</script>

	<script type="text/template" id="learning-online-template-course-next-item">
