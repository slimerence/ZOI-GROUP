<?php
/*
Plugin Name: LearningOnline
Plugin URI: 
Description: LearningOnline is a WordPress complete solution for creating a Learning Management System (LMS). It can help you to create courses, lessons and quizzes.
Author: JwsTheme
Version: 1.0.0
Author URI: http://jwstheme.com
Requires at least: 3.8
Tested up to: 4.8

Text Domain: learningonline
...
if ( !class_exists( 'LearningOnline' ) ) {
	...
	class LearningOnline {
		 ...
		 * @var string
		 */
		public $version = LEARNINGONLINE_VERSION;

		/**
		 * The single instance of the class
		 *
		 * @var LearningOnline object
		 */
				...
				case 'course':
					if ( empty( $this->_course ) ) {
						if ( learning_online_is_course() ) {
							$this->_course = learning_online_setup_object_data( get_the_ID() );
						}
					}
					$return = $this->_course;
					break;
				case 'quiz':
					if ( empty( $this->_quiz ) ) {
						if ( learning_online_is_quiz() ) {
							$this->_quiz = learning_online_setup_object_data( get_the_ID() );
						}
					}
					$return = $this->_quiz;
					break;
		...
		public function define_tables() {
			...
			foreach ( $tables as $short_name ) {
				...
				$backward_key          = 'learningonline_' . $short_name;
		...
		private function plugin_basename() {
			return learning_online_plugin_basename( __FILE__ );
		}
		...
		public function init_hooks() {
			...
			add_action( 'template_redirect', 'learning_online_handle_purchase_request' );
		...
		function wp_loaded() {
			learning_online_get_current_user();
		...
		public function loaded() {
			...
			do_action( 'learning_online_ready' );
			do_action( 'learning_online_loaded', $this );
		}
		 ...
		 *
		 * @hook learning_online_activate
		 */
		public function on_activate() {
			do_action( 'learning_online_activate', $this );
		}

		/**
		 * Trigger this function while deactivating LP
		 *
		 * @hook learning_online_deactivate
		 */
		public function on_deactivate() {
			do_action( 'learning_online_deactivate', $this );
		}

		/**
		 * Init LearningOnline when WP initialises
		 */
		public function init() {
			...
			if ( !empty( $_REQUEST['view-log'] ) ) {
				...
				echo $log = learning_online_get_log_file_path( $log );
				@readfile( $log );
				echo '<pre>';
				die();
			}
			...
			if ( get_option( 'learning_online_install' ) == 'yes' ) {
				flush_rewrite_rules();
				delete_option( 'learning_online_install' );
			}
		}
		...
		public function get_session() {
			if ( !$this->session ) {
				$session_class = apply_filters( 'learning_online_session_class', 'LP_Session_Handler' );
		...
		public function get_cart() {
			if ( !$this->cart ) {
				$cart_class = apply_filters( 'learning_online_cart_class', 'LP_Cart' );
		...
		public function get_checkout_cart() {
			return learning_online_get_checkout_cart();
		}
		...
		public function get_user( $user_id = 0 ) {
			...
			$user = false;
			if ( !$this->user ) {
				$this->user = learning_online_get_current_user();
			}
			if ( $user_id ) {
				if ( $user_id == $this->user->id ) {
					...
					if ( empty( $users[$user_id] ) ) {
						$users[$user_id] = learning_online_get_user( $user_id );
						$user            = $users[$user_id];
					}
				}
			} else {
		...
		public function template_path( $slash = false ) {
			return learning_online_template_path( $slash );
		}
		...
		/**
		 * Short way to return js file is located in LearningOnline directory
		 *
		 * @param string
		 *
		 * @return string
		...
		/**
		 * Short way to return css file is located in LearningOnline directory
		 *
		 * @param string
		 *
		 * @return string
		...
		/**
		 * Short way to return image file is located in LearningOnline directory
		 *
		 * @param string
		 *
		 * @return string
 ...
 */
function LP() {
	return LearningOnline::instance();
}
 ...
 * Create new instance of LearningOnline and put it to global
 */
$GLOBALS['LearningOnline'] = LP();
