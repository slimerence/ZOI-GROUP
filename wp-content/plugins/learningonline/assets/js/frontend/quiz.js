...
;(function ($) {
	...
	Quiz.Model_Question = Model_Question = Backbone.Model.extend({
		...
		submit    : function (args) {
			...
			this.fetch({
				data    : $.extend({
					action     : 'learningonline_load_quiz_question',
		...
		check     : function (args) {
			...
			LP.doAjax({
				data    : $.extend({
					...
					question_id: this.get('id')
					//question_answer: $('form#learning-online-quiz-question').serializeJSON()
				}, args.data || {}),
				dataType: 'html',
				success : function (response, raw) {
					var $content = that.get('response');
					if ($content) {
						$content.find('.learning-online-question-options').replaceWith($(response).filter('.learning-online-question-options'));
					}
	...
	Quiz.Model = Backbone.Model.extend({
		...
		initialize           : function (args) {
			...
			LP.Hook.addFilter('learning_online_finish_quiz_data', this.getQuizData);
		...
		fetchCurrent         : function (callback) {
			...
			LP.Hook.doAction('learning_online_before_fetch_question', current.get('id'), that);
			$.ajax({
				url     : current.get('url'),
				dataType: 'html',
		...
		next                 : function (callback, args) {
			...
			LP.Hook.doAction('learning_online_before_next_question', id, that);
			this.select(id, function (args) {
				LP.Hook.doAction('learning_online_previous_question', id, that);
		...
		prev                 : function (callback, args) {
			...
			id = prev.get('id');
			LP.Hook.doAction('learning_online_before_previous_question', id, that);
		...
		select               : function (id, callback, args) {
			var question = this.questions.findWhere({id: parseInt(id)}),
				...
				loadedCallback = function () {
					...
					LP.Hook.doAction('learning_online_select_question', question, that);
				};
			if (!question) {
				return;
			}
			...
			LP.Hook.doAction('learning_online_before_select_question', question, that);
			...
			if (question.get('response')) {
				...
				$.ajax({
					...
					dataType: 'html',
					success : function (response) {
						var $html = $(response).contents().find('.learning-online-content-item-summary');
		...
		getRequestParams     : function (args) {
			var defaults = LP.Hook.applyFilters('learning_online_request_quiz_params', {
				quiz_id  : this.get('id'),
				user_id  : this.get('user_id'),
		...
		getQuestionAnswerData: function (question) {
			...
			return LP.Hook.applyFilters('learning_online_question_answer_data', answer, $form, question, this);
		}
	});
	Quiz.View = Backbone.View.extend({
		...
		initialize            : function () {
			...
			LP.Hook.addAction('learning_online_before_finish_quiz', this.destroy);
			LP.Hook.addFilter('learning_online_get_current_item_url', this.itemUrl);
			this.model.current(true).set('response', this.$('.learning-online-content-item-summary'));
			this.model.set('view', this);
			this.model.on('destroy', this._onDestroy);
			this._initCountDown();
			this.updateButtons();
			this.windowTarget = windowTarget;
			this.localized = window.learning_online_single_course_localize || {};
		...
		_onTick               : function () {
			...
			this.updateCountdown();
			$(document).triggerHandler('learning-online/quiz-tick');
			if (timer.remainingTime == 0) {
				LP.Hook.doAction('learning_online_quiz_timeout', this);
		...
		_loadQuestionCompleted: function (question, model) {
			var loaded = model.get('loaded'),
				...
				$oldElement = this.$('.learning-online-content-item-summary');
		...
		_showHint             : function (e) {
			...
			this.$('.button-hint').attr('disabled', true);
			this.$('.learning-online-question-hint').removeClass('hide-if-js');
		},
		...
		destroy               : function () {
			this.pause();
			LP.Hook.removeAction('learning_online_before_finish_quiz');
			LP.Hook.removeFilter('learning_online_get_current_item_url');
		},
		/********************************/
		_submit               : function (args) {
			...
			windowTarget.LP.blockContent();
			LP.Hook.doAction('learning_online_before_start_quiz', this.currentItem, this);
		...
		_startQuiz            : function (e) {
			var that = this,
				$button = $(e.target);
			windowTarget.LP.Hook.doAction('learning_online_before_start_quiz', this);
		...
		_finishQuiz           : function (e) {
			...
			if (typeof e.originalEvent === 'undefined') {
				windowTarget.LP.Hook.doAction('learning_online_before_finish_quiz', this);
		...
		_doFinishQuiz         : function (security) {
			var that = this;
			LP.Hook.doAction('learning_online_before_finish_quiz', that.currentItem, that);
			that.finishQuiz({
				security  : security,
				beforeSend: windowTarget.LP.blockContent(),
				callback  : function (response, item) {
					windowTarget.LP.unblockContent();
					that.$('#learning-online-content-item').html(response.html.content);
		...
		_retakeQuiz           : function (e) {
			...
			this._confirm('confirm_retake_quiz', function () {
				LP.Hook.doAction('learning_online_before_retake_quiz', this);
				that._submit($button.data());
			});
			return false;
		},
