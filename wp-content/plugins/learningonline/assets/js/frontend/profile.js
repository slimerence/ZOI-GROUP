;(function ($) {
	...
	UserProfile.View = Backbone.View.extend({
		...
		_getUploader  : function () {
			...
			this.uploader = new plupload.Uploader({
				...
				url           : LP_Settings.ajax.addQueryVar('action', 'learningonline_upload-user-avatar'),
				filters       : {
					max_file_size: '10mb',
					mime_types   : [
	...
	$(document).on('submit', '#learning-online-form-login', function (e) {
		var $form = $(this),
			data = $form.serialize();
		$form.find('.learning-online-error, .learning-online-notice, .learning-online-message').fadeOut();
		...
		LP.doAjax({
			...
			success: function (response, raw) {
				...
				if (response.result == 'error') {
					...
					$('#learning-online-form-login input[type="text"]').focus();
				}
	...
	$(document).on('click', '.table-orders .cancel-order', function (e) {
		...
		LP.alert(learning_online_js_localize.confirm_cancel_order, function (confirm) {
			if (confirm) {
