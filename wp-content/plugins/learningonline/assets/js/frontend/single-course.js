 ...
 */
if (typeof LearningOnline === 'undefined') {
	window.LP = window.LearningOnline = {};
}
;(function ($) {
	"use strict";
	LearningOnline.Course = $.extend(
		LearningOnline.Course || {}, {
			finish: function (data, callback) {
				LearningOnline.$LP_Course && LearningOnline.$LP_Course.finishCourse({data: data, success: callback});
			}
		}
	);
	...
	var Course = function (args) {
		...
		Template = function (id, tmpl_data) {
			var compiled,
				...
				template = function (data) {
					compiled = compiled || _.template($('#learning-online-template-' + id).html(), null, options);
		...
		Course_Item = window.Course_Item = Backbone.Model.extend({
			...
			_changeContent : function () {
				LP.Hook.doAction('learning_online_update_item_content', this);
			},
			...
			request        : function (args) {
				...
				LP.ajax({
					...
					success : function (response) {
						LP.Hook.doAction('learning_online_course_item_loaded', response, that);
						response = LP.Hook.applyFilters('learning_online_course_item_content', response, that);
			...
			complete       : function (args) {
				...
				LP.ajax({
					...
					success : function (response) {
						...
						LP.Hook.doAction('learning_online_course_item_completed', response, that);
						response = LP.Hook.applyFilters('learning_online_course_item_complete_response', response, that);
						$.isFunction(args.callback) && args.callback.call(args.context, response, that);
					}
				});
			},
			...
			finishQuiz     : function (args) {
				...
				var data = this._validateObject(args), that = this;
				data = LP.Hook.applyFilters('learning_online_finish_quiz_data', data);
	...
	Course.View = Backbone.View.extend({
		...
		events             : {
			...
			'click .learning-online-nav-tab'                      : '_tabClick',
			...
			'click .learning-online-content-item-title .lp-expand': '_expand'

			//'click #learning-online-button-complete-item': '_c'
		},
		itemLoading        : 0,
		currentItem        : null,
		initialize         : function () {
			...
			this.itemEl = this.$('#learning-online-content-item');
			...
			LP.Hook.addAction('learning_online_receive_message', this.receiveMessage);
		...
		expand             : function (expand) {
			...
			$('#learning-online-content-item').toggleClass('expand', expand);
			if (localStorage) {
				localStorage.setItem("lp-item-expanded", expand ? 'yes' : 'no');
			}
		},
		_initHooks         : function () {
			LP.Hook.addAction('learning_online_update_item_content', this.updateItemContent);
			LP.Hook.addAction('learning_online_set_location_url', this.pushContent);

			$(document).on('learning_online_popup_course_remove', this.removePopup);
		},
			...
			this.currentItem.complete({
				...
				callback  : function (response, item) {
					if (response.result === 'success') {
						...
						that.$('.learning-online-course-results-progress').replaceWith($(response.html.progress));
						$section.find('.section-header').replaceWith($(response.html.section_header));
						that.$('.learning-online-course-buttons').replaceWith($(response.html.buttons));
		...
		_tabClick          : function (e) {
			var tab = $(e.target).closest('.learning-online-nav-tab').data('tab');
			if (tab === 'overview') {
		...
		_retakeCourse    : function (e) {
			e.preventDefault();
			var that = this;
			jConfirm(learning_online_single_course_localize.confirm_retake_course.message, learning_online_single_course_localize.confirm_retake_course.title, function (confirm) {
				if (confirm) {
					var $button = $(e.target),
						data = $button.data(),
						args = {
		...
		_finishCourse    : function (e) {
			e.preventDefault();
			var _this = this;
			jConfirm(learning_online_single_course_localize.confirm_finish_course.message, learning_online_single_course_localize.confirm_finish_course.title, function (confirm) {
				if (confirm) {
					var $button = $(e.target),
						data = $button.data(),
						args = {
		...
		updateUrl     : function (url) {
			if (!url && this.currentItem) {
				url = LP.Hook.applyFilters('learning_online_get_current_item_url', this.currentItem.get('url'), this.currentItem);
			}
			if (url) {
				LP.setUrl(url);
			}
	...
	Course.Popup = Backbone.View.extend({
		...
		render              : function () {
			$('.learning-online-content-item-summary').remove();
			...
			var $curriculum = this.course.$('#learning-online-course-curriculum');
			var $progress = this.course.$('.learning-online-course-results-progress');
			...
			$(document).on('learning_online_unblock_content', this.hideScrollBar);
		},
		_closePopup         : function (e) {
			e.preventDefault();
			LP.setUrl(this.course.model.get('url'));
			$('#learning-online-content-item').hide();
			this.curriculumPlaceholder.replaceWith(this.$('#learning-online-course-curriculum'));
			this.progressPlaceholder.replaceWith(this.$('.learning-online-course-results-progress'));
			...
			$(document).off('focusin').trigger('learning_online_popup_course_remove').unbind('learning_online_unblock_content', this.hideScrollBar);
		},
	...
	$(document).ready(function () {
		...
		(typeof window.LP_Course_Params != 'undefined') && $(document).ready(function () {
			...
			LP.Hook.doAction('learning_online_course_initialize', LP.$LP_Course);
	...
	$(document).ajaxComplete(function (a, b, c) {
		if (c.data && c.data.match(/lp-ajax=/)) {
			...
			LP.unblockContent();
			$('.learning-online-tooltip').tooltip({offset: [24, 24]});
		}
	}).ajaxError(function () {
		LP.unblockContent();
	});
