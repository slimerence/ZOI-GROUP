...
(function ($) {
	...
	LP.Checkout = {
		...
		init               : function () {
			...
			$doc.on('click', '#learning-online-checkout-login-button', this.login);
		...
		selectPaymentMethod: function () {
			var methodId = $(this).attr('id'),
				checkoutButton = $('#learning-online-checkout-place-order'),
				isError = false;
					...
					success : function (response) {
						...
						if (response.result === 'fail') {
							if (response.messages) {
								...
								LP.Checkout.showErrors('<div class="learning-online-error">Unknown error!</div>');
							}
						} else {
		...
		doCheckout         : function () {
			var $form = $(this),
				$place_order = $form.find('#learning-online-checkout-place-order'),
				processing_text = $place_order.attr('data-processing-text'),
				text = $place_order.attr('value');
			if ($form.triggerHandler('learning_online_checkout_place_order') !== false && $form.triggerHandler('learning_online_checkout_place_order_' + $('#order_review').find('input[name=payment_method]:checked').val()) !== false) {
				if (processing_text) {
				...
				$.ajax({
						...
						if (response.result === 'fail') {
							...
							if (!response.messages) {
								if (response.code && response.code == 30) {
									LP.Checkout.showErrors('<div class="learning-online-error">' + learning_online_js_localize.invalid_field + '</div>');
								} else {
									LP.Checkout.showErrors('<div class="learning-online-error">' + learning_online_js_localize.unknown_error + '</div>');
								}
							} else {
								LP.Checkout.showErrors(response.messages);
							}
					...
					error   : function (jqXHR, textStatus, errorThrown) {
						LP.Checkout.showErrors('<div class="learning-online-error">' + errorThrown + '</div>');
						$place_order.val(text);
						$place_order.prop('disabled', false);
						LP.unblockContent();
					}
		...
		showErrors         : function (messages) {
			...
			$(document).trigger('learningonline_checkout_error');
		}
	};
