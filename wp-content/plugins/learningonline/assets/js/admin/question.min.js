		...
		_getEmptyOption                                                                 : function (question_id) {
			...
			if ($newOption) {
				...
				var templateArgs = {
					...
					text       : '',
					value      : LP.uniqueId()
				}, tmpl = wp.template($('#learning-online-question-' + question_id).attr('data-type') + '-option'), $list = $('#learning-online-list-options-' + question_id + ' tbody');
...
if($newOption&&args.autoFocus){$newOption.find('.lp-answer-text').focus()}},removeOption:function(theOption){var $theOption=null;if($.type(theOption)=='integer'){$theOption=$('lp-list-option-'+theOption)}else{$theOption=$(theOption)}
			...
			var post_data = $.extend({action: 'learningonline_add_question'}, args);
...
$(question).addClass('added')},_showQuestion                                            :function(question){if($.type(question)=='number'){question=$('#learning-online-dropdown-questions .question a[data-id="'+question+'"]').parent()}
$(question).removeClass('added')}};function updateHiddenQuestions(hidden){if(hidden==undefined){hidden=[];var len=$('.quiz-question-content').each(function(){if($(this).is(':hidden')){hidden.push($('.learning-online-question',this).attr('data-id'))}}).length;if(hidden.length==0){$('.questions-toggle a[data-action="collapse"]').show().siblings('a[data-action="expand"]').hide()}else if(hidden.length==len){$('.questions-toggle a[data-action="collapse"]').hide().siblings('a[data-action="expand"]').show()}}
		$.ajax({
			url    : LP_Settings.ajax,
			data   : {action: 'learningonline_update_quiz_question_state', quiz_id: $('#post_ID').val(), hidden: hidden},
			success: function () {
			}
		});
		return hidden
...
var $this=$(this),cols=$this.find('tr:first').children().length;$this.find('.ui-sortable-placeholder td:gt(0)').remove();$this.find('.ui-sortable-placeholder td:eq(0)').attr('colspan',cols)},stop:function(){}})}
	function _ready() {
		...
		$doc.on('click', '.add-question-option-button', function (e, data) {
			...
			var $select = $(this), $wrap = $select.closest('.learning-online-question'), questionId = $wrap.data('id'), from = $select.data('selected'), to = this.value, _do = function () {
				...
				$.ajax({
					...
					success : function (response) {
						response = LP.parseJSON(response);
						var $newOptions = $(response.html), $question = $('#learning-online-question-' + questionId), $icon = $question.closest('.quiz-question').find('.quiz-question-icon img');
...
var $wrap=$(this).closest('.learning-online-question'),$button=$wrap.find('.add-question-option-button');$button.trigger('click',{autoFocus:e.keyCode==13})})}
