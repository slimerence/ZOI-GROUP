...
(function ($) {
	var LP_Order_View = window.LP_Order_View = Backbone.View.extend({
		el                : 'body',
		events            : {
			'click #learning-online-add-order-item'           : '_addItem',
			'click #learning-online-calculate-order-total'    : 'calculateTotal',
			'click #learning-online-courses-result a[data-id]': 'addItem',
			'click .remove-order-item'                    : 'removeItem',
			'keyup #learning-online-search-item-term'         : 'searchItems',
			'change select[name="order-status"]'          : '_updateDescription'
		},
		initialize        : function () {
			_.bindAll(this, 'resetModal', 'updateModal', '_updateDescription');
			LP.Hook.addAction('learning_online_message_box_before_resize', this.resetModal);
			LP.Hook.addAction('learning_online_message_box_resize', this.updateModal);
			...
			$('select[name="order-status"]').on('change', function () {
                ...
                $.ajax({
                    ...
                    data    : {
                        action  : 'learningonline_update_order_status',
                        order_id: order_id,
                        value: $sel.val(),
                    },
                    dataType: 'text',
			...
			$(document).on('learning_online_modal_search_items_response', this.addItem2);
			this.userSuggest();
		},
		...
		userSuggest       : function () {
			...
			$('.wp-suggest-user').each(function () {
				...
				$this.autocomplete({
					source   : LP_Settings.ajax + '?action=learningonline_search_users&autocomplete_type=' + autocompleteType + '&autocomplete_field=' + autocompleteField + id,
					delay    : 500,
					minLength: 2,
					position : position,
					open     : function () {
		...
		resetModal        : function (height, $app) {
			this.$('#learning-online-courses-result').css('height', height - 120).css('overflow', 'auto');
		},
		updateModal       : function ($app) {
			this.$('#learning-online-courses-result').css('height', '').css('overflow', '');
		},
...
//			var $form = $('#learning-online-modal-add-order-courses');
//			if ($form.length == 0) {
//				$form = $(wp.template('learning-online-modal-add-order-courses')());
//			}
//			LP.MessageBox.show($form);
		...
		addItem2          : function (e, $view, $items) {
			...
			$.ajax({
				url     : LP_Settings.ajax,
				data    : {
					action  : 'learningonline_add_item_to_order',
					order_id: parseInt($('input#post_ID').val()),
					item_id : ids,
					nonce   : $('#learning-online-modal-add-order-courses').attr('data-nonce')
				},
				dataType: 'text',
				type    : 'post',
				success : function (response) {
...
//				 _makeListSortable();
		...
		removeItem        : function (e) {
			...
			$.ajax({
				url     : LP_Settings.ajax,
				data    : {
					action      : 'learningonline_remove_order_item',
		...
		fetchResults      : function (results) {
			var $list = this.$('#learning-online-courses-result'),
			...
			var that = this,
				...
				_search = function () {
					$.ajax({
						url     : LP_Settings.ajax,
						data    : {
							action: 'learningonline_search_courses',
