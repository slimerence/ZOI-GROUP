;(function ($) {
	var LP_Admin = window.LP_Admin = {
		init          : function () {
			var $doc = $(document);
			$doc.on('click', '#learning-online-install-sample-data-notice a', this._importCourses)
				.on('click', '.learning-online-admin-notice-dismiss', this._dismissNotice)
		...
		_dismissNotice: function (e) {
			...
			if (context) {
				$.ajax({
					...
					data   : {
						action   : 'learningonline_dismiss_notice',
						context  : context,
						transient: transient
					},
		...
		_importCourses: function (e) {
			var $container = $('#learning-online-install-sample-data-notice'),
			...
			if (action == 'yes') {
				...
				$('#learning-online-install-sample-data-notice').fadeOut();
			}
			$.ajax({
				url     : ajaxurl,
				dataType: 'html',
				type    : 'post',
				data    : {
					action: 'learningonline_install_sample_data',
					yes   : action
				},
				success : function (response) {
	...
	function _ready() {
		...
		$(document).on('click', '.plugin-action-buttons a', function (e) {
			...
			if ($(e.target).closest('.learningonline-premium-plugin').length) {
				return;
			}
		...
		var $sandbox_mode = $('#learning_online_paypal_sandbox_mode'),
			$paypal_type = $('#learning_online_paypal_type');
		$paypal_type.change(function () {
			$('.learning_online_paypal_type_security').toggleClass('hide-if-js', 'security' != this.value);
		});
		$sandbox_mode.change(function () {
			this.checked ? $('.sandbox input').removeAttr('readonly') : $('.sandbox input').attr('readonly', true);
		});

		$('#learning_online_paypal_enable').change(function () {
		...
		$('.learning-online-toggle-lesson-preview').on('change', function () {
			$.ajax({
				url     : LP_Settings.ajax,
				data    : {
					action     : 'learningonline_toggle_lesson_preview',
...
jQuery(document).ready(function ($) {
	$('#wp-admin-bar-be_teacher').click(function () {
		$.ajax({
			url    : ajaxurl,
			data   : {
				action: 'learningonline_be_teacher'
			},
			success: function () {
...
jQuery(document).ready(function ($) {
	$('.lpr-set-up').click(function (evt) {
		...
		$.ajax({
			url    : ajaxurl,
			data   : {
				action: 'learningonline_ignore_setting_up'
			},
			success: function () {
				if ($link.attr('href')) {
...
(function ($) {
	...
	$doc.ready(function () {
		...
		$('input.lpr-fancy-checkbox')
			.on('change', function () {
				...
				$.ajax({
					...
					data   : {
						...
						action: 'learningonline_update_add_on_status'
					},
					success: function (response) {
			...
			.lprFancyCheckbox();
		$('#learning-online-add-ons-wrap').on('click', '.plugin-action-buttons a', function (evt) {

			if ($(evt.target).closest('.learningonline-premium-plugin').length) {
				return;
			}
			evt.preventDefault();
		...
		$('#learning-online-bundle-activate-add-ons').click(function () {
			var $button = $(this);
			$button.addClass('spinner').attr('disabled', 'disabled');
			$.ajax({
				url     : ajaxurl,
				data    : {
					action: 'learningonline_bundle_activate_add_ons'
				},
				dataType: 'html',
				success : function (response) {
					response = LP.parseJSON(response);
					if (response.addons) {
						for (var slug in response.addons) {
							...
							if ('activate' == plugin.status) {
								$('.plugin-card-' + slug).find('.install-now.jwstheme, .active-now.jwstheme').addClass('disabled').attr('href', '').html(plugin.status_text);
							} else {
			...
			$select.click(function () {
				...
				if (option == 'add_new_page') {
					...
					$.ajax({
						url     : ajaxurl,
						data    : {
							action : 'learningonline_get_page_permalink',
							page_id: option
						},
						success : function (response) {
							if (response) {
	...
	$.extend(LP, {
		...
		showLessonQuiz: function (pos, ed) {
			...
			ed.execCommand('mceInsertContent', false, '<span id="learning_online_book_mark"></span>');
			offset = $('#learning_online_book_mark', textNode).position();
			$('#learning_online_book_mark', textNode).remove();
...
jQuery(document).ready(function ($) {
	...
	learning_online_open_pointer(0);
	function learning_online_open_pointer(i) {
...
(function ($) {
	...
	$(document).ready(function () {
		$('.learning-online-checkbox').iosCheckbox();

		if (typeof Switchery !== 'undefined') {
			var elems = Array.prototype.slice.call(document.querySelectorAll('.rwmb-learningonline-switchbutton'));
...
(function ($) {
	...
	$(document).on('click', '.learningonline-dismiss-notice', function (event) {

		var $parent = $(this).closest('.learningonline-search-notices');
		...
		if ($parent.length) {
			...
			$.ajax({
				...
				type    : 'POST',
				data    : {
					action: 'learningonline_remove_notice_popup',
					slug  : slug,
					user  : user
				},
	...
	$.fn.learningonlineSlider = function (opts) {
		...
		var LeanpressSlider = {
			...
			options    : options,
			$items     : [],
			$control   : $('<div class="learningonline-controls"><div class="next-item"></div><div class="prev-item"></div></div>'),
			$wrapSlider: $('<div class="learningonline-wrap-items"></div>'),
			...
			init: function () {
				...
				_this.$el.data({
					'LearningonlineSlider'       : _this,
					'LearningonlineSliderOptions': _this.options
				});
			},
			...
			createHTML: function () {
				...
				$('> *', _this.$el).each(function (index, item) {
					...
					$(this).addClass('learningonline-item');
			...
			activeItem: function (index) {
				...
				if (_this.$currentActive) {
					_this.$currentActive.removeClass('learningonline-active');
				}
				$item.addClass('learningonline-active');
	...
	$(document).ready(function () {
		...
		var $wrapAddon = $('#learning-online-add-ons-wrap'),
		...
		$wrapAddon.on('keydown change', '.lp-search-addon', function (event) {
			...
			setTimeout(function () {
				...
				var txt = $this.val(),
					...
					$wrapFreeAddon = $('.learningonline-free-plugin-wrap', $clone),
					$wrapPremium = $('.learningonline-premium-plugin-wrap', $clone),
					$wrapThemesEducation = $('.learningonline-theme-education', $clone),
					$wrapThemesOther = $('.learningonline-theme-other', $clone);

				txt = txt.trim().toUpperCase();

				$('.plugin-card-learningonline', $clone).each(function (index, item) {
				...
				// Count for plugin
				$('.learningonline-count-addon', $wrapFreeAddon).text($('.plugin-card-learningonline:not(.lp-addon-hidden)', $wrapFreeAddon).length);
				$('.learningonline-count-addon', $wrapPremium).text($('.plugin-card-learningonline:not(.lp-addon-hidden)', $wrapPremium).length);

				/* Count for theme */
				$('.learningonline-count', $wrapThemesEducation).text($('.plugin-card-learningonline:not(.lp-addon-hidden)', $wrapThemesEducation).length);
				$('.learningonline-count', $wrapThemesOther).text($('.plugin-card-learningonline:not(.lp-addon-hidden)', $wrapThemesOther).length);

				$('> .learning-online-add-ons', $wrapAddon).remove();
				$wrapAddon.append($('> .learning-online-add-ons', $clone));
		...
		return $.each(this, function () {
			...
			if (!tabs) {
				$el.find('.heading-tabs a').click(function (e) {
					...
					$el.find('input[name="learning-online-meta-box-tab"]').val(id);
					LP.setUrl(url);
	...
	$(window).load(function () {
		...
		if ($('.learningonline-advertis-admin').length) {
			$('.learningonline-advertis-admin').learningonlineSlider();
		}
		$('.learning-online-tabs').metaBoxTabs();
		//
