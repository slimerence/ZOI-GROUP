...
( function ($, Backbone, _) {
	...
	LP.ModalSearchItems.View = Backbone.View.extend({
		tagName                  : 'div',
		id                       : 'learning-online-modal-search-items',
		options                  : {
		...
		initialize               : function (options) {
			...
			this.render();
			LP.Hook
				.addAction('learning_online_message_box_before_resize', function () {
					that.$('article').css('height', '');
				})
				.addAction('learning_online_message_box_resize', function (height, app) {
					that.$('article').css({
//						height: height - 135
                                            height: height - 50 - $( '#learning-online-modal-search-items header' ).height() - $( '#learning-online-modal-search-items footer' ).height()
					});
				});
		},
		render                   : function () {
			...
			$(document.body).trigger('learning_online_modal_search_items_loaded', this);
		},
		...
		_fetchItems              : function (response) {
			this.$('article .lp-list-items').removeClass('lp-ajaxload').html(response.html);
                        if ( this.$('.learningonline-search-notices').length == 0 ) {
//                            console.debug( $('#learning-online-modal-search-items footer') );
                            $('#learning-online-modal-search-items header').prepend( response.notices );
                        }
			LP.log(response.html);
			this.refreshModal();
			$(document.body).trigger('learning_online_modal_search_items_fetch', this);
		},
		search                   : function (args) {
			...
			$.ajax({
				...
				data    : {
					action    		: 'learningonline_modal_search_items',
...
//                                        console.debug( that.$('#learning-online-modal-search-items footer') );
//                                        that.$('#learning-online-modal-search-items footer').append( response.notices );
				}
			});
		},
		_closeModal              : function (e) {
			e.preventDefault();
			$(document.body).trigger('learning_online_modal_search_items_before_remove', this);
...
//			});
			...
			this.remove();
			LP.MessageBox.hide();
			$(document.body).trigger('learning_online_modal_search_items_removed', this);
		},
		_addItems                : function (e) {
                        e.preventDefault();
			$(document.body).trigger('learning_online_modal_search_items_response', [this, this.getItems()]);
			this.refreshModal(e);
		...
		keyboardActions          : function (e) {
			...
			if (e.target.tagName && ( e.target.tagName.toLowerCase() === 'input' || e.target.tagName.toLowerCase() === 'textarea' ) && e.target.value != this.searchTerm) {
				...
				this.searchTimer = setTimeout(function () {
					that.search({
						term   : e.target.value,
						exclude: LP.Hook.applyFilters('learning_online_modal_search_items_exclude', that.options.exclude, that)
					});
				}, 300);
