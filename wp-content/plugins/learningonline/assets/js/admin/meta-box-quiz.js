...
(function ($) {
	...
	$(document).ready(function () {
		...
		var LP_Quiz_Question_View = window.LP_Quiz_Question_View = Backbone.View.extend({
			el                      : 'body',
			events                  : {
				'click #learning-online-button-add-question'                   : 'showListQuestions',
				...
				'change input.learning-online-question-duration'               : 'updateQuizDuration',
			},
			initialize              : function () {
				$('#learning-online-list-questions').sortable({
			...
			updateQuizDuration      : function () {
				var duration = 0;
				$('#_lp_duration_select').val('minute');
				$('input.learning-online-question-duration').each(function (index, el) {
				...
				var $form = $input.closest('.lp-modal-search'),
					...
					_search = function () {
						$.ajax({
							...
							data    : {
								action : 'learningonline_search_questions',
								quiz_id: $('#post_ID').val(),
								term   : text,
			...
			getSelectedItems        : function (exclude) {
				return this.$('.learning-online-question[data-id]').map(function () {
			...
			addQuestion             : function (args) {
				...
				var that = this,
					post_data = $.extend({
						action : 'learningonline_add_quiz_question',
						quiz_id: $('#post_ID').val()
					}, args);
