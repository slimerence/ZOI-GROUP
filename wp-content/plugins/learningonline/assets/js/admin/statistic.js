;(function ($) {
		...
		return $.each(this, function () {
			var $buttons = $('.chart-buttons button').on('click', function () {
					var $button = $(this),
						...
						from = '', to = '',
						$container = $('#learning-online-chart');
					...
					$.ajax({
						url     : 'admin-ajax.php',
						data    : {
							action: 'learningonline_load_chart',
							type  : type,
							range : [from, to]
						},
						dataType: 'text',
	...
	$.fn.LP_Statistic_Orders = function () {
		...
		$('#report-by-course-id').select2({
				...
				ajax: {
					url: ajaxurl+'?action=learningonline_search_course',
					dataType: 'json',
					quietMillis: 250,
		...
		$('#report-by-course-category-id').select2({
				...
				minimumInputLength: 1,
				ajax: {
					url: ajaxurl+'?action=learningonline_search_course_category',
					dataType: 'json',
					quietMillis: 250,
					data: function (term, page) {
						return {
	...
	$(document).ready(function () {
		...
		$('.learning-online-statistic-users').LP_Statistic_Users();
		$('.learning-online-statistic-courses').LP_Statistic_Courses();
		$('.learning-online-statistic-orders').LP_Statistic_Orders();
