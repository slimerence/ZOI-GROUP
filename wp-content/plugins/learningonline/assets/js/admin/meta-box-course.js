...
(function ($) {
	...
	$(document).ready(function () {
		...
		var LP_Curriculum_View = window.LP_Curriculum_View = Backbone.View.extend({
			...
			events                  : {
				...
				'click .learning-online-dropdown-item-types > li a' : '_changeItemType',
				'change .lp-item-name '                         : '_updateItem',
				'focus .lp-item-name'                           : '_focusItem',
				'blur .lp-item-name'                            : '_blurItem',
				'click #learning-online-toggle-course-results'      : '_toggleCourseResults'
			},
			removeSectionIds        : [],
			removeItemIds           : [],
			...
			initialize              : function (model) {
				...
				this.initPage();
				LP.Hook.addAction('learning_online_message_box_before_resize', this.resetModalSearch);
				LP.Hook.addAction('learning_online_message_box_resize', this.updateModalSearch);
				$(document).on('learning_online_modal_search_items_response', this.addItemsToSection);
				LP.Hook.addFilter('learning_online_modal_search_items_exclude', this.getSelectedItems);
				LP.Hook.addFilter('learning_online_create_new_item', this.newItemFilter);
			...
			_changeItemType         : function (e) {
				...
				var pos = ( iconIindex === 0 ) ? 1 : iconIindex,
					$rep = $icon.closest('.learning-online-dropdown-item-types').find('>span'),
			...
			initPage                : function () {
				...
				this.$el
					.find('.section-item-icon .learning-online-dropdown-item-types ul .item-selected')
					.parent()
					.hide();
				...
				$(document)
					...
					.on('change', 'input[name="learning_online_submit_course_notice_reviewer"]', this.updatePublishAction)
					.on('change', 'input[name="learning-online-submit-for-review"]', this.toggleMessage);

				$('input[name="learning_online_submit_course_notice_reviewer"]').trigger('change');
			...
			toggleItemType          : function (e) {
				...
				LP.Hook.doAction('learning_online_change_section_item_type', from, to);
			},
			...
			_addNewItem             : function (e) {
				...
				$.ajax({
					...
					data    : {
						action: 'learningonline_add_new_item',
						name  : $input.val(),
						type  : type
					},
					type    : 'post',
			...
			createItem              : function (args, $section) {
				...
				var tmpl = wp.template('section-item'),
					$item = $(tmpl(args || {}));
				$item = LP.Hook.applyFilters('learning_online_create_new_item', $item, $section);
				_makeListSortable();
				return $item;
			},
			...
			showFormItems           : function (type, action, $button, notices) {
				var $form = LP.ModalSearchItems({
					template  : 'tmpl-learning-online-search-items',
					type      : type,
			...
			sectionActionHandler    : function (e) {
				...
				switch (action) {
					...
					case 'add-lp_lesson':
						...
						break;
					default:
						LP.Hook.doAction('learning_online_section_button_click', $button, action, this);
				}
			},
			...
			addItemToSection        : function ($item, $section) {
				...
				if (!parseInt($item.attr('data-item_id'))) {
					...
					$item.removeClass('focus').find('.learning-online-dropdown-item-types ul').remove();
				}
				this.model.addItem(parseInt($item.attr('data-id')));
				LP.Hook.doAction('learning_online_add_item_to_section', $item, $section);
			},
			addNewItem              : function (e) {
				...
				$.ajax({
					url     : LP_Settings.ajax,
					data    : {
						action: 'learningonline_quick_add_item',
			...
			newItemFilter           : function ($item) {
				if (!$item) return;
				$item.find('.section-item-icon .learning-online-dropdown-item-types ul .item-selected')
					.parent()
					.hide();
				return $item;
			},
			...
			removeItemDB            : function (id) {
				$.ajax({
					url    : LP_Settings.ajax,
					data   : {
						action: 'learningonline_remove_course_items',
						id    : id
					},
					success: function (response) {
					}
						...
						LP.MessageBox.quickConfirm($button, {
							onOk: function (a) {
								...
								$.ajax({
									url    : LP_Settings.ajax,
									data   : {
										action: 'learningonline_remove_course_section',
	...
	function updateHiddenCurriculum(hidden) {
		...
		$.ajax({
			url    : LP_Settings.ajax,
			data   : {
				action   : 'learningonline_update_curriculum_section_state',
	...
	function _toggleEditorHandler(e) {
		if (e.type == '_click') {
			...
			$.ajax({
				...
				type   : 'post',
				data   : {
					action   : 'learningonline_update_editor_hidden',
					course_id: $('#post_ID').val(),
					is_hidden: is_hidden
				},
				success: function () {
	...
	function _translate(key, text_default) {
		var texts = learning_online_mb_course_localize || {},
			text = learning_online_mb_course_localize[key] || text_default || '';
		return text;
	}

	function _ready() {
		$('#learning-online-button-toggle-editor').on('click _click', _toggleEditorHandler).trigger('_click');
		...
		$('#publish').val(learning_online_mb_course_localize.save_course);
		...
		var pendingReview = $('input[name="learning-online-course-pending-review"]').val() == 'yes';
		...
		$('#post').submit(function (e) {
			...
			/*var $review = $('textarea[name="review_message"]', this),
			 ...
			 current_status = $('#learning-online-course-status').val(),
			 ...
			 var $check = $('input[name="learning_online_submit_course_notice_instructor"]').prop('checked', true);
			 $check.trigger('change');
			 return false;
			 } else {
			 }*/
		}).attr('novalidate', 'true');
		$('#learning-online-notice-check').change(function () {
			var that = this,
		...
		jQuery('#learning-online-notice-check').change(function () {
		...
		$(document).on('click.learning-online-toggle-course-results', '#learning-online-toggle-course-results a', function (e) {
			e.preventDefault();
			var $a = $(this),
				click = $a.data('click');
			$a.html($a.data(click));
		...
		if ($.inArray($('input[name="_lp_course_result"]:checked').val(), ['', 'evaluate_lesson', 'evaluate_final_quiz']) != -1) {
			$('#learning-online-toggle-course-results').closest('.rwmb-input').find('li:gt(1)').hide();
		}
