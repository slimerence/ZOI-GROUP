...
;(function ($) {
	...
	LP.Question = {
		_getEmptyOption: function(question_id){
			var $options = $('#learning-online-list-options-' + question_id + ' tbody .lp-list-option-empty');
		...
		addOption  : function (question_id, args) {
			...
			if( $newOption ){
				...
				var templateArgs = {
					...
					tmpl = wp.template($('#learning-online-question-' + question_id).attr('data-type') + '-option'),
					$list = $('#learning-online-list-options-' + question_id + ' tbody');
		...
		addQuestion: function (args) {
			...
			var post_data = $.extend({
				action: 'learningonline_add_question'
			}, args);
			...
			$.ajax({
				...
				success : function (response) {
					...
					$('#learning-online-list-questions').append($newQuestion);
					LP.Question._hideQuestion(args.id);
					LP.Hook.doAction('learning_online_add_quiz_question', $newQuestion, args);
				}
			});
		},
		_hideQuestion: function(question){
			if($.type( question ) == 'number' ) {
				question = $('#learning-online-dropdown-questions .question a[data-id="' + question + '"]').parent()
			}
	...
	function updateHiddenQuestions(hidden){
		if( hidden == undefined ) {
			...
			var len = $('.quiz-question-content').each(function () {
				if ($(this).is(':hidden')) {
					hidden.push($('.learning-online-question', this).attr('data-id'));
				}
			}).length;
			if( hidden.length == 0 ){
		...
		$.ajax({
			url    : LP_Settings.ajax,
			data   : {
				action: 'learningonline_update_quiz_question_state',
				quiz_id: $('#post_ID').val(),
				hidden: hidden
			},
			success: function(){
	...
	function _ready() {
		$('#learning-online-toggle-questions').on('click', function () {
			$(this).siblings('ul').toggle();
		});

		$doc.on('click', '#learning-online-dropdown-questions ul li a', function (e) {
			e.preventDefault();
			LP.Question.addQuestion({id: $(this).data('id')});
			$(this).closest('ul').hide();
		});

		$('#learning-online-button-add-question').on('click', function () {
			//LP.Question.addQuestion({name: $('#learning-online-question-name').val(), type: 'true_or_false'});
		});
		...
		$doc.on('click', '.add-question-option-button', function (e, data) {
			...
			var $select = $(this),
				$wrap = $select.closest('.learning-online-question'),
				...
				_do = function(){
					...
					$.ajax({
						...
						dataType: 'html',
						data    : {
							action: 'learningonline_convert_question_type',
						...
						success : function(response){
							...
							var $newOptions = $(response.html),
								$question = $('#learning-online-question-'+questionId),
							...
							LP.Hook.doAction('learning_online_convert_question_type', questionId, from, to, $newOptions);
							LP.MessageBox.hide();
						}
					});
				};
			...
			switch (action){
				...
				case 'collapse':
					...
					break;
				case 'duplicate':
					if ( jConfirm( learning_online_admin_localize.duplicate_question.message, learning_online_admin_localize.duplicate_question.title, function( confirm ){
                                            if ( confirm ) {
                                                ...
                                                $.ajax({
                                                    ...
                                                    data: {
                                                        ...
                                                        'action'      : 'learningonline_duplicate_question',
                                                        '_nonce'      : learning_online_admin_localize.nonces.duplicate_question
                                                    },
                                                    beforeSend: function(){
				...
				case 'remove':
					LP.MessageBox.quickConfirm($link, {
						onOk: function(a){
							...
							$.ajax({
								url: $link.attr('href'),
								/*data: {
								 action: 'learningonline_remove_quiz_question',
					...
					break;
					LP.MessageBox.show( learning_online_admin_localize.remove_question, {
						buttons: 'yesNo',
						data: $(this).closest('.quiz-question'),
						events: {
			...
			var $wrap = $(this).closest('.learning-online-question'),
