...
class LP_Quiz extends LP_Abstract_Course_Item {
	...
	public function __construct( $quiz ) {
		if ( empty( $GLOBALS['learningonline_question_answers'] ) ) {
			$GLOBALS['learningonline_question_answers'] = array();
		}
		if ( is_numeric( $quiz ) ) {
			$this->id = absint( $quiz );
		...
		_learning_online_get_quiz_questions( $this->id );
	...
	public function get_duration_html() {
		...
		if ( $duration ) {
			$duration = learning_online_seconds_to_time( $duration );
		} else {
			$duration = __( 'Unlimited', 'learningonline' );
		}
		return apply_filters( 'learning_online_quiz_duration_html', $duration, $this );
	}
	...
	public function get_localize() {
		$localize = array(
			'confirm_finish_quiz' => array(
				'title'   => __( 'Finish quiz', 'learningonline' ),
				'message' => __( 'Are you sure you want to finish this quiz?', 'learningonline' )
			),
			'confirm_retake_quiz' => array(
				'title'   => __( 'Retake quiz', 'learningonline' ),
				'message' => __( 'Are you sure you want to retake this quiz?', 'learningonline' )
			),
			'quiz_time_is_over'   => array(
				'title'   => __( 'Time out!', 'learningonline' ),
				'message' => __( 'The time is over! Your quiz will automate come to finish', 'learningonline' )
			),
			'finished_quiz'       => __( 'Congrats! You have finished this quiz', 'learningonline' ),
			'retaken_quiz'        => __( 'Congrats! You have re-taken this quiz. Please wait a moment and the page will reload', 'learningonline' )
		);
		return apply_filters( 'learning_online_single_quiz_localize', $localize, $this );
	}
	...
	public function get_settings( $user_id = 0, $course_id = 0, $force = false ) {
		...
		if ( !array_key_exists( $key, $quiz_params ) || $force ) {
			...
			if ( $results = $user->get_quiz_results( $this->id, $course_id, $force ) ) {
				...
				$questions = learning_online_get_quiz_questions();
		...
		return apply_filters( 'learning_online_single_quiz_params', $quiz_params[$key], $this );
	}
	...
	public function get_question_params( $ids, $current = 0 ) {
		...
		if ( $questions = $this->get_questions() ) {
			...
			foreach ( $questions as $question_id => $question ) {
				...
				if ( $show_check_answer == 'yes' ) {
					//$_question->check_answer = learning_online_question_type_support( $question->type, 'check-answer' );
					$_question->hasCheckAnswer = learning_online_question_type_support( $question->type, 'check-answer' ) ? 'yes' : 'no';
				...
				/*if ( empty( $results[$row->id] ) ) {
					...
					if ( $show_check_answer == 'yes' ) {
						$results[$row->id]->check_answer = learning_online_question_type_support( $row->type, 'check-answer' );
		...
		return apply_filters( 'learning_online_quiz_param_questions', $results, $this->id );
	}

	public function frontend_assets() {
		if ( learning_online_is_course() && ( $quiz = LP()->global['course-item'] ) && $quiz->id == $this->id ) {
			$translate = $this->get_localize();
			LP_Assets::add_localize( $translate, false, 'learning-online-single-quiz' );
			LP_Assets::add_param( $this->get_settings(), false, 'learning-online-single-quiz' );
		}
	}
	...
	public function __get( $key ) {
		...
		switch ( $key ) {
			...
			case 'current_question':
				if ( ( $question = learning_online_get_request( 'question' ) ) && learning_online_is_quiz() ) {
			...
			default:
				if ( array_key_exists( $key, self::$_meta[$this->id] ) ) {
					...
					if ( $key == '_lp_duration' ) {
						$value = learning_online_human_time_to_seconds( $value );
	...
	public function get_content() {
		//if ( !did_action( 'learning_online_get_content_' . $this->id ) ) {
			...
			wp_reset_postdata();
			do_action( 'learning_online_get_content_' . $this->id );
		//}
		return $this->content;
	}
	...
	public function get_course( $args = null ) {
		if ( empty( $this->course ) ) {
			...
			$query = $wpdb->prepare( "
				...
				INNER JOIN {$wpdb->learningonline_section_items} si on si.section_id = s.section_id AND si.item_id = %d
				", $this->id );
		...
		if ( empty( $this->user_id ) ) {
			...
			if ( !$user_id ) {
				if ( !$this->is_require_enrollment() ) {
					$user_id = $_SESSION['learning_online_temp_user_id'];
				}
			}
	...
	public function get_questions( $force = false ) {
		...
		return apply_filters( 'learning_online_quiz_questions', $questions, $this->id, $force );
	...
	private function _get_questions() {
		...
		$query               = $wpdb->prepare( "
			...
			FROM {$wpdb->learningonline_quiz_questions} qq INNER JOIN {$wpdb->posts} p ON p.ID = qq.question_id AND p.post_type = %s
	...
	public function get_questions_( $force = false ) {
		...
		if ( !array_key_exists( 'questions', self::$_meta[$this->id] ) || $force ) {
			...
			/*$query = $wpdb->prepare( "
					SELECT q.*, qq.params
					FROM {$wpdb->posts} q
					INNER JOIN {$wpdb->learningonline_quiz_questions} qq ON qq.question_id = q.ID
					AND q.post_type = %s
					AND qq.quiz_id = %d
		...
		", '_lp_type', 'lp_question' );
			...
			if ( $this_questions = $wpdb->get_results( $query, OBJECT_K ) ) {
				...
				foreach ( $this_questions as $id => $question ) {
					...
					$GLOBALS['learningonline_question_answers'][$id] = array();
				...
				if ( $answers = $wpdb->get_results( "
					SELECT *
					FROM {$wpdb->learningonline_question_answers}
				...
				" )
					...
					foreach ( $answers as $answer ) {
						$GLOBALS['learningonline_question_answers'][$answer->question_id][$answer->question_answer_id]       = (array) maybe_unserialize( $answer->answer_data );
						$GLOBALS['learningonline_question_answers'][$answer->question_id][$answer->question_answer_id]['id'] = $answer->question_answer_id;
					}
					//print_r($GLOBALS['learningonline_question_answers']);
				}
			}
		} else {
			$this_questions = self::$_meta[$this->id]['questions'];
		}
		return apply_filters( 'learning_online_quiz_questions', $this_questions, $this->id, $force );
	}
	...
	public function has( $feature ) {
		...
		if ( is_callable( $callback ) ) {
			...
			throw new Exception( sprintf( __( 'The function %s doesn\'t exists', 'learningonline' ), $feature ) );
		}
	}
	...
	public function get_mark( $force = false ) {
		...
		return apply_filters( 'learning_online_quiz_mark', $this->_mark, $this->id );
	}
	...
	public function get_question_link( $question_id = null ) {
		...
		return apply_filters( 'learning_online_quiz_question_permalink', $permalink, $question_id, $this );
	}
	...
	public function check_question( $question_id, $user_id ) {
		...
		$checked = (array) learning_online_get_user_quiz_meta( $history->history_id, 'checked' );
		$checked = array_filter( $checked );
		if ( !in_array( $question_id, $checked ) ) {
			$checked[] = $question_id;
		}

		learning_online_update_user_quiz_meta( $history->history_id, 'checked', $checked );
	...
	private static function get_quiz_object( $the_quiz ) {
		...
		return apply_filters( 'learning_online_quiz_object', $the_quiz );
	}
