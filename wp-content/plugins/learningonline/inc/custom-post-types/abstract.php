...
abstract class LP_Abstract_Post_Type {
	...
	public function add_meta_boxes() {
		if ( $this->_post_type != learning_online_get_requested_post_type() ) {
			return;
		}
		do_action( 'learning_online_add_meta_boxes', $this->_post_type, $this );
		do_action( "learning_online_{$this->_post_type}_add_meta_boxes", $this );
		if ( !$this->_meta_boxes ) {
			return;
		}
	...
	public function updated_messages( $messages ) {
		...
		$messages[$this->_post_type] = array(
			...
			1  => sprintf( '%s %s', $post_type_object->labels->singular_name, __( 'updated.', 'learningonline' ) ),
			2  => __( 'Custom field updated.', 'learningonline' ),
			3  => __( 'Custom field deleted.', 'learningonline' ),
			4  => sprintf( '%s %s', $post_type_object->labels->singular_name, __( 'updated.', 'learningonline' ) ),
			/* translators: %s: date and time of the revision */
			5  => isset( $_GET['revision'] ) ? sprintf( __( 'Lesson restored to revision from %s', 'learningonline' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6  => sprintf( '%s %s', $post_type_object->labels->singular_name, __( 'published.', 'learningonline' ) ),
			7  => sprintf( '%s %s', $post_type_object->labels->singular_name, __( 'saved.', 'learningonline' ) ),
			8  => sprintf( '%s %s', $post_type_object->labels->singular_name, __( 'submitted.', 'learningonline' ) ),
			9  => sprintf(
				sprintf( '%s %s', $post_type_object->labels->singular_name, __( 'scheduled for: <strong>%1$s</strong>.', 'learningonline' ) ),
				// translators: Publish box date format, see http://php.net/date
				date_i18n( __( 'M j, Y @ G:i', 'learningonline' ), strtotime( $post->post_date ) )
			),
			10 => sprintf( '% %s', $post_type_object->labels->singular_name, __( 'draft updated.', 'learningonline' ) )
		);

		if ( $post_type_object->publicly_queryable ) {
			$permalink = get_permalink( $post->ID );

			$view_link = sprintf( ' <a href="%s">%s</a>', esc_url( $permalink ), sprintf( '%s %s', __( 'View', 'learningonline' ), $post_type_object->labels->singular_name ) );
			switch ( $this->_post_type ) {
				case LP_LESSON_CPT:
				case LP_QUIZ_CPT:
					$view_link = learning_online_get_item_course_id( $post->ID, $post->post_type ) ? $view_link : '';
					break;
				case LP_ORDER_CPT:
					$order     = learning_online_get_order( $post->ID );
					$view_link = $order->get_view_order_url();
					$view_link = sprintf( ' <a href="%s">%s</a>', esc_url( $view_link ), sprintf( '%s %s', __( 'View', 'learningonline' ), $post_type_object->labels->singular_name ) );
					break;
			...
			$preview_link      = sprintf( ' <a target="_blank" href="%s">%s</a>', esc_url( $preview_permalink ), sprintf( '%s %s', __( 'Preview', 'learningonline' ), $post_type_object->labels->singular_name ) );
