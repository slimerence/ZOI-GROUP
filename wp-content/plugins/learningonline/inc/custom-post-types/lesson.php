...
if ( !class_exists( 'LP_Lesson_Post_Type' ) ) {
	...
	final class LP_Lesson_Post_Type extends LP_Abstract_Post_Type {
		...
		public function hide_view_lesson_link_if_not_assigned() {
            ...
            if ( $current_screen->id === LP_LESSON_CPT && !learning_online_get_item_course_id( $post->ID, $post->post_type ) ) {
                ?>
                <style type="text/css">
                    #wp-admin-bar-view {
                        display: none;
		...
		public function register() {
			return
				array(
					'labels'             => array(
						'name'               => __( 'Lessons', 'learningonline' ),
						'menu_name'          => __( 'Lessons', 'learningonline' ),
						'singular_name'      => __( 'Lesson', 'learningonline' ),
						'add_new_item'       => __( 'Add New Lesson', 'learningonline' ),
						'all_items'          => __( 'Lessons', 'learningonline' ),
						'view_item'          => __( 'View Lesson', 'learningonline' ),
						'add_new'            => __( 'Add New', 'learningonline' ),
						'edit_item'          => __( 'Edit Lesson', 'learningonline' ),
						'update_item'        => __( 'Update Lesson', 'learningonline' ),
						'search_items'       => __( 'Search Lessons', 'learningonline' ),
						'not_found'          => __( 'No lesson found', 'learningonline' ),
						'not_found_in_trash' => __( 'No lesson found in Trash', 'learningonline' ),
					),
		...
		public function add_meta_boxes() {
			$prefix     = '_lp_';
			$meta_boxes = apply_filters( 'learning_online_lesson_meta_box_args',
				array(
					'id'     => 'lesson_settings',
					'title'  => __( 'Lesson Settings', 'learningonline' ),
					'pages'  => array( LP_LESSON_CPT ),
					'fields' => array(
						array(
							'name'         => __( 'Lesson Duration', 'learningonline' ),
							'id'           => "{$prefix}duration",
							'type'         => 'number',
							'type'         => 'duration',//'number',
							'default_time' => 'minute',
							'desc'         => __( 'Duration of the lesson. Set 0 to disable', 'learningonline' ),
							'std'          => 30,
						),
						array(
							'name'    => __( 'Preview Lesson', 'learningonline' ),
							'id'      => "{$prefix}preview",
							'type'    => 'yes_no',
							'desc'    => __( 'If this is a preview lesson, then student can view this lesson content without taking the course', 'learningonline' ),
							'std' => 'no'
						)
					)
				)
            ...
            <script>
				...
				form.submit(function (evt) {
					...
					if (0 == $title.val().length) {
						alert('<?php _e( 'Please enter the title of the lesson', 'learningonline' );?>');
						$title.focus();
						is_error = true;
					}
					if (is_error) {
		...
		public function columns_head( $columns ) {
			...
			if ( current_theme_supports( 'post-formats' ) ) {
				$new_columns['format']   = __( 'Format', 'learningonline' );
				$new_columns['duration'] = __( 'Duration', 'learningonline' );
			}
			$new_columns['preview'] = __( 'Preview', 'learningonline' );
		...
		public function columns_content( $name, $post_id = 0 ) {
			switch ( $name ) {
				case LP_COURSE_CPT:
					...
					if ( $courses ) {
						foreach ( $courses as $course ) {
							...
							echo "&nbsp;|&nbsp;";
							if ( $course_id = learning_online_get_request( 'filter_course' ) ) {
				...
				case 'preview':
					printf(
						'<input type="checkbox" class="learning-online-checkbox learning-online-toggle-lesson-preview" %s value="%s" data-nonce="%s" />',
						get_post_meta( $post_id, '_lp_preview', true ) == 'yes' ? ' checked="checked"' : '',
						$post_id,
						wp_create_nonce( 'learning-online-toggle-lesson-preview' )
					);
					break;
				case 'format':
					learning_online_item_meta_format( $post_id, __( 'Standard', 'learningonline' ) );
					break;
				case 'duration':
		...
		public function posts_join_paged( $join ) {
			...
			if ( $this->_filter_course() || ( $this->_get_orderby() == 'course-name' ) || ( $this->_get_search() ) ) {
				$join .= " LEFT JOIN {$wpdb->prefix}learningonline_section_items si ON si.item_id = {$wpdb->posts}.ID";
		...
		public static function create_default_meta( $id ) {
			$meta = apply_filters( 'learning_online_default_lesson_meta',
				array(
					'_lp_duration' => 10,
					'_lp_preview'  => 'no'
				)
