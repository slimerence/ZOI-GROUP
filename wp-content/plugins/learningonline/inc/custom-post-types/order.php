...
if ( !class_exists( 'LP_Order_Post_Type' ) ) {
	...
	final class LP_Order_Post_Type extends LP_Abstract_Post_Type {
		...
		public function post_new() {
			global $post;
			if ( $post && $post->post_type == 'lp_order' && $post->post_status == 'auto-draft' && learning_online_get_request( 'multi-users' ) == 'yes' ) {
		...
		public function delete_order_data( $post_id ) {
			...
			// get order items
			$query = $wpdb->prepare( "
				SELECT order_item_id FROM {$wpdb->prefix}learningonline_order_items
				WHERE order_id = %d
			", $post_id );
			if ( $item_ids = $wpdb->get_col( $query ) ) {
				...
				$query = "
					DELETE FROM {$wpdb->prefix}learningonline_order_itemmeta
					WHERE learningonline_order_item_id IN(" . join( ',', $item_ids ) . ")
				";
				$wpdb->query( $query );

				// delete order items
				$query = $wpdb->prepare( "
					DELETE FROM {$wpdb->prefix}learningonline_order_items
					WHERE order_id = %d
				", $post_id );
				$wpdb->query( $query );
				...
				if ( $course_ids = $wpdb->get_col( $query ) ) {
					...
					$query = $wpdb->prepare( "
						DELETE
						FROM ui, uim
						USING {$wpdb->prefix}learningonline_user_items AS ui
						LEFT JOIN {$wpdb->prefix}learningonline_user_itemmeta AS uim ON ui.user_item_id = uim.learningonline_user_item_id
						WHERE ref_id = %d AND user_id = %d AND ref_type = %s
					", $post_id, $user_id, LP_ORDER_CPT );
					$wpdb->query( $query );
		...
		private function _save_order_multi_users( $post_id, $user_id ) {
			...
			learning_online_reset_auto_increment( $wpdb->postmeta );
		}
		...
		public function save_order( $post_id ) {
			...
			if ( $action == 'editpost' && get_post_type( $post_id ) == 'lp_order' ) {
				...
				$user_id = learning_online_get_request( 'order-customer' );
				$order   = learning_online_get_order( $post_id );
				...
				$order_statuses = learning_online_get_order_statuses();
				$order_statuses = array_keys( $order_statuses );
				$status         = learning_online_get_request( 'order-status' );
				...
				$force = learning_online_get_request( 'trigger-order-action' ) == 'yes';
		...
		public function admin_footer() {
            ...
            <script type="text/javascript">
                jQuery(function ($) {
                    $('#post-search-input').prop('placeholder', '<?php esc_attr_e( 'Order number, user name, user email, course name etc...', 'learningonline' ); ?>').css('width', 400)
                });
            </script>
			<?php
		}
		...
		public function posts_join_paged( $join ) {
			...
			$join .= " INNER JOIN {$wpdb->learningonline_order_items} AS orderItem ON orderItem.order_id = {$wpdb->posts}.ID";
			return $join;
		}
		...
		public function admin_head() {
            ...
            <script>
                $('#update-order-status').click(function () {
                    var $button = $(this).attr('disabled', 'disabled').html('<?php _e( 'Processing...', 'learningonline' ); ?>');
                    $.ajax({
                        url     : ajaxurl,
                        type    : 'POST',
                        dataType: 'json',
                        data    : {
                            ...
                            status  : $('select[name="learning_online_order_status"]').val()
                        },
                        success : function (res) {
                            ...
                            $button.removeAttr('disabled').html('<?php _e( 'Apply', 'learningonline' ); ?>');
                        },
                        error   : function () {
                            $button.removeAttr('disabled').html('<?php _e( 'Apply', 'learningonline' ); ?>');
                        }
		...
		public function row_actions( $actions, $post ) {
			...
			if ( !empty( $actions['edit'] ) ) {
				$actions['edit'] = preg_replace( '/>(.*?)<\/a>/', ">" . __( 'View Order', 'learningonline' ) . "</a>", $actions['edit'] );
			}
			return $actions;
		}
		...
		public function columns_head( $existing ) {
			...
			$columns['title']         = __( 'Order', 'learningonline' );
			$columns['order_student'] = __( 'Student', 'learningonline' );
			$columns['order_items']   = __( 'Purchased', 'learningonline' );
			$columns['order_date']    = __( 'Date', 'learningonline' );
			$columns['order_total']   = __( 'Total', 'learningonline' );
//			$columns['order_status']  = '<span class="status_head tips" data-tip="' . esc_attr__( 'Status', 'learningonline' ) . '">' . esc_attr__( 'Status', 'learningonline' ) . '</span>';
		...
		public function order_title( $title, $post_id ) {
			if ( LP_ORDER_CPT == get_post_type( $post_id ) )
				$title = learning_online_transaction_order_number( $post_id );
			return $title;
		}
		...
		public function columns_content( $column, $post_id = 0 ) {
			global $post;
			$the_order = learning_online_get_order( $post->ID );
			switch ( $column ) {
				case 'order_student':
					if ( $the_order->is_multi_users() ) {
						...
						if ( $the_order->customer_exists() ) {
							$user = learning_online_get_user( $the_order->user_id );
					...
					break;
//				case 'order_status' :
//					echo learning_online_get_order_status_label( $post->ID );
//                    echo '<select name="order-status" data-status="lp-' . $the_order->get_status() .'">';
//
//                        $statuses = learning_online_get_order_statuses();
//                        foreach ( $statuses as $status => $status_name ) {
//                            echo '<option data-desc="' . esc_attr( _learning_online_get_order_status_description( $status ) ) . '" value="' .  esc_attr( $status ) . '" ' . selected( $status, 'lp-' . $the_order->get_status(), false ) . '>' . esc_html( $status_name ) . '</option>';
//                        }
//
//                    echo '</select>';
//					break;
				case 'order_date' :
					...
					if ( $time_diff > 0 && $time_diff < DAY_IN_SECONDS )
						$h_time = sprintf( __( '%s ago', 'learningonline' ), human_time_diff( $time ) );
					else
						$h_time = mysql2date( 'Y/m/d', $m_time );

					echo '<abbr title="' . esc_attr( $t_time ) . '">' . esc_html( apply_filters( 'learning_online_order_column_time', $h_time, $the_order ) ) . '</abbr>';
				...
				case 'order_items' :
					...
					foreach ( $the_order->get_items() as $item ) {
						if ( empty( $item['course_id'] ) || get_post_type( $item['course_id'] ) !== LP_COURSE_CPT ) {
							$links[] = __( 'Course does not exist', 'learningonline' );
						} else {
					...
					break;
				case 'order_total' :
					echo learning_online_format_price( $the_order->order_total, learning_online_get_currency_symbol( $the_order->order_currency ) );
					if ( $title = $the_order->get_payment_method_title() ) {
						?>
                        <div class="payment-method-title">
							<?php echo $the_order->order_total == 0 ? $title : sprintf( __( 'Pay via <strong>%s</strong>', 'learningonline' ), $title ); ?>
                        </div>
						<?php
					}
					break;
				case 'order_titlexx' :
					...
					?>
                    <div class="tips">
                        <a href="post.php?post=<?php echo $the_order->ID ?>&action=edit"><strong><?php echo learning_online_transaction_order_number( $order_number ); ?></strong></a>
                    </div>
					<?php
					break;
			}
		...
		public function register() {
			return array(
				'labels'             => array(
					'name'               => __( 'Orders', 'learningonline' ),
					'menu_name'          => __( 'Orders', 'learningonline' ),
					'singular_name'      => __( 'Order', 'learningonline' ),
					'add_new_item'       => __( 'Add New Order', 'learningonline' ),
					'edit_item'          => __( 'Order Details', 'learningonline' ),
					'all_items'          => __( 'Orders', 'learningonline' ),
					'view_item'          => __( 'View Order', 'learningonline' ),
					'add_new'            => __( 'Add New', 'learningonline' ),
					'update_item'        => __( 'Update Order', 'learningonline' ),
					'search_items'       => __( 'Search Orders', 'learningonline' ),
					'not_found'          => __( 'No order found', 'learningonline' ),
					'not_found_in_trash' => __( 'No order found in Trash', 'learningonline' )
				),
				'public'             => false,
				'show_ui'            => true,
		...
		public static function order_details( $post ) {
			learning_online_admin_view( 'meta-boxes/order/details.php', array( 'order' => LP_Order::instance( $post ) ) );
		}

		public static function order_actions( $post ) {
			learning_online_admin_view( 'meta-boxes/order/actions.php', array( 'order' => LP_Order::instance( $post ) ) );
		}
	...
	$order_post_type
		->add_meta_box( 'order_details', __( 'Order Details', 'learningonline' ), 'order_details', 'normal', 'high' )
		->add_meta_box( 'submitdiv', __( 'Order Actions', 'learningonline' ), 'order_actions', 'side', 'high' );
}
