...
if ( !class_exists( 'LP_Email_Enrolled_Course' ) ) {
	class LP_Email_Enrolled_Course extends LP_Email {
		...
		public function __construct() {
			...
			$this->title = __( 'Enrolled course', 'learningonline' );

			$this->template_html  = 'emails/enrolled-course.php';
			$this->template_plain = 'emails/plain/enrolled-course.php';

			$this->default_subject = __( '[{{site_title}}]  You have enrolled in this course ({{course_name}})', 'learningonline' );
			$this->default_heading = __( 'Enrolled course', 'learningonline' );
			...
			add_action( 'learning_online_user_enrolled_course_notification', array( $this, 'trigger' ), 99, 3 );

			parent::__construct();
		}

		public function admin_options( $settings_class ) {
			$view = learning_online_get_admin_view( 'settings/emails/enrolled-course.php' );
			include_once $view;
		}
