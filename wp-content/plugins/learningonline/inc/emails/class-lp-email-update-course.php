...
if ( ! class_exists( 'LP_Email_Update_Course' ) ) {
	...
	class LP_Email_Update_Course extends LP_Email {
		...
		public function __construct () {
			...
			$this->title = __( 'Update course', 'learningonline' );

			$this->template_html  = 'emails/update-course.php';
			$this->template_plain = 'emails/plain/update-course.php';

			$this->default_subject = __( '[{{site_title}}]  The course ({{course_name}}) has just been updated.', 'learningonline' );
			$this->default_heading = __( 'Update course', 'learningonline' );
...
//				'{{site_admin_name}}',
		...
		public function admin_options ( $settings_class ) {
			$view = learning_online_get_admin_view( 'settings/emails/update-course.php' );
			include_once $view;
		}
		...
		public function trigger ( $course_id, $user_id ) {
			...
			$is_send_mail = apply_filters( 'learning_online_has_send_mail_update_course', $course->send_mail_update_course, $course, $user );
