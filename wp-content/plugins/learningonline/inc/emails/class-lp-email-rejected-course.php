...
if ( !class_exists( 'LP_Email_Rejected_Course' ) ) {
	class LP_Email_Rejected_Course extends LP_Email {
		...
		public function __construct() {
			...
			$this->title = __( 'Rejected course', 'learningonline' );

			$this->template_html  = 'emails/rejected-course.php';
			$this->template_plain = 'emails/plain/rejected-course.php';

			$this->default_subject = __( '[{{site_title}}] Your course {{course_name}} has been rejected', 'learningonline' );
			$this->default_heading = __( 'Rejected course', 'learningonline' );
		...
		public function admin_options( $obj ) {
			$view = learning_online_get_admin_view( 'settings/emails/rejected-course.php' );
			include_once $view;
		}
