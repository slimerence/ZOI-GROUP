...
if ( !class_exists( 'LP_Email_User_Order_Completed' ) ) {
	...
	class LP_Email_User_Order_Completed extends LP_Email {
		...
		public function __construct() {
			...
			$this->title = __( 'User order completed', 'learningonline' );

			$this->template_html  = 'emails/user-order-completed.php';
			$this->template_plain = 'emails/plain/user-order-completed.php';

			$this->default_subject = __( 'Your order {{order_date}} is completed', 'learningonline' );
			$this->default_heading = __( 'Your order {{order_number}} is completed', 'learningonline' );
			...
			// $this->email_text_message_description = sprintf( '%s {{order_number}}, {{order_total}}, {{order_view_url}}, {{user_email}}, {{user_name}}, {{user_profile_url}}', __( 'Shortcodes', 'learningonline' ) );

			add_action( 'learning_online_order_status_completed_notification', array( $this, 'trigger' ) );

			parent::__construct();
		}

		public function admin_options( $settings_class ) {
			$view = learning_online_get_admin_view( 'settings/emails/user-order-completed.php' );
			include_once $view;
		}
		...
		public function trigger( $order_id ) {
			...
			$this->object = $this->get_common_template_data(
				...
				array(
					...
					'order_detail_url'  => learning_online_user_profile_link( $order->user_id, 'orders' ),///$order->get_view_order_url(),
			...
			if ( $order->is_multi_users() ) {
				if ( $recipient = $order->get_user_data() ) {
					...
					foreach ( $recipient as $uid => $data ) {
						...
						$this->object['order_detail_url'] = learning_online_user_profile_link( $uid, 'orders' );
