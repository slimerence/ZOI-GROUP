...
if ( ! class_exists( 'LP_Email_Enrolled_Course_Admin' ) ) {
	...
	class LP_Email_Enrolled_Course_Admin extends LP_Email {
		...
		public function __construct () {
			...
			$this->title = __( 'Enrolled course admin', 'learningonline' );

			$this->template_html  = 'emails/enrolled-course-admin.php';
			$this->template_plain = 'emails/plain/enrolled-course-admin.php';

			$this->default_subject = __( '[{{site_title}}]  ({{course_name}}) has been enrolled by {{custom_name}}', 'learningonline' );
			$this->default_heading = __( 'Course has been enrolled', 'learningonline' );
			...
			//$this->email_text_message_description = sprintf( '%s {{course_id}}, {{course_title}}, {{course_url}}, {{user_email}}, {{user_name}}, {{user_profile_url}}', __( 'Shortcodes', 'learningonline' ) );

			add_action( 'learning_online_user_enrolled_course_notification', array( $this, 'get_users_to_send' ), 99, 3 );

			parent::__construct();
		}

		public function admin_options ( $settings_class ) {
			$view = learning_online_get_admin_view( 'settings/emails/enrolled-course-admin.php' );
			include_once $view;
		}
		...
		public function get_users_to_send ( $course_id, $user_id, $user_course_id ) {
			...
			$all_users_id = apply_filters( 'learning_online_user_admin_send_mail_enrolled_course', $all_users_id, $course_id, $user_id, $user_course_id );
		...
		public function trigger ( $course_id, $user_id, $user_course_id ) {
			...
			$user_course_data = $wpdb->get_row(
				$wpdb->prepare( "SELECT * FROM {$wpdb->prefix}learningonline_user_items WHERE user_item_id = %d", $user_course_id )
			);
			$start_time       = '';
			$customer         = array(
				'name'        => '',
			...
			if ( ! empty( $user_course_data->start_time ) ) {
				...
				if ( ! empty( $data ) && ! empty( $data->data ) ) {
					...
					$customer['profile_url'] = learning_online_user_profile_link( $data->ID );
				}
			}
			...
			$this->object = $this->get_common_template_data(
				...
				array(
					...
					'user_id'            => $user_id,
					'user_name'          => learning_online_get_profile_display_name( $user ),
					'user_email'         => $user->user_email,
					'user_profile_url'   => learning_online_user_profile_link( $user->id ),
