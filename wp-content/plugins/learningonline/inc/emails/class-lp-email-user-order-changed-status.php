...
if ( ! class_exists( 'LP_Email_User_Order_Changed_Status' ) ) {
	...
	class LP_Email_User_Order_Changed_Status extends LP_Email {
		...
		public function __construct () {
			...
			$this->title             = __( 'User order changed status', 'learningonline' );
			$this->template_html     = 'emails/user-order-changed-status.php';
			$this->template_plain    = 'emails/plain/user-order-changed-status.php';
			$this->default_subject   = __( 'Your order {{order_date}} has just been changed status', 'learningonline' );
			$this->default_heading   = __( 'Your order {{order_number}} has just been changed status', 'learningonline' );
			...
			add_action( 'learning_online_update_order_status', array( $this, 'update_order_status' ), 10, 2 );
			parent::__construct();
		}

		public function admin_options ( $settings_class ) {
			$view = learning_online_get_admin_view( 'settings/emails/user-order-changed-status.php' );
			include_once $view;
		}
		...
		public function trigger ( $new_status, $order_id ) {
			...
			$this->object = $this->get_common_template_data(
				...
				array(
					...
					'order_detail_url'  => learning_online_user_profile_link( $order->user_id, 'orders' ),
		...
		public function get_recipient () {
			$user            = learning_online_get_user( $this->object['order']->user_id );
