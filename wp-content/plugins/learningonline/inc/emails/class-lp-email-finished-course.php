...
if ( !class_exists( 'LP_Email_Finished_Course' ) ) {
	class LP_Email_Finished_Course extends LP_Email {
		...
		public function __construct() {
			...
			$this->title = __( 'Finished course', 'learningonline' );

			$this->template_html  = 'emails/finished-course.php';
			$this->template_plain = 'emails/plain/finished-course.php';

			$this->default_subject = __( '[{{site_title}}] You have finished this course ({{course_name}})', 'learningonline' );
			$this->default_heading = __( 'Finished course', 'learningonline' );
		...
		public function admin_options( $settings_class ) {
			$view = learning_online_get_admin_view( 'settings/emails/finished-course.php' );
			include_once $view;
		}

		public function trigger( $course_id, $user_id, $result ) {

			if ( !$this->enable || !( $user = learning_online_get_user( $user_id ) ) ) {
				return;
			}
		...
		*/
		public function get_template_data( $content_type = 'plain' ) {
			...
			return array(
				...
				'profile_url'   => learning_online_user_profile_link( $this->object['user']->id ),
