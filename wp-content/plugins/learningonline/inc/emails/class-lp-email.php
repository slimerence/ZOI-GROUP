...
class LP_Email {
	...
	public function __construct() {
		...
		if ( empty( $this->template_path ) ) {
			$this->template_path = learning_online_template_path();
		}
	...
	public function get_variables_support() {
		return apply_filters( 'learning_online_email_variables_support', $this->support_variables, $this );
	}
	...
	protected function template_actions() {
		if (
			...
			if ( empty( $_GET['_learning_online_email_nonce'] ) || !wp_verify_nonce( $_GET['_learning_online_email_nonce'], 'learning_online_email_template_nonce' ) ) {
				return;
			}
		...
		if ( $template = $this->get_template( 'template_' . $type ) ) {
			if ( !empty( $template ) ) {
				...
				if ( wp_mkdir_p( dirname( $theme_file ) ) && !file_exists( $theme_file ) ) {
					...
					echo '<div class="updated"><p>' . __( 'Template file copied to theme.', 'learningonline' ) . '</p></div>';
				}
			}
		}
	}
		...
		if ( $template = $this->get_template( 'template_' . $type ) ) {
			...
			if ( !empty( $template ) ) {
				...
				if ( file_exists( $theme_file ) ) {
					unlink( $theme_file );
					echo '<div class="updated"><p>' . __( 'Template file deleted from theme.', 'learningonline' ) . '</p></div>';
				}
			}
		}
	}
	...
	public function format_string( $string ) {
		...
		$search  = apply_filters( 'learning_online_email_format_string_find', $search, $this );
		$replace = apply_filters( 'learning_online_email_format_string_replace', $replace, $this );
		return str_replace( $search, $replace, $string );
	}

	public function get_recipient() {
		return apply_filters( 'learning_online_email_recipient_' . $this->id, $this->recipient, $this->object );
	}

	public function get_subject() {
		return apply_filters( 'learning_online_email_subject_' . $this->id, $this->format_string( $this->subject ), $this->object );
	}
	...
	public function get_heading() {
		return apply_filters( 'learning_online_email_heading_' . $this->id, $this->format_string( $this->heading ), $this->object );
	}

	public function get_footer_text() {
		return apply_filters( 'learning_online_email_footer_text_' . $this->id, LP()->settings->get( 'emails_general.footer_text' ) );
	}
	...
	public function get_content_text_message() {
		return apply_filters( 'learning_online_email_text_message_' . $this->id, LP()->settings->get( 'emails_' . $this->id . '.content_text_message' ) );
	}

	public function get_headers() {
		return apply_filters( 'learning_online_email_headers', "Content-Type: " . $this->get_content_format() . "\r\n", $this->id, $this->object );
	}

	public function get_attachments() {
		return apply_filters( 'learning_online_email_attachments', array(), $this->id, $this->object );
	}
	...
	public function apply_style_inline( $content ) {
		if ( in_array( $this->get_content_format(), array( 'text/html', 'multipart/alternative' ) ) && class_exists( 'DOMDocument' ) ) {
			...
			// get CSS styles
			ob_start();
			learning_online_get_template( 'emails/email-styles.php' );
			$css = apply_filters( 'learning_online_email_styles', ob_get_clean(), $this->id, $this );
	...
	protected function save_template( $code, $path ) {
		...
		if ( current_user_can( 'edit_themes' ) && !empty( $code ) && !empty( $path ) ) {
			$saved = false;
			$file  = trailingslashit( get_stylesheet_directory() ) . trailingslashit( learning_online_template_path() ) . $path;
			...
			if ( !$saved ) {
				$redirect = add_query_arg( 'learning_online_error', urlencode( __( 'Could not write to template file.', 'learningonline' ) ) );
	...
	public function get_theme_template_file( $template, $template_path = null ) {
		return trailingslashit( get_stylesheet_directory() ) . trailingslashit( apply_filters( 'learning_online_template_directory', $template_path ? $template_path : learning_online_template_path(), $template ) ) . $template;
	}
	...
	public function send( $to, $subject, $message, $headers, $attachments ) {
		...
		$return  = false;
		$message = apply_filters( 'learning_online_mail_content', $this->apply_style_inline( $message ), $this );
		if ( !is_array( $to ) ) {
			$to = preg_split( '~\s?,\s?~', $to );
		}
		$separated = apply_filters( 'learning_online_email_to_separated', false, $to, $this );
		if ( !$separated ) {
	...
	public function get_common_template_data( $format = 'plain' ) {
		...
		$common     = array(
			...
			'site_admin_name'  => learning_online_get_profile_display_name( $admin_user ),
