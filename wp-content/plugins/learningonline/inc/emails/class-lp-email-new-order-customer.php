...
if ( !class_exists( 'LP_Email_New_Order_Customer' ) ) {
	...
	class LP_Email_New_Order_Customer extends LP_Email {
		...
		public function __construct() {
			...
			$this->title = __( 'New order customer', 'learningonline' );

			$this->template_html  = 'emails/new-order-customer.php';
			$this->template_plain = 'emails/plain/new-order-customer.php';

			$this->default_subject = __( '[{{site_title}}] Order placed', 'learningonline' );
			$this->default_heading = __( 'Order placed', 'learningonline' );
...
//        $this->recipient = LP()->settings->get( 'emails_' . $this->id . '.recipients', get_option( 'admin_email' ) );
			...
			add_action( 'learning_online_order_status_draft_to_pending_notification', array( $this, 'trigger' ) );
			add_action( 'learning_online_order_status_draft_to_processing_notification', array( $this, 'trigger' ) );
			add_action( 'learning_online_order_status_draft_to_on-hold_notification', array( $this, 'trigger' ) );
		...
		public function admin_options( $settings_class ) {
			$view = learning_online_get_admin_view( 'settings/emails/new-order-customer.php' );
			include_once $view;
		}
