...
class LP_Gateway_Paypal_Security extends LP_Gateway_Paypal{
	public function get_request_url( $order_id ) {
		...
		if ( !empty( $paypal_email )
			...
			$temp_id = learning_online_uniqid();
			...
			//$L_BUTTONVARS[]               = 'amount=' . learning_online_get_cart_total();
			//$L_BUTTONVARS[]               = 'quantity=1';
			$nonce                        = wp_create_nonce( 'learning-online-paypal-nonce' );

			$L_BUTTONVARS[] = 'business=' . $paypal_email;
			//$L_BUTTONVARS[] = 'item_name=' . learning_online_get_cart_description();
			$L_BUTTONVARS[] = 'return=' . add_query_arg( array( 'learning-online-transaction-method' => 'paypal-standard-secure', 'paypal-nonce' => $nonce ), learning_online_get_cart_course_url() );
			$L_BUTTONVARS[] = 'currency_code=' . learning_online_get_currency();//$general_settings['default-currency'];
			$L_BUTTONVARS[] = 'notify_url=' . learning_online_get_web_hook( 'paypal-standard-secure' );
			...
			$L_BUTTONVARS[] = 'cancel_return=' . learning_online_get_cart_course_url();
			...
			$L_BUTTONVARS = apply_filters( 'learning_online_paypal_standard_secure_button_vars', $L_BUTTONVARS );
			$count        = 0;
			...
			$button_request = apply_filters( 'learning_online_paypal_standard_secure_button_request', $button_request );
