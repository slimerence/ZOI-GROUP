...
return;
	...
	$order_data        = array(
		...
		'post_title'    => __( 'Order on ', 'learningonline' ) . ' ' . date( "l jS F Y h:i:s A", strtotime( $date ) )
	);
	$order_meta        = array(
	...
	learning_online_update_order( $order_data, $order_meta, $purchased_items );
	...
	*/
	if ( strtoupper( $_POST['payment_status'] ) == 'COMPLETED' ) {
		...
		if ( $course_price == $_POST['mc_gross'] && $params['currency'] == $_POST['mc_currency'] ) {
			...
			learning_online_update_user_course( $user_id, $course_id );
		}
		$purchased_items = array();
		$post_password   = substr( 'lpr_order_' . $_POST['txn_id'], 0, 20 );
		...
		if ( $order_id ) {
			learning_online_update_order_status( $order_id, 2 );
		}
	}
} else {
	/*
