...
class LP_Gateway_Paypal extends LP_Gateway_Abstract {
	...
	public function __construct() {
		...
		$this->title       = 'Paypal';
		$this->description = __( 'Pay with Paypal', 'learningonline' );
	...
	public function init() {
		if ( $this->settings->get( 'paypal_enable' ) ) {
			...
			//add_action( 'learning_online_take_course_paypal', array( $this, 'process_payment' ) );
			//add_action( 'learning_online_do_transaction_paypal-standard-secure', array( $this, 'process_order_paypal_standard_secure' ) );
			//add_action( 'learning_online_do_transaction_paypal-standard', array( $this, 'process_order_paypal_standard' ) );
			//add_action('learning_online_update_order_status', array($this, 'remove_transient'), 5, 2);
			//add_action('learning_online_payment_gateway_form_paypal', array($this, 'payment_form'));

			add_action( 'learning_online_web_hook_learning_online_paypal', array( $this, 'web_hook_process_paypal' ) );
		...
		add_filter( 'learning_online_payment_gateway_available_paypal', array( $this, 'paypal_available' ), 10, 2 );
	}

	public function register_web_hook() {
		learning_online_register_web_hook( 'paypal', 'learning_online_paypal' );
	}
	...
	public function get_order_id( $txn_id ) {
		...
		$args = array(
			'meta_key'    => '_learning_online_transaction_method_id',
			'meta_value'  => $txn_id,
			'numberposts' => 1, //we should only have one, so limit to 1
		);

		$orders = learning_online_get_orders( $args );
		print_r( $orders );
	...
	public function get_payment_form() {
		...
		if ( $error ) {
			$output .= learning_online_get_message( __( 'Paypal settings is not setup', 'learningonline' ), 'error' );
	...
	public function process_order_paypal_standard() {
		...
		if ( !empty( $_REQUEST['learning-online-transaction-method'] ) && ( 'paypal-standard' == $_REQUEST['learning-online-transaction-method'] ) ) {
			...
			if ( !empty( $_REQUEST['paypal-nonce'] ) && wp_verify_nonce( $_REQUEST['paypal-nonce'], 'learning-online-paypal-nonce' ) ) {
				...
				if ( !empty( $transaction_id ) && !empty( $transient_transaction_id ) && !empty( $transaction_status ) ) {
					...
					try {
						...
						if ( $transaction_object = learning_online_get_transient_transaction( 'lpps', $transient_transaction_id ) ) {

							learning_online_delete_transient_transaction( 'lpps', $transient_transaction_id );
							$order_id = $this->get_order_id( $transaction_id );
							$order_id = learning_online_add_transaction(
								array(
									'order_id'           => $order_id,
							...
							wp_redirect( ( $confirm_page_id = learning_online_get_page_id( 'taken_course_confirm' ) ) && get_post( $confirm_page_id ) ? learning_online_get_order_confirm_url( $order_id ) : get_site_url() );
							die();
						}
	...
	protected function payment_status_completed( $order, $request ) {
		...
		if ( 'completed' === $request['payment_status'] ) {
			$this->payment_complete( $order, ( !empty( $request['txn_id'] ) ? $request['txn_id'] : '' ), __( 'IPN payment completed', 'learningonline' ) );
			// save paypal fee
	...
	protected function add_line_item( $item_name, $quantity = 1, $amount = 0, $item_number = '' ) {
		...
		$this->line_items['item_name_' . $index]   = html_entity_decode( $item_name ? $item_name : __( 'Item', 'learningonline' ), ENT_NOQUOTES, 'UTF-8' );
	...
	public function get_paypal_args( $order ) {
		...
		$args = array_merge(
			array(
				...
				'cancel_return' => esc_url( learning_online_is_enable_cart() ? learning_online_get_page_link( 'cart' ) : get_site_url() ),
				'bn'            => 'LearningOnline_Cart',
				//'invoice'       => $order->id,
				'custom'        => json_encode( $custom ),
				'notify_url'    => get_site_url() . '/?' . learning_online_get_web_hook( 'paypal' ) . '=1',
				'email'         => $user->user_email
			),
			$this->get_item_lines()
		);
		//print_r($args);die();
		return apply_filters( 'learning_online_paypal_args', $args );
	}
