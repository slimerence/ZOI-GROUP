...
class LP_Gateways {
	...
	public function init() {
		if ( !$this->payment_gateways ) {
			...
			// Filter
			$gateways = apply_filters( 'learning_online_payment_method', $gateways );
			if ( $gateways ) {
				foreach ( $gateways as $k => $gateway ) {
					if ( is_string( $gateway ) && class_exists( $gateway ) ) {
						$gateway = new $gateway();
					}
					$this->payment_gateways[$k] = apply_filters( 'learning_online_payment_method_init', $gateway );
				}
			}
		}
	}
	...
	public function get_available_payment_gateways() {
		...
		foreach ( $this->payment_gateways as $slug => $gateway ) {
			...
			if ( apply_filters( 'learning_online_payment_gateway_available_' . $slug, false, $gateway ) ) {
				$_available_gateways[$slug] = $gateway;
			};
		}

		return apply_filters( 'learning_online_available_payment_gateways', $_available_gateways );
	}
