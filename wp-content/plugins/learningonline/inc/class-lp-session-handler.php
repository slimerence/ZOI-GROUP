...
class LP_Session_Handler {
	...
	public function __construct() {
		...
		$this->_cookie = 'wp_learning_online_session_' . COOKIEHASH;
		$this->_table  = $wpdb->prefix . 'learningonline_sessions';
		...
		add_action( 'learning_online_set_cart_cookies', array( $this, 'set_customer_session_cookie' ), 10 );
		add_action( 'learning_online_cleanup_sessions', array( $this, 'cleanup_sessions' ), 10 );
	...
	public function set_customer_session_cookie( $set ) {
		if ( $set ) {
			...
			// Set the cookie
			learning_online_setcookie( $this->_cookie, $cookie_value, $this->_session_expiration, apply_filters( 'learning_online_session_use_secure_cookie', false ) );
		}
	}
	...
	public function set_session_expiration() {
		$this->_session_expiration = time() + intval( apply_filters( 'learning_online_session_expiration', 60 * 60 * 48 ) );
	}
	...
	public function get_cache_prefix( $group = LP_SESSION_CACHE_GROUP ) {
		$prefix = wp_cache_get( 'learning_online_' . $group . '_cache_prefix', $group );

		if ( false === $prefix ) {
			$prefix = 1;
			wp_cache_set( 'learning_online_' . $group . '_cache_prefix', $prefix, $group );
		}

		return 'learning_online_cache_' . $prefix . '_';
	}
	...
	public function incr_cache_prefix( $group ) {
		wp_cache_incr( 'learning_online_' . $group . '_cache_prefix', 1, $group );
	}
	...
	public function destroy_session() {
		// Clear cookie
		learning_online_setcookie( $this->_cookie, '', time() - YEAR_IN_SECONDS, apply_filters( 'learning_online_session_secure_cookie', false ) );

		$this->delete_session( $this->_customer_id );

		// Clear cart
		learning_online_session_set( 'cart', '' );
 ...
 * @return array|string
 */
function learning_online_session_get( $key, $default = null ) {
 ...
 * @param $value
 */
function learning_online_session_set( $key, $value ) {
	return LP_Session_Handler::instance()->set( $key, $value );
}

function learning_online_session_remove( $key ) {
