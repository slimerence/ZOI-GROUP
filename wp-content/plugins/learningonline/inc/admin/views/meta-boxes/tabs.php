...
?>
<div class="learning-online-tabs initialize">
	<ul class="heading-tabs">
		...
		foreach ( $tabs as $k => $tab ) {
			...
			?>
			<a href="<?php echo add_query_arg( 'tab', $tab['id'], learning_online_get_current_url() ); ?>"><?php echo esc_html( $tab['title'] ); ?></a>
			<?php
			echo '</li>';
			$tabs[$k] = $tab;
		}
	...
	<ul class="learning-online-tab-content" data-text="<?php esc_attr_e( 'Initializing...', 'learningonline' ); ?>">
		<?php
		foreach ( $tabs as $tab ) {
			if ( empty( $tab['title'] ) ) {
				continue;
			...
			if ( !empty( $tab['content'] ) ) {
				...
				do_action( 'learning_online_meta_box_tab_content', $tab );
			}
			echo '</li>';
		}
		...
		if ( is_array( $current_tab ) ) {
			echo '<input type="hidden" name="learning-online-meta-box-tab" value="' . $current_tab['id'] . '" />';
		}
		?>
	</ul>
</div>