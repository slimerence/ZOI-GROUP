...
?>
<div class="learning-online-course-attributes">
	<?php
	$attributes        = learning_online_get_attributes();
	$course_attributes = learning_online_get_course_attributes( $postId );
	if ( $attributes ) {
		?>
		...
		printf( '<p class="description">%s <a class="button button-primary" href="%s">%s</a></p>',
			__( 'The is no attribute. Please add course attribute first', 'learningonline' ),
			esc_url( admin_url( 'edit-tags.php?taxonomy=course_attribute&post_type=lp_course' ) ),
			__( 'Add now', 'learningonline' )
		);
	}
	?>
</div>
