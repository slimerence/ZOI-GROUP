...
<script type="text/html" id="tmpl-quick-add-lesson">
	<div id="lpr-quick-add-lesson-form" class="lpr-quick-add-form">
		...
		<button type="button" class="button" data-action="cancel"><?php _e( 'Cancel [ESC]', 'learningonline' ); ?></button>
		<button type="button" class="button" data-action="add"><?php _e( 'Add [Enter]', 'learningonline' ); ?></button>
...
<script type="text/html" id="tmpl-lp-modal-search-lesson">
	<div id="lp-modal-search-lesson" class="lp-modal-search">
		...
		$lessons = learning_online_get_current_user()->get_lessons(array('orderby' => 'name', 'order' => 'ASC', 'posts_per_page' => -1));
		?>
		<div class="lp-search-items">
			<input type="text" name="lp-item-name" placeholder="<?php _e( 'Type here to search lesson', 'learningonline' ); ?>" />
			<!--<button type="button" class="button lp-add-new-item"><?php _e( 'Add New', 'learningonline' );?></button>-->
		</div>
		<ul>
			<?php
			if ( $lessons ) {
		...
		<button class="lp-add-item button" disabled="disabled" data-text="<?php _e( 'Add to section', 'learningonline' );?>"><?php _e( 'Add to section', 'learningonline' );?></button>
...
<script type="text/html" id="tmpl-lp-modal-search-quiz">
	<div id="lp-modal-search-quiz" class="lp-modal-search">
		...
		$quizzes = learning_online_get_current_user()->get_quizzes( $args );
		?>
		<div class="lp-search-items">
			<input type="text" name="lp-item-name" placeholder="<?php _e( 'Type here to search quiz', 'learningonline' ); ?>" />
			<!--<button type="button" class="button lp-add-new-item"><?php _e( 'Add New', 'learningonline' );?></button>-->
		</div>
		<ul>
			<?php
