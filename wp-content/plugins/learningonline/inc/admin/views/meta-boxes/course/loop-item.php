...
$support_types   = learning_online_course_get_support_item_types();

//$post_item = $item_id ? get_post( $item_id ) : learning_online_post_object( array( 'post_type' => LP_LESSON_CPT ) );
//if($post_item) {
...
<?php do_action( 'learning_online_admin_before_section_loop_item', $item, $section ); ?>

<tr <?php learning_online_admin_section_loop_item_class( $item, $section ); ?>
	...
	<?php do_action( 'learning_online_admin_begin_section_item', $item, $section ); ?>
	<td class="section-item-icon">
		<div class="learning-online-dropdown-item-types">
			<span class="learning-online-icon item-<?php echo $item->post_type; ?>" data-type="<?php echo $item->post_type; ?>"></span>
			<ul>
				<?php foreach ( $support_types as $_type => $text ) { ?>
					<li>
						<span
							class="learning-online-icon<?php echo $_type == $item_selected ? ' item-selected' : ''; ?> item-<?php echo $_type; ?>"
							data-content="<?php echo sprintf( __( 'Switch to %s', 'learningonline' ), $text ); ?>"
	...
	<td class="section-item-input">
		<input type="text"
			   ...
			   placeholder="<?php _e( 'Add new item here and press Enter', 'learningonline' ); ?>" />
	...
	<td class="section-item-actions">
		<p class="lp-item-actions lp-button-actions">
			<?php do_action( 'learning_online_admin_begin_section_item_actions', $item, $section ); ?>
			...
			<a href="" class="lp-item-action lp-remove dashicons dashicons-trash" data-confirm-remove="<?php _e( 'Are you sure you want to remove this item?', 'learningonline' ); ?>"></a>
            <span class="item-checkbox">
                <input type="checkbox" value="<?php echo $section_item_id; ?>" />
				<a class="dashicons dashicons-menu"></a>
            </span>
			<?php do_action( 'learning_online_admin_end_section_item_actions', $item, $section ); ?>
		</p>
	</td>
	<?php do_action( 'learning_online_admin_end_section_item', $item, $section ); ?>
</tr>