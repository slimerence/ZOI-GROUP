		...
		<div id="misc-publishing-actions">
			<?php if ( $can_publish ) : // Contributors don't get to choose the date of publish ?>
				...
				<div class="misc-pub-section">
					<label>
						<?php _e( 'Order status', 'learningonline' ); ?>
					</label>
					<select name="order-status" data-status="<?php echo 'lp-' . $order->get_status(); ?>">
						<?php
						$statuses = learning_online_get_order_statuses();
						foreach ( $statuses as $status => $status_name ) {
							echo '<option data-desc="' . esc_attr( _learning_online_get_order_status_description( $status ) ) . '" value="' . esc_attr( $status ) . '" ' . selected( $status, 'lp-' . $order->get_status(), false ) . '>' . esc_html( $status_name ) . '</option>';
						}
						?>
					</select>
					...
					<div class="description order-status-description">
						<?php if ( $order->get_status() == 'auto-draft' ) {
							echo _learning_online_get_order_status_description( 'lp-pending' );
						} ?>
						<?php echo _learning_online_get_order_status_description( 'lp-' . $order->get_status() ); ?>
					</div>
				</div>
				<div class="misc-pub-section hide-if-js order-action-section">
					<label for="trigger-order-action">
						...
						<?php _e( 'Trigger order status action', 'learningonline' ); ?>
					</label>
					<p class="description"><?php esc_attr_e( 'Check this option to force an action to be triggered. Normally, an action only is triggered after changing to an another action.', 'learningonline' ); ?></p>
				</div>

				<div class="misc-pub-section">
					<label>
						<?php _e( 'Customer', 'learningonline' ); ?>
					</label>
					<!--
					...
					if ( $order->is_multi_users() ) {
						...
						wp_dropdown_users(
							array(
								'show_option_none' => __( '[Guest]', 'learningonline' ),
								'name'             => 'order-customer',
								'id'               => null,
					...
					if ( $order->get_status() == 'auto-draft' && !$order->is_multi_users() ) {
						?>
						--
						<a href="<?php echo add_query_arg( 'multi-users', 'yes' ); ?>"><?php _e( 'Multiple users', 'learningonline' ); ?></a>
						<?php
					}
					//					}else{
