<script type="text/html" id="tmpl-lp-modal-quiz-questions">
	<div id="lp-modal-quiz-questions" class="lp-modal-search">
		<div class="lp-search-items">
			<input type="text" name="lp-item-name" placeholder="<?php _e( 'Type here to search the questions', 'learningonline' );?>" />
		...
		<ul class="lp-list-items">
			<?php
			$questions = learning_online_get_user_questions( null, array( 'posts_per_page' => 9999 ) );
			if ( $questions ) {
				foreach( $questions as $question ){
					...
					if( $question->post_title ){
						...
						$title = __( '(Untitled)', 'learningonline' );
					}
					?>
		...
		<button class="lp-add-item button" disabled="disabled" data-text="<?php _e( 'Add to quiz', 'learningonline' );?>"><?php _e( 'Add to quiz', 'learningonline' );?></button>
