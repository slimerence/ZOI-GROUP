...
$base_slug        = urldecode( ( $courses_page_id > 0 && get_post( $courses_page_id ) ) ? get_page_uri( $courses_page_id ) : _x( 'courses', 'default-slug', 'learningonline' ) );
$course_base      = _x( 'course', 'default-slug', 'learningonline' );

/*if ( !$course_permalink ) {
	global $wpdb;
	if ( $wpdb->get_results( $wpdb->prepare( "SELECT count(option_id) FROM {$wpdb->options} WHERE option_name = %s", 'learning_online_course_base' ) ) == 0 ) {
...
$structures = array(
	...
	3 => array(
		...
		'text'  => __( 'Courses base with category', 'learningonline' ),
		'code'  => esc_html( sprintf( '%s/%s/course-category/sample-course/', home_url(), $base_slug ) )
	)
);

$base_type = get_option( 'learning_online_course_base_type' );
$is_custom = ( $base_type == 'custom' && $course_permalink != '' );

?>
<?php foreach ( $structures as $k => $structure ): ?>
	<tr class="learning-online-single-course-permalink<?php if ( $k == 2 || $k == 3 ) {
		echo ' learning-online-courses-page-id';
		...
		<th>
			...
			?>
			<label>
				<input name="<?php echo $this->get_field_name( "course_base" ); ?>" type="radio" value="<?php echo esc_attr( $structure['value'] ); ?>" class="learning-online-course-base" <?php echo $is_checked; ?> />
...
<?php endforeach; ?>
<tr class="learning-online-single-course-permalink custom-base">
	<th>
		<label>
			<input name="<?php echo $this->get_field_name( "course_base" ); ?>" id="learning_online_custom_permalink" type="radio" value="custom" <?php checked( $is_custom, true ); ?> />
			<?php _e( 'Custom Base', 'learningonline' ); ?>
		</label>
	</th>
	<td>
		...
		<p class="description"><?php _e( 'Enter a custom base to use. A base <strong>must</strong> be set or WordPress will use default values instead.', 'learningonline' ); ?></p>
	</td>
</tr>
