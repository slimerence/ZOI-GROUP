...
<tr>
	...
	<td>
		<p><?php learning_online_email_formats_dropdown( array( 'name' => $settings_class->get_field_name( 'emails_' . $this->id . '[email_format]' ), 'id' => 'learning_online_email_formats', 'selected' => $email_format ) ); ?></p>
		<div id="templates">
			<?php
			$templates = array(
			...
			foreach ( $templates as $template_type => $title ) :
				...
				$template_dir  = $this->template_path;//learning_online_template_path();
				$classes       = array( 'learning-online-email-template' );
				...
				<div class="<?php echo join( ' ', $classes ); ?>">
					...
					<?php if ( $has_local_file ): ?>
						...
						<p class="description">
							<?php printf( __( 'This template has been overridden by your theme and can be found in: <code>%s</code>. <br />Please open the file in an editor program to edit', 'learningonline' ), $theme_folder . '/' . $template_dir . '/' . $template ); ?>
						</p>
					<?php endif; ?>
					<div class="<?php echo $has_local_file ? 'hide-if-js' : ''; ?>">
						<?php if ( $template_type == 'html' ): ?>
							...
							wp_editor(
								$content_html,
								'learning_online_emails_' . $this->id . '_' . $template_type,
								array(
									'textarea_rows' => 20,
									'wpautop'       => false,
						...
						<?php else: ?>
							<div class="editor">
								<textarea id="<?php echo esc_attr( 'learning_online_emails_' . $this->id . '_' . $template_type ); ?>" name="<?php echo $settings_class->get_field_name( 'emails_' . $this->id . '[email_content_plain]' ); ?>" class="code" cols="25" rows="20" style="width: 97%;"><?php echo $content_plain; ?></textarea>
							</div>
						<?php endif; ?>
					</div>
					...
					<?php if ( !$has_local_file ): ?>
						<?php if ( $this->get_variables_support() ): ?>
							<p>
								<strong><?php esc_html_e( 'Click on variables to add it into email content', 'learningonline' ); ?></strong>
							</p>
							<ol class="learning-online-email-variables<?php echo $template_type == 'html' ? ' has-editor' : ''; ?>" data-target="<?php echo esc_attr( 'learning_online_emails_' . $this->id . '_' . $template_type ); ?>">
						...
						<?php endif; ?>
						<p class="description">
							<?php printf( __( 'To override and edit this email template copy <code>%s</code> to your theme folder: <code>%s</code>.', 'learningonline' ), plugin_basename( $template_file ), $theme_folder . '/' . $template_dir . '/' . $template ); ?>
						</p>
					<?php endif; ?>
				</div>
				<?php
