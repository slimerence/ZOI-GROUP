...
switch ( $options['type'] ) {
	...
	case 'checkbox' :
		...
		$size             = learning_online_get_image_size( $image_size );
		...
		if ( has_filter( 'learning_online_get_image_size_' . $image_size ) ) {
			$disabled_attr    = 'disabled="disabled"';
			$disabled_message = "<p><small>" . __( 'The settings of this image size have been disabled because its values are being overwritten by a filter.', 'learningonline' ) . "</small></p>";
		}
		...
		<td class="forminp image_width_settings">
			...
			<label><input name="<?php echo esc_attr( $options['id'] ); ?>[crop]" <?php echo $disabled_attr; ?> id="<?php echo esc_attr( $options['id'] ); ?>-crop" type="checkbox" value="1" <?php checked( 1, $crop ); ?> /> <?php _e( 'Hard Crop?', 'learningonline' ); ?>
			</label>
	...
	default:
		do_action( 'learning_online_admin_field_' . $options['type'], $options );
		break;
}