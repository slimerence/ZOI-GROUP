...
class LP_Plugin_Install_List_Table extends WP_List_Table {
    ...
    private $repo_url = 'http://jwstheme.com/lprepo/';
    ...
    private function _get_items_from_wp( $args = array() ){
        ...
        $plugins = learning_online_get_add_ons_from_wp( $args );
        if( ( $tab == 'bundle_activate' ) && sizeof( $plugins ) ) {

            $_plugins = array_values( $plugins );
            $plugins = array();
            $bundle_activate = $learning_online_add_ons['bundle_activate'];
    ...
    private function _get_items_from_jwstheme( $add_ons ){
        ...
        if( file_exists( $cache ) && ( time() - filemtime( $cache ) ) < $timeover  ){
            ...
            $repo_url = 'http://jwstheme.com/lprepo/';
            ...
            foreach ($add_ons as $slug) {
                ...
                $item = array(
                    ...
                    'slug' => '',
                    'version' => '0.0',
                    'author' => '<a href="http://profiles.wordpress.org/jwstheme">jwstheme</a>',
                    'author_profile' => 'http://profiles.wordpress.org/jwstheme',
                    'contributors' => array(),
                    'requires' => '4.0',
                    'tested' => '4.2.2',
                    'rating' => 0,
                    ...
                    'homepage' => 'http://jwstheme.com/learningonline',
                    'short_description' => '',
                    'icons' => array(
    ...
    public function prepare_items( $args = array() ) {
        ...
        $tab = ! empty( $_REQUEST['tab'] ) ? $_REQUEST['tab'] : '';
        if( 'learning-online-addons' != $page ) return;

        if( 'more' == $tab ){
            $add_ons = $learning_online_add_ons['more'];
        ...
        if( ! $this->test_mode ){
            ...
            $this->_get_items_from_jwstheme( $add_ons );
        }
    }


    public function no_items() {
        $message = __( 'No plugin found.', 'learningonline' );
    ...
    public function display_rows() {
        ...
        foreach ( (array) $this->items as $kk => $plugin ) {
            ...
            if ( ! empty( $author ) ) {
                $author = ' <cite>' . sprintf( __( 'By %s', 'learningonline' ), $author ) . '</cite>';
            }

            $action_links = array();
            $plugin_file = learning_online_plugin_basename_from_slug( $plugin['slug'] );
            ...
            if ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) {
                ...
                if( $this->test_mode ){
                    $status = array(
                        ...
                        'url'       => admin_url( 'admin-ajax.php?action=learningonline_plugin_install&plugin=' . $plugin['slug'] )
                    );
                }else{
                ...
                switch ( $status['status'] ) {
                    case 'install':
                        if ( $status['url'] ) {
                            ...
                            $action_links[] = '<a class="install-now button jwstheme" data-action="install-now" data-slug="' . esc_attr( $plugin['slug'] ) . '" href="' . esc_url( $status['url'] ) . '&learningonline=active" aria-label="' . esc_attr( sprintf( __( 'Install %s now', 'learningonline' ), $name ) ) . '" data-name="' . esc_attr( $name ) . '">' . __( 'Install and Active', 'learningonline' ) . '</a>';
                        }
                    ...
                    case 'update_available':
                        if ( $status['url'] ) {
                            /* translators: 1: Plugin name and version */
                            $action_links[] = '<a class="update-now button jwstheme" data-action="update-now" data-plugin="' . esc_attr( $status['file'] ) . '" data-slug="' . esc_attr( $plugin['slug'] ) . '" href="' . esc_url( $status['url'] ) . '&learningonline=active" aria-label="' . esc_attr( sprintf( __( 'Update %s now', 'learningonline' ), $name ) ) . '" data-name="' . esc_attr( $name ) . '">' . __( 'Update Now', 'learningonline' ) . '</a>';
                        }
                    ...
                    case 'newer_installed':
                        if( is_plugin_active( $plugin_file ) ){
                            ...
                            $action_links[] = '<a class="active-now button jwstheme" data-action="active-now" href="'.wp_nonce_url( 'plugins.php?action=activate&learningonline=active&plugin='.$plugin_file, 'activate-plugin_' . $plugin_file ) . '" >' . _x('Enable', 'plugin') . '</a>';
                        }
                        break;
                }
            }
            ...
            */
            ...
            if( 'bundle_activate' == $tab ){
                ...
                if( learning_online_is_plugin_install( $plugin_file ) ) {

                    if (is_plugin_active( $plugin_file )) {
                        $message = sprintf('<span class="addon-status enabled">%s</span>', __( 'Enabled', 'learningonline' ));
                    } else {
                        $message = sprintf('<span class="addon-status disabled">%s</span>', __( 'Disabled', 'learningonline' ));
                    }
                }else{
                    $message = sprintf('<span class="addon-status not_install">%s</span>', __( 'Not Install', 'learningonline' ));
                }
            }
            ?>
            <div class="plugin-card plugin-card-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
                ...
                <div class="plugin-card-bottom">
                    ...
                    <div class="column-updated">
                        <strong><?php _e( 'Last Updated:', 'learningonline' ); ?></strong> <span title="<?php echo esc_attr( date_i18n( $date_format, $last_updated_timestamp ) ); ?>">
