 ...
 * @return mixed
 */
function learning_online_get_admin_view($name, $plugin_file = null)
{
    if (!preg_match('/\.(.*)$/', $name)) {
        $name .= '.php';
    }
    ...
    return apply_filters('learning_online_admin_view', $view, $name);
}
 ...
 * @return bool
 */
function learning_online_admin_view($name, $args = array(), $include_once = false, $return = false)
{
    $view = learning_online_get_admin_view($name, !empty($args['plugin_file']) ? $args['plugin_file'] : null);
    if (file_exists($view)) {
        ob_start();
        // extract parameters as local variables if passed
        is_array($args) && extract($args);
        do_action('learning_online_before_display_admin_view', $name, $args);
        if ($include_once) {
            include_once $view;
        } else {
            include $view;
        }
        do_action('learning_online_after_display_admin_view', $name, $args);
        $output = ob_get_clean();
        if (!$return) {
            echo $output;
        }
 ...
 * @return mixed|string
 */
function learning_online_pages_dropdown($name, $selected = false, $args = array())
{
    $id = null;
    $class = null;
    $css = null;
    $before = array(
        'add_new_page' => __('[ Add a new page ]', 'learningonline')
    );
    $after = null;
    $echo = true;
    $allow_create = true;
    ...
    $args = array(
        ...
        'sort_order' => 'ASC',
        'show_option_none' => __('Select Page', 'learningonline'),
        'class' => $class,
        'echo' => false,
        'selected' => $selected,
        'allow_create' => true
    ...
    $class .= 'learning-online-dropdown-pages';
    ...
    $replace .= " data-placeholder='" . __('Select a page&hellip;', 'learningonline') . "' id=";
    ...
    if ($allow_create) {
        ...
        <button class="button button-quick-add-page" data-id="<?php echo $id; ?>"
                type="button"><?php _e('Create', 'learningonline'); ?></button>
        <p class="learning-online-quick-add-page-inline <?php echo $id; ?> hide-if-js">
            <input type="text" placeholder="<?php esc_attr_e('New page title', 'learningonline'); ?>"/>
            <button class="button" type="button"><?php esc_html_e('Ok [Enter]', 'learningonline'); ?></button>
            <a href=""><?php _e('Cancel [ESC]', 'learningonline'); ?></a>
        </p>
        <p class="learning-online-quick-add-page-actions <?php echo $id; ?><?php echo $selected ? '' : ' hide-if-js'; ?>">
            <a class="edit-page" href="<?php echo get_edit_post_link($selected); ?>"
               target="_blank"><?php _e('Edit Page', 'learningonline'); ?></a>
            <a class="view-page" href="<?php echo get_permalink($selected); ?>"
               target="_blank"><?php _e('View Page', 'learningonline'); ?></a>
        </p>
 ...
 * @return string
 */
function learning_online_dropdown_question_types($args = array())
{
    $args = wp_parse_args(
        $args,
        array(
            'name' => 'learning-online-dropdown-question-types',
            'id' => '',
            'class' => '',
            'selected' => '',
            'echo' => true
...
function learning_online_field_question_duration($args = array(), $question)
{
    ...
    $args = wp_parse_args(
        $args,
        array(
            'name' => 'learning_online_question[' . $question->id . '][duration]',
            'id' => 'learning-online-question-duration-' . $question->id,
            'class' => 'learning-online-question-duration',
            'selected' => '',
            'echo' => true,
            'value' => 0,
            'step' => 1,
            'min' => 0,
            'placeholder' => __('Minutes', 'learningonline'),
        )
    );
    $args['value'] = $value;
 ...
 * @return string
 */
function learning_online_email_formats_dropdown($args = array())
{
    $args = wp_parse_args(
        $args,
        array(
            'name' => 'learning-online-dropdown-email-formats',
            'id' => '',
            'class' => '',
            'selected' => '',
            'echo' => true
    ...
    $formats = array(
        //'text_message' => __( 'Text message', 'learningonline' ),
        'plain_text' => __('Plain text', 'learningonline'),
        'html' => __('HTML', 'learningonline'),
        //'multipart'    => __( 'Multipart', 'learningonline' )
    );
 ...
 */
function learning_online_admin_localize_script()
{
    if (defined('LP_DOING_AJAX') || !is_admin()) {
        return;
    }
    $translate = array(
        'quizzes_is_not_available' => __('Quiz is not available', 'learningonline'),
        'lessons_is_not_available' => __('Lesson is not available', 'learningonline'),
        'duplicate_course' => array(
            'title' => __('Duplicate course', 'learningonline'),
            'message' => __('Duplicate course curriculum?', 'learningonline')
        ),
        'duplicate_question' => array(
            'title' => __('Duplicate Question', 'learningonline'),
            'message' => __('Do you want to duplicate this question?', 'learningonline')
        ),
        'remove_question' => __('Do you want to remove this question from quiz?', 'learningonline'),
        'nonces' => array(
...
add_action('init', 'learning_online_admin_localize_script');
 ...
 * @return mixed
 */
function learning_online_settings_tabs_array()
{
    $tabs = array(
        'general' => __('General', 'learningonline'),
        'courses' => __('Courses', 'learningonline'),
        'pages' => __('Pages', 'learningonline'),
        'payments' => __('Payments', 'learningonline'),
        'checkout' => __('Checkout', 'learningonline'),
        //'profile'  => __( 'Profile', 'learningonline' ),
        'emails' => __('Emails', 'learningonline')
    );

    return apply_filters('learning_online_settings_tabs_array', $tabs);
}
 ...
 * @return int
 */
function learning_online_get_order_by_time($by, $time)
{
    global $wpdb;
        ...
        case 'days':
            $orders = $wpdb->get_var(
                $wpdb->prepare(
                    ...
                    $user_id, LP_ORDER_CPT, 'publish', '_learning_online_transaction_status', 'completed', $y, $m, $d
                )
            );
            break;
        case 'months':
            $orders = $wpdb->get_var(
                $wpdb->prepare(
                    ...
                    $user_id, LP_ORDER_CPT, 'publish', '_learning_online_transaction_status', 'completed', $y, $m
                )
            );
            break;
    }
 ...
 * @return int
 */
function learning_online_get_courses_by_status($status)
{
    global $wpdb;
    $user_id = get_current_user_id();
 ...
 * @return int
 */
function learning_online_get_courses_by_price($fee)
{
    global $wpdb;
    $user_id = get_current_user_id();
    $courses = $wpdb->get_var(
 ...
 * @return array
 */
function learning_online_get_chart_students($from = null, $by = null, $time_ago)
{
    $labels = array();
    $datasets = array();
    if (is_null($from)) {
    ...
    for ($i = -$time_ago + 1; $i <= 0; $i++) {
        ...
        $datasets[0]['data'][] = learning_online_get_order_by_time($by, strtotime("$i $by", strtotime($from)));
    }
    $colors = learning_online_get_admin_colors();
 ...
 * @return array
 */
function learning_online_get_chart_users($from = null, $by = null, $time_ago)
{
    global $wpdb;
    ...
    $dataset_params = array(
        array(
            ...
            'color2' => '#FFF',
            'label' => __('All', 'learningonline')
        ),
        array(
            'color1' => 'rgba(212, 208, 203, %s)',
            'color2' => '#FFF',
            'label' => __('Instructors', 'learningonline')
        ),
        array(
            'color1' => 'rgba(234, 199, 155, %s)',
            'color2' => '#FFF',
            'label' => __('Students', 'learningonline')
        )
    );
 ...
 * @return array
 */
function learning_online_get_chart_courses($from = null, $by = null, $time_ago)
{
    global $wpdb;
    ...
    $dataset_params = array(
        ...
        array(
            'color1' => 'rgba(212, 208, 203, %s)',
            'color2' => '#FFF',
            'label' => __('Publish', 'learningonline')
        ),
        array(
            'color1' => 'rgba(234, 199, 155, %s)',
            'color2' => '#FFF',
            'label' => __('Pending', 'learningonline')
        ),
        array(
            'color1' => 'rgba(234, 199, 155, %s)',
            'color2' => '#FFF',
            'label' => __('Paid', 'learningonline')
        ),
        array(
            'color1' => 'rgba(234, 199, 155, %s)',
            'color2' => '#FFF',
            'label' => __('Free', 'learningonline')
        )
    );
 ...
 * @return array
 */
function learning_online_get_chart_orders($from = null, $by = null, $time_ago)
{
    global $wpdb;
//	var_dump( current_user_can(LP_TEACHER_ROLE) );
//	exit();
    $report_sales_by = learning_online_get_request('report_sales_by');
    $course_id = learning_online_get_request('course_id');
    $cat_id = learning_online_get_request('cat_id');
    ...
    if ('course' === $report_sales_by) {
        $sql_join .= " INNER JOIN `{$wpdb->prefix}learningonline_order_items` `lpoi` "
            . " ON o.ID=lpoi.order_id "
            . " INNER JOIN {$wpdb->prefix}learningonline_order_itemmeta loim "
            . " ON lpoi.order_item_id=loim.learningonline_order_item_id "
    ...
    if (current_user_can(LP_TEACHER_ROLE)) {
        ...
        $query_where .= $wpdb->prepare(" AND o.ID IN( SELECT oi.order_id
										FROM lptest.{$wpdb->prefix}learningonline_order_items oi
											inner join {$wpdb->prefix}learningonline_order_itemmeta oim
												on oi.order_item_id = oim.learningonline_order_item_id
...
//			$results['completed'][$v->d] = $v;
    ...
    $dataset_params = array(
        ...
        array(
            'color1' => 'rgba(212, 208, 203, %s)',
            'color2' => '#FFF',
            'label' => __('Completed', 'learningonline')
        ),
        array(
            'color1' => 'rgba(234, 199, 155, %s)',
            'color2' => '#FFF',
 ...
 * @return array
 */
function learning_online_get_chart_courses2()
{
    $labels = array(
        __('Pending Courses / Publish Courses', 'learningonline'),
        __('Free Courses / Priced Courses', 'learningonline')
    );
    $datasets = array();
    $datasets[0]['data'] = array(
        learning_online_get_courses_by_status('pending'),
        learning_online_get_courses_by_price('free')
    );
    $datasets[1]['data'] = array(
        learning_online_get_courses_by_status('publish'),
        learning_online_get_courses_by_price('not_free')
    );
 ...
 * @return array
 */
function learning_online_get_admin_colors()
{
 ...
 * @param array $chart
 */
function learning_online_process_chart($chart = array())
{
    $data = json_encode(
        array(
            'labels' => $chart['labels'],
 ...
 */
function learning_online_config_chart()
{
    $colors = learning_online_get_admin_colors();
    $config = array(
 ...
 * @return mixed
 */
function learning_online_add_row_action_link($actions)
{
    global $post;
    if (LP_COURSE_CPT == $post->post_type) {
        ...
        $duplicate_link = array(
            array(
                'link' => $duplicate_link,
                'title' => __('Duplicate this course', 'learningonline'),
                'class' => 'lp-duplicate-course'
            )
        );
        $links = apply_filters('learning_online_row_action_links', $duplicate_link);
        if (count($links) > 1) {
            ...
            $drop_down[] = '</ul>';
            $link = sprintf('<div class="lpr-row-actions"><a href="%s">%s</a>%s</div>', 'javascript: void(0);', __('Course', 'learningonline'), join("\n", $drop_down));
        } else {
        ...
        $link = sprintf('<a href="%s" class="lp-duplicate-lesson">%s</a>', $url, __('Duplicate this quiz', 'learningonline'));
        $actions['lpr-course-row-action'] = $link;
        ...
        $link = sprintf('<a href="%s" class="lp-duplicate-lesson">%s</a>', $url, __('Duplicate this question', 'learningonline'));
        $actions['lpr-course-row-action'] = $link;
        ...
        $link = sprintf('<a href="%s" class="lp-duplicate-lesson">%s</a>', $url, __('Duplicate this lesson', 'learningonline'));
        $actions['lpr-course-row-action'] = $link;
    }
...
add_filter('post_row_actions', 'learning_online_add_row_action_link');
add_filter('page_row_actions', 'learning_online_add_row_action_link');

function learning_online_copy_post_meta($from_id, $to_id)
{
    global $wpdb;
 ...
 * @author  TuNN
 */
function learning_online_process_duplicate_action()
{
    ...
    if (isset($_REQUEST['action']) && $action == 'lp-duplicate-course') {
        ...
        if (!wp_verify_nonce($nonce, 'lp-duplicate-' . $post_id)) {
            wp_die(__('Error', 'learningonline'));
        }
        ...
        if (!($post_id && $post = get_post($post_id))) {
            wp_die(__('Oops! The course does not exist.', 'learningonline'));
        }
        // ensure that user can create course
        if (!current_user_can('edit_posts')) {
            wp_die(__('Sorry! You have not permission to duplicate this course.', 'learningonline'));
        }
        ...
        if (!$new_post_id) {
            LP_Admin_Notice::add_redirect(__('<p>Sorry! Duplicate course failed!</p>', 'learningonline'));
        ...
        global $wpdb;
        //learning_online_copy_post_meta( $post_id, $new_post_id );

        $query = $wpdb->prepare("
			SELECT *
			FROM {$wpdb->prefix}learningonline_sections s
        ...
        if ($sections = $wpdb->get_results($query)) {
            foreach ($sections as $section) {
                $new_section_id = $wpdb->insert(
                    $wpdb->prefix . 'learningonline_sections',
                    array(
                ...
                if ($new_section_id) {
                    $query = $wpdb->prepare("
						SELECT i.*
						FROM {$wpdb->posts} i
						INNER JOIN {$wpdb->prefix}learningonline_sections s ON i.item_id = i.ID
        ...
        LP_Admin_Notice::add_redirect(__('<p>Course duplicated.</p>', 'learningonline'));
    ...
    if ($action === 'lp-duplicate-question') {
        $new_post_id = learning_online_duplicate_question($post_id);
        $post_type = LP_QUESTION_CPT;
        $error = __('Sorry! Duplicate question failed!', 'learningonline');
        $success = __('Question duplicated.', 'learningonline');
    } else if ($action === 'lp-duplicate-lesson') {
        $new_post_id = learning_online_duplicate_lesson($post_id);
        $post_type = LP_LESSON_CPT;
        $error = __('Sorry! Duplicate lesson failed!', 'learningonline');
        $success = __('Lesson duplicated.', 'learningonline');
    } else if ($action === 'lp-duplicate-quiz') {
        $new_post_id = learning_online_duplicate_quiz($post_id);
        $post_type = LP_QUIZ_CPT;
        $error = __('Sorry! Duplicate quiz failed!', 'learningonline');
        $success = __('Quiz duplicated.', 'learningonline');
    }
...
add_action('load-edit.php', 'learning_online_process_duplicate_action');
 ...
 * @return array
 */
function learning_online_install_add_on($plugin_name)
{
    ...
    $package = 'http://jwstheme.com/lprepo/' . $plugin_name . '.zip';
    ...
    if (is_wp_error($package)) {
        ...
        if (is_wp_error($working_dir)) {
            ...
            if (is_wp_error($result)) {
                ...
                $response = $result;
                $response['text'] = __('Installed', 'learningonline');
            }
        }
    }
...
function learning_online_accept_become_a_teacher()
{
    ...
    if (!learning_online_user_maybe_is_a_teacher($user_id)) {
        $be_teacher = new WP_User($user_id);
        $be_teacher->set_role(LP_TEACHER_ROLE);
        delete_transient('learning_online_become_teacher_sent_' . $user_id);
        do_action('learning_online_user_become_a_teacher', $user_id);
...
add_action('plugins_loaded', 'learning_online_accept_become_a_teacher');

function learning_online_user_become_a_teacher_notice()
{
    if ($user_id = learning_online_get_request('user_id') && learning_online_get_request('become-a-teacher-accepted') == 'yes') {
        $user = new WP_User($user_id);
        ?>
        <div class="updated">
            <p><?php printf(__('The user %s has become a teacher', 'learningonline'), $user->user_login); ?></p>
        </div>
        <?php
    }
}

add_action('admin_notices', 'learning_online_user_become_a_teacher_notice');
 ...
 * @return bool
 */
function learning_online_is_plugin_install($plugin)
{
 ...
 * @return mixed
 */
function learning_online_plugin_basename_from_slug($slug)
{
...
function learning_online_one_click_install_sample_data_notice()
{
    $courses = get_posts(
        array(
    ...
    if ((0 == sizeof($courses)) && ('off' != get_transient('learning_online_install_sample_data'))) {
        printf(
            '<div class="updated" id="learning-online-install-sample-data-notice">
            ...
            __('You haven\'t got any courses yet! Would you like to import sample data?', 'learningonline'),
            __('If yes, it requires to install addon name', 'learningonline'),
            __('LearningOnline Import/Export', 'learningonline'),
            __('but don\'t worry because it is completely automated.', 'learningonline'),
            __('Import now', 'learningonline'),
            __('No, thanks!', 'learningonline')
        );
    }
}

//add_action( 'admin_notices', 'learning_online_one_click_install_sample_data_notice' );

function learning_online_request_query($vars = array())
{
    global $typenow, $wp_query, $wp_post_statuses;
    if (LP_ORDER_CPT === $typenow) {
        // Status
        if (!isset($vars['post_status'])) {
            $post_statuses = learning_online_get_order_statuses();
...
add_filter('request', 'learning_online_request_query', 0);

function _learning_online_reset_course_data()
{
    if (empty($_REQUEST['reset-course-data'])) {
        return false;
    }
    learning_online_reset_course_data(intval($_REQUEST['reset-course-data']));
    wp_redirect(remove_query_arg('reset-course-data'));
}

add_action('init', '_learning_online_reset_course_data');

/***********************/
function learning_online_admin_section_loop_item_class($item, $section)
{
    $classes = array(
        'lp-section-item'
    );
    ...
    $classes = apply_filters('learning_online_section_loop_item_class', $classes, $item, $section);
    if ($classes) {
...
function learning_online_disable_checked_ontop($args)
{
...
add_filter('wp_terms_checklist_args', 'learning_online_disable_checked_ontop');

function learning_online_output_admin_template()
{
    learning_online_admin_view('admin-template.php');
}

add_action('admin_print_scripts', 'learning_online_output_admin_template');

function learning_online_output_screen_id()
{
...
//add_action( 'admin_head', 'learning_online_output_screen_id' );

function learning_online_get_screens()
{
    $screen_id = sanitize_title('LearningOnline');
    $screens = array(
        'toplevel_page_' . $screen_id,
        $screen_id . '_page_learning-online-statistics',
        $screen_id . '_page_learning-online-addons',
        $screen_id . '_page_learning-online-settings',
        $screen_id . '_page_learning-online-tools'
    );
    ...
    return apply_filters('learning_online_screen_ids', $screens);
}

function learning_online_get_admin_pages()
{
    return apply_filters(
        'learning_online_admin_pages',
        array(
...
function learning_online_is_post_type_screen($post_type, $union = 'OR')
{
    if (is_array($post_type)) {
        $return = null;
        foreach ($post_type as $_post_type) {
            $check = learning_online_is_post_type_screen($_post_type);
            if ($union == 'OR' && $check) {
                return true;
            }
            if ($return == null) {
...
function learning_online_get_notice_dismiss($context, $type = 'transient')
{
    if ($type == 'transient') {
        return get_transient('learning_online_dismiss_notice_' . $context);
    }

    return get_option('learning_online_dismiss_notice_' . $context);
}

if (!function_exists('learning_online_course_insert_section')) {

    function learning_online_course_insert_section($section = array())
    {
        global $wpdb;
...
if (!function_exists('learning_online_course_insert_section_item')) {

    function learning_online_course_insert_section_item($item_data = array())
    {
        global $wpdb;
        $wpdb->insert(
            $wpdb->learningonline_section_items,
            array(
...
if (!function_exists('learning_online_duplicate_post')) {

    function learning_online_duplicate_post($post_id = null, $args = array(), $meta = true)
    {
        ...
        if (!is_wp_error($new_post_id) && $meta) {
            learning_online_duplicate_post_meta($post_id, $new_post_id);
            // assign related tags/categories to new course
            $taxonomies = get_object_taxonomies($post->post_type);
            foreach ($taxonomies as $taxonomy) {
                $post_terms = wp_get_object_terms($post_id, $taxonomy, array('fields' => 'slugs'));
        ...
        return apply_filters('learning_online_duplicate_post', $new_post_id, $post_id);
    }
}

if (!function_exists('learning_online_duplicate_post_meta')) {
    /**
     * duplicate all post meta just in two SQL queries
     */
    function learning_online_duplicate_post_meta($old_post_id, $new_post_id, $excerpt = array())
    {
        global $wpdb;
        ...
        if (count($post_meta_infos) != 0) {
            ...
            $excerpt = apply_filters('learning_online_excerpt_duplicate_post_meta', $excerpt, $old_post_id, $new_post_id);
            $sql_query = "INSERT INTO $wpdb->postmeta (post_id, meta_key, meta_value) ";
            $sql_query_sel = array();
...
add_filter('learning_online_question_types', 'learning_online_sort_questions', 99);
if (!function_exists('learning_online_sort_questions')) {
    function learning_online_sort_questions($types)
    {
        $user_id = get_current_user_id();
        $question_types = get_user_meta($user_id, '_learning_online_memorize_question_types', true);
...
if (!function_exists('learning_online_duplicate_course')) {

    function learning_online_duplicate_course($course_id = null, $force = true)
    {
        if (!function_exists('_learning_online_get_course_curriculum')) {
        ...
        if (!$course_id || !($post = get_post($course_id))) {
            return new WP_Error(__('<p>Op! The course does not exist</p>', 'learningonline'));
        } else {
            // ensure that user can create course
            if (!current_user_can('edit_posts')) {
                return new WP_Error(__('<p>Sorry! You have not permission to duplicate this course</p>', 'learningonline'));
            } else {
                // duplicate course
                $new_course_id = learning_online_duplicate_post($course_id);
                if (!$new_course_id || is_wp_error($new_course_id)) {
                    return new WP_Error(__('<p>Sorry! Duplicate course failed!</p>', 'learningonline'));
                } else {
                    $curriculums = _learning_online_get_course_curriculum($course_id);
                    foreach ($curriculums as $section_id => $section) {
                        $new_section_id = learning_online_course_insert_section(array(
                            'section_name' => $section->section_name,
                        ...
                        if ($section->items) {
                            foreach ($section->items as $item) {
                                ...
                                if ($force && $item->post_type === LP_QUIZ_CPT) {
                                    $item_id = learning_online_duplicate_quiz($item->ID, array('post_status' => 'publish'));
                                } else {
                                    $item_id = learning_online_duplicate_post($item->ID, array('post_status' => 'publish'));
                                }
                                if ($force) {
                                    $section_item_id = learning_online_course_insert_section_item(array(
...
if (!function_exists('learning_online_duplicate_question')) {

    function learning_online_duplicate_question($question_id = null, $quiz_id = null)
    {
        if (!$question_id || !get_post($question_id)) {
            return new WP_Error(sprintf(__('Question id %s is not exists.', 'learningonline'), $question_id));
        }
        if ($quiz_id && !get_post($quiz_id)) {
            return new WP_Error(sprintf(__('Quiz id %s is not exists.', 'learningonline'), $quiz_id));
        }

        global $wpdb;
        $new_question_id = learning_online_duplicate_post($question_id);
        if ($quiz_id) {
            // learningonline_quiz_questions
            $sql = $wpdb->prepare("
                        SELECT * FROM $wpdb->learningonline_quiz_questions WHERE quiz_id = %d AND question_id = %d
                    ", $quiz_id, $question_id);
            $quiz_question_data = $wpdb->get_row($sql);
            $max_order = $wpdb->get_var($wpdb->prepare("SELECT max(question_order) FROM {$wpdb->prefix}learningonline_quiz_questions WHERE quiz_id = %d", $quiz_id));

            if ($quiz_question_data) {
                $wpdb->insert(
                    $wpdb->learningonline_quiz_questions,
                    array(
                        'quiz_id' => $quiz_id,
        ...
        $sql = $wpdb->prepare("
                    SELECT * FROM $wpdb->learningonline_question_answers WHERE question_id = %d
                ", $question_id);
        $question_answers = $wpdb->get_results($sql);
        if ($question_answers) {
            foreach ($question_answers as $q_a) {
                $wpdb->insert(
                    $wpdb->learningonline_question_answers,
                    array(
                        'question_id' => $new_question_id,
...
if (!function_exists('learning_online_duplicate_quiz')) {

    function learning_online_duplicate_quiz($quiz_id = null, $args = array())
    {
        global $wpdb;
        $new_quiz_id = learning_online_duplicate_post($quiz_id, $args, true);
 ...
 * @return string
 */
function learning_online_get_file_version($file)
{
    if (!file_exists($file)) {
        return '';
    }
 ...
 * @return array|bool
 */
function learning_online_get_theme_templates($check = false)
{
    $template_folder = learning_online_template_path();
    ...
    $scanned_files = learning_online_scan_template_files($template_path);
    foreach ($scanned_files as $file) {
        ...
        if (!empty($theme_file)) {
            $core_version = learning_online_get_file_version($template_path . $file);
            $theme_version = learning_online_get_file_version($theme_file);
...
function learning_online_detect_outdated_template()
{
    $template_folder = learning_online_template_path();
    $template_path = LP_PLUGIN_PATH . '/templates/';
    $template_dir = get_template_directory();
...
function learning_online_scan_template_files($template_path)
{
        ...
        foreach ($files as $key => $value) {
            if (!in_array($value, array(".", "..", 'index.php', 'index.html'))) {
                if (is_dir($template_path . '/' . $value)) {
                    $sub_files = learning_online_scan_template_files($template_path . '/' . $value);
...
if (!function_exists('learning_online_duplicate_lesson')) {

    function learning_online_duplicate_lesson($lesson_id = null)
    {
        return learning_online_duplicate_post($lesson_id);
    }
 ...
 * @return array
 */
function learning_online_get_chart_general($from = null, $by = null, $time_ago)
{
    global $wpdb;
...
function learning_online_get_default_section($section = null)
{
    if (!$section) {
...
function lp_placeholder_img_src() {

	return apply_filters( 'lp_placeholder_img_src', LP()->plugin_url() . '/assets/images/placeholder.jpg' );
