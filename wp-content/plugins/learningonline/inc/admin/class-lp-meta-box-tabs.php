	...
	class LP_Meta_Box_Tabs {
		...
		public function __construct( $args = array() ) {
			$defaults   = array(
				...
				'title'     => __( 'Meta box tabs', 'learningonline' ),
		...
		public function add_tab_arg( $url ) {
			if ( array_key_exists( 'learning-online-meta-box-tab', $_REQUEST ) && !empty( $_REQUEST['learning-online-meta-box-tab'] ) ) {
				$url = remove_query_arg('tab', $url	);
				$url = add_query_arg( 'tab', $_REQUEST['learning-online-meta-box-tab'], $url );
			}
			return $url;
		}
		...
		public function display() {
			...
			include learning_online_get_admin_view( 'meta-boxes/tabs' );
		}
		...
		public function get_tabs() {
			...
			if ( is_array( $post_types ) ) {
				foreach ( $post_types as $post_type ) {
					$tabs = apply_filters( "learning_online_{$post_type}_tabs", $tabs );
				}
			}
			return $tabs;
		}
