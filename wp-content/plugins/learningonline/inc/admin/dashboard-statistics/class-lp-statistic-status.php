...
if ( !class_exists( 'LP_Statistic_Status' ) ) :
	...
	class LP_Statistic_Status {
		...
		public static function render() {
			$order_statuses    = learning_online_get_order_statuses(true, true);
			...
			$counts = learning_online_count_orders( array( 'status' => $specific_statuses ) );
			?>
			<ul class="learningonline-statistic-status">
				<li class="full-width">
					<a href="#" class="total-raised">
                            <span>
                                <?php echo _learning_online_total_raised() ?>
								<?php _e( 'Total Raised', 'learningonline' ); ?>
                            </span>
					</a>
				</li>
					...
					<li>
						<?php if ( $count ): ?>
							<a href="<?php echo esc_url( admin_url( 'edit.php?post_status=' . LP_ORDER_CPT . '&post_type=' . $status ) ); ?>" class="<?php echo esc_attr( $status ) ?>">
								<span><?php printf( translate_nooped_plural( _n_noop( '%d order', '%d orders' ), $count, 'learningonline' ), $count ) ?></span>
				...
				<li class="full-width featured-theme">
					...
					<p>
						<?php _e( 'Created by: ', 'learningonline' ) ?>
						<a href="https://jwstheme.com/" class="author"><?php echo esc_html( $eduma_data['item']['user'] ); ?></a>
					</p>
				</li>
			</ul>
			<?php
		...
		public static function get_eduma_info( $item_id ) {
			...
			$transient_id = 'learningonline_envato_item_data';
