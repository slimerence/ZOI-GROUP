	...
	class LP_Admin_Ajax {
		...
		public static function init() {
			...
			foreach ( $ajaxEvents as $ajaxEvent => $nopriv ) {
				add_action( 'wp_ajax_learningonline_' . $ajaxEvent, array( __CLASS__, $ajaxEvent ) );
				// enable for non-logged in users
				if ( $nopriv ) {
					add_action( 'wp_ajax_nopriv_learningonline_' . $ajaxEvent, array( __CLASS__, $ajaxEvent ) );
				}
			}
			add_filter( 'learning_online_modal_search_items_exclude', array(
				__CLASS__,
				'_modal_search_items_exclude'
			), 10, 4 );
			add_filter( 'learning_online_modal_search_items_not_found', array(
				__CLASS__,
				'_modal_search_items_not_found'
			), 10, 2 );
			do_action( 'learning_online_admin_ajax_load', __CLASS__ );
		}
		...
		public static function remove_course_items() {
			...
			if ( $id ) {
				...
				echo $wpdb->prepare( "
						DELETE FROM {$wpdb->prefix}learningonline_section_items
		...
		public static function search_users() {
			...
			$customers_query = new WP_User_Query( apply_filters( 'learning_online_search_customers_query', array(
				'fields'         => 'all',
		...
		public static function dismiss_notice() {
			$context   = learning_online_get_request( 'context' );
			$transient = learning_online_get_request( 'transient' );

			if ( $context ) {
				if ( $transient >= 0 ) {
					set_transient( 'learning_online_dismiss_notice_' . $context, 'off', $transient ? $transient : DAY_IN_SECONDS * 7 );
				} else {
					update_option( 'learning_online_dismiss_notice_' . $context, 'off' );
				}
			}
			die();
		}
		...
		public static function _modal_search_items_not_found( $message, $type ) {
			switch ( $type ) {
				case 'lp_lesson':
					$message = __( 'There are no available lessons for this course, please use ', 'learningonline' );
					$message .= '<a target="_blank" href="' . admin_url( 'post-new.php?post_type=lp_lesson' ) . '">' . esc_html__( 'Adding New Item.', 'learningonline' ) . '</a>';
					break;
				case 'lp_quiz':
					$message = __( 'There are no available quizzes for this course, please use ', 'learningonline' );
					$message .= '<a target="_blank" href="' . admin_url( 'post-new.php?post_type=lp_quiz' ) . '">' . esc_html__( 'Adding New Item.', 'learningonline' ) . '</a>';
					break;
				case 'lp_question':
					$message = __( 'There are no available questions for this quiz, please use ', 'learningonline' );
					$message .= '<a target="_blank" href="' . admin_url( 'post-new.php?post_type=lp_question' ) . '">' . esc_html__( 'Adding New Item.', 'learningonline' ) . '</a>';
					break;
			}
		...
		public static function _modal_search_items_exclude( $exclude, $type, $context = '', $context_id = null ) {
			...
			switch ( $type ) {
				...
				case 'lp_quiz':
					$query      = $wpdb->prepare( "
						SELECT item_id
						FROM {$wpdb->prefix}learningonline_section_items si
						INNER JOIN {$wpdb->prefix}learningonline_sections s ON s.section_id = si.section_id
		...
		public static function add_item_to_section() {
			global $wpdb;
			$section = learning_online_get_request( 'section' );
			if ( !$section ) {
				wp_die( __( 'Error', 'learningonline' ) );
			}
			$items = (array) learning_online_get_request( 'item' );
			if ( !$items ) {
				$max_order = $wpdb->get_var( $wpdb->prepare( "SELECT max() FROM {$wpdb}learningonline_section_items WHERE section_id = %d", $section ) );
		...
		public static function modal_search_items() {
			...
			$term       = (string) ( stripslashes( learning_online_get_request( 'term' ) ) );
			$type       = (string) ( stripslashes( learning_online_get_request( 'type' ) ) );
			$context    = (string) ( stripslashes( learning_online_get_request( 'context' ) ) );
			$context_id = (string) ( stripslashes( learning_online_get_request( 'context_id' ) ) );
			$current_items_in_order =  learning_online_get_request( 'current_items' );
			$current_items = array();


			foreach ($current_items_in_order as $item) {
			    $sql = "SELECT meta_value
                        FROM {$wpdb->prefix}learningonline_order_itemmeta 
                        WHERE meta_key = '_course_id' 
                        AND learningonline_order_item_id = $item";
			...
			$exclude = array_unique( (array) apply_filters( 'learning_online_modal_search_items_exclude', $exclude, $type, $context, $context_id ) );
			...
			$args = apply_filters('learning_online_filter_admin_ajax_modal_search_items_args', $args, $context, $context_id  );
			...
			if ( !empty( $posts ) ) {
				if ($current_items_in_order) {
                    foreach ( $posts as $post ) {
                        ...
                        $found_items[$post->ID]->post_title = !empty( $post->post_title ) ? $post->post_title : sprintf( '(%s)', __( 'Untitled', 'learningonline' ) );
                    }
                } else {
                    foreach ( $posts as $post ) {
                        $found_items[$post->ID]             = $post;
			...
			if ( $found_items ) {
				...
				echo '<li>' . apply_filters( 'learning_online_modal_search_items_not_found', __( 'No item found', 'learningonline' ), $type ) . '</li>';
			}
			...
			$response       = array(
				'html'    => ob_get_clean(),
				'data'    => $found_items,
				'notices' => '<div class="learningonline-search-notices notice notice-warning" data-post-type="' . esc_attr( $item_object->name ) . '" data-user="' . esc_attr( $user->id ) . '">' . sprintf( '<p>' . __( 'A ', 'learningonline' ) . '<span style="text-transform: lowercase;">%s</span>' . __( ' is just used for only one ', 'learningonline' ) . '<span style="text-transform: lowercase;">%s</span></p>', $item_object->labels->singular_name, $post_type->labels->singular_name ) . '<a class="learningonline-dismiss-notice"></a></div>'
			);
			$dismiss_notice = 'learningonline_notice_' . $item_object->name . '_' . $user->id;
		...
		public static function remove_quiz_question() {
			...
			if ( !wp_verify_nonce( learning_online_get_request( 'remove-nonce' ), 'remove_quiz_question' ) ) {
				wp_die( __( 'Error', 'learningonline' ) );
			}
			$query = $wpdb->prepare( "
			...
			if ( $type ) {
				...
				update_user_meta( $user_id, '_learning_online_memorize_question_types', $question_types );
			}
		...
		public static function search_questions() {
			...
			if ( !$user->is_admin() && get_post_field( 'post_author', $quiz_id ) != get_current_user_id() ) {
				wp_die( __( 'You have no permission to access this section.', 'learningonline' ) );
			}
			$term    = (string) ( stripslashes( learning_online_get_request( 'term' ) ) );
			$exclude = array();
			...
			if ( !empty( $posts ) ) {
				foreach ( $posts as $post ) {
					$found_questions[$post->ID] = !empty( $post->post_title ) ? $post->post_title : sprintf( '(%s)', __( 'Untitled', 'learningonline' ) );
				}
			}
			...
			if ( $found_questions ) {
				...
				echo '<li>' . __( 'No questions found', 'learningonline' ) . '</li>';
			}
		...
		public static function plugin_action() {
			$url = learning_online_get_request( 'url' );
			ob_start();
			wp_remote_get( $url );
			ob_get_clean();
			echo wp_remote_get( admin_url( 'admin.php?page=learning-online-addons&tab=installed' ) );
			die();
		}
		...
		public static function remove_order_item() {
			...
			if ( !current_user_can( 'edit_lp_orders' ) ) {
				die( __( 'Permission denied', 'learningonline' ) );
			}

			// verify nonce
			$nonce = learning_online_get_request( 'remove_nonce' );
			if ( !wp_verify_nonce( $nonce, 'remove_order_item' ) ) {
				die( __( 'Check nonce failed', 'learningonline' ) );
			}

			// validate order
			$order_id = learning_online_get_request( 'order_id' );
			if ( !is_numeric( $order_id ) || get_post_type( $order_id ) != 'lp_order' ) {
				die( __( 'Order invalid', 'learningonline' ) );
			}

			// validate item
			$item_id = learning_online_get_request( 'item_id' );
			$post    = get_post( learning_online_get_order_item_meta( $item_id, '_course_id' ) );
			if ( !$post || ( 'lp_course' !== $post->post_type ) ) {
				die( __( 'Course invalid', 'learningonline' ) );
			}

			learning_online_remove_order_item( $item_id );

			$order_data                  = learning_online_update_order_items( $order_id );
			$currency_symbol             = learning_online_get_currency_symbol( $order_data['currency'] );
			$order_data['subtotal_html'] = learning_online_format_price( $order_data['subtotal'], $currency_symbol );
			$order_data['total_html']    = learning_online_format_price( $order_data['total'], $currency_symbol );
...
//			}
			...
			// validate item
			$item_ids   = learning_online_get_request( 'item_id' );
			$item_html  = '';
			$order_data = array();
...
//					die( __( 'Course invalid', 'learningonline' ) );
				...
				$course = learning_online_get_course( $post->ID );
				$item   = array(
					'course_id' => $course->id,
				...
				// Add item
				$item_id = learning_online_add_order_item( $order_id, array(
				...
				// Add item meta
				if ( $item_id ) {
					$item = apply_filters( 'learning_online_ajax_order_item', $item );

					learning_online_add_order_item_meta( $item_id, '_course_id', $item['course_id'] );
					learning_online_add_order_item_meta( $item_id, '_quantity', $item['quantity'] );
					learning_online_add_order_item_meta( $item_id, '_subtotal', $item['subtotal'] );
					learning_online_add_order_item_meta( $item_id, '_total', $item['total'] );

					do_action( 'learning_online_ajax_add_order_item_meta', $item );
				}
				...
				ob_start();
				include learning_online_get_admin_view( 'meta-boxes/order/order-item.php' );
		...
		public static function search_courses() {
			...
			if ( !wp_verify_nonce( $nonce, 'search_item_term' ) ) {
				LP_Debug::exception( __( 'Verify nonce failed', 'learningonline' ) );
			}

			$term    = learning_online_get_request( 'term' );
			$exclude = learning_online_get_request( 'exclude' );

			$posts         = learning_online_get_all_courses(
				array(
					'term'    => $term,
					'exclude' => $exclude
				)
			...
			$found_courses = apply_filters( 'learning_online_json_search_found_courses', $found_courses );

			learning_online_send_json( $found_courses );
		}
		...
		public static function toggle_lesson_preview() {
			$id = learning_online_get_request( 'lesson_id' );
			if ( get_post_type( $id ) == 'lp_lesson' && wp_verify_nonce( learning_online_get_request( 'nonce' ), 'learning-online-toggle-lesson-preview' ) ) {
				$previewable = learning_online_get_request( 'previewable' );
		...
		public static function add_new_item() {
			$post_type  = learning_online_get_request( 'type' );
			$post_title = learning_online_get_request( 'name' );
			$response   = array();
		...
		public static function update_editor_hidden() {
			if ( $id = learning_online_get_request( 'course_id' ) ) {
				if ( learning_online_get_request( 'is_hidden' ) ) {
					update_post_meta( $id, '_lp_editor_hidden', 'yes' );
				} else {
					delete_post_meta( $id, '_lp_editor_hidden' );
				}
			}
			learning_online_send_json( $_POST );
		}

		public static function update_quiz_question_state() {
			$hidden = learning_online_get_request( 'hidden' );
			$post   = learning_online_get_request( 'quiz_id' );
		...
		public static function update_curriculum_section_state() {
			$hidden = learning_online_get_request( 'hidden' );
			$post   = learning_online_get_request( 'course_id' );
		...
		public static function create_page() {
			...
			if ( $page_name ) {
				...
				if ( $page_id ) {
					...
					$html             = learning_online_pages_dropdown( '', '', array( 'echo' => false ) );
					preg_match_all( '!value=\"([0-9]+)\"!', $html, $matches );
					$response['positions'] = $matches[1];
					$response['html']      = '<a href="' . get_edit_post_link( $page_id ) . '" target="_blank">' . __( 'Edit Page', 'learningonline' ) . '</a>&nbsp;';
					$response['html'] .= '<a href="' . get_permalink( $page_id ) . '" target="_blank">' . __( 'View Page', 'learningonline' ) . '</a>';
				} else {
					$response['error'] = __( 'Error! Create page failed. Please try again!', 'learningonline' );
				}
			} else {
				$response['error'] = __( 'Empty page name!', 'learningonline' );
			}
			learning_online_send_json( $response );
			die();
		}
		...
		public static function add_quiz_question() {
			...
			if ( !$id ) {
				...
				$args_item = apply_filters( 'learningonline_quiz_insert_item_args', $args_item, $quiz_id );
			...
			if ( $id && $quiz_id ) {
				...
				$wpdb->insert(
					$wpdb->prefix . 'learningonline_quiz_questions',
					array(
						'quiz_id'        => $quiz_id,
						'question_id'    => $id,
				...
				learning_online_admin_view( 'meta-boxes/quiz/question.php', array( 'question' => $question ) );
		...
		public static function convert_question_type() {
			if ( ( $from = learning_online_get_request( 'from' ) ) && ( $to = learning_online_get_request( 'to' ) ) && $question_id = learning_online_get_request( 'question_id' ) ) {
				$data = array();
				parse_str( $_POST['data'], $data );

				do_action( 'learning_online_convert_question_type', $question_id, $from, $to, $data );
				...
				//throw new Exception( __( 'Convert question type must be specify the id, source and destination type', 'learningonline' ) );
				throw new Exception( __( 'Something went wrong! Please try again or ask <a href="https://wordpress.org/support/">support forums</a>.', 'learningonline' ) );
			}
			die();
		}
		...
		public static function install_sample_data() {
			...
			$retry_button   = sprintf( '<a href="" class="button yes" data-action="yes">%s</a>', __( 'Try again!', 'learningonline' ) );
			$dismiss_button = sprintf( '<a href="" class="button disabled no" data-action="no">%s</a>', __( 'Cancel', 'learningonline' ) );
			$buttons        = sprintf( '<p>%s %s</p>', $retry_button, $dismiss_button );
			if ( 'no' == $yes ) {
				set_transient( 'learning_online_install_sample_data', 'off', 12 * HOUR_IN_SECONDS );
			} else {
				$result = array( 'status' => 'activate' );//learning_online_install_and_active_add_on( 'learningonline-import-export' );
				if ( 'activate' == $result['status'] ) {
					...
					if ( file_exists( lpie_import_path() ) ) {
						...
						if ( file_exists( $copy ) ) {
							$url                 = admin_url( 'admin-ajax.php?page=learning_online_import_export&tab=import-course' );
							$postdata            = array(
								'step'        => 2,
								'action'      => 'learning_online_import',
							...
							$response['message'] = sprintf( '<p>%s <a href="edit.php?post_type=lp_course">%s</a> </p>', __( 'Import sample data successes.', 'learningonline' ), __( 'View courses', 'learningonline' ) );
						}
					}
					if ( $response['result'] == 'fail' ) {
						$response['message'] = sprintf( '<p>%s</p>%s', __( 'Import sample data failed. Please try again!.', 'learningonline' ), $buttons );
					}
				} else {
					$response['result']  = 'fail';
					$response['message'] = sprintf( '<p>%s</p>', __( 'Unknown error when installing/activating Import/Export addon. Please try again!', 'learningonline' ) ) . $buttons;
				}
			}
			learning_online_send_json( $response );
			die();
		...
		public static function bundle_activate_add_ons() {
			global $learning_online_add_ons;
			include_once( ABSPATH . 'wp-admin/includes/plugin-install.php' ); //for plugins_api..
			$response = array( 'addons' => array() );

			if ( !current_user_can( 'activate_plugins' ) ) {
				$response['error'] = __( 'You do not have sufficient permissions to deactivate plugins for this site.', 'learningonline' );
			} else {

				$add_ons = $learning_online_add_ons['bundle_activate'];

				if ( $add_ons ) {
					foreach ( $add_ons as $slug ) {
						$response['addons'][$slug] = learning_online_install_and_active_add_on( $slug );
					}
				}
			}
			learning_online_send_json( $response );
		...
		public static function bundle_activate_add_on() {
			...
			if ( !current_user_can( 'activate_plugins' ) ) {
				...
				$response[$slug] = learning_online_install_and_active_add_on( $slug );
			}
			learning_online_send_json( $response );
		}
		...
		public static function plugin_install() {
			...
			$response    = learning_online_install_add_on( $plugin_name );
			learning_online_send_json( $response );
			die();
		}
		...
		public static function dummy_image() {
			...
			learning_online_text_image( $text, $_GET );
			die();
		}
		...
		public static function get_page_permalink() {
			...
			?>
			<a href="<?php echo get_edit_post_link( $page_id ); ?>" target="_blank"><?php _e( 'Edit Page', 'learningonline' ); ?></a>
			<a href="<?php echo get_permalink( $page_id ); ?>" target="_blank"><?php _e( 'View Page', 'learningonline' ); ?></a>
			<?php
			die();
		}
		...
		public function custom_stats() {
			...
			learning_online_process_chart( learning_online_get_chart_students( $to, 'days', floor( $date_diff / ( 60 * 60 * 24 ) ) + 1 ) );
			die();
		}
		...
		public static function duplicate_course() {
			...
			$new_course_id = learning_online_duplicate_course( $course_id, $force );
			if ( is_wp_error( $course_id ) ) {
				LP_Admin_Notice::add_redirect( $course_id->get_error_message(), 'error' );
			} else {
				LP_Admin_Notice::add_redirect( sprintf( '<strong>%s</strong> %s', get_the_title( $course_id ), __( ' course has duplicated', 'learningonline' ) ), 'updated' );
		...
		public static function duplicate_question() {
			...
			global $wpdb;
			$question_id = learning_online_get_request( 'question-id' );
			$quiz_id     = learning_online_get_request( 'quiz-id' );
			$user_id     = learning_online_get_current_user_id();

			$new_question_id = learning_online_duplicate_question( $question_id, $quiz_id );
			if ( !is_wp_error( $new_question_id ) ) {
				ob_start();
				$question = LP_Question_Factory::get_question( $new_question_id );
				$post     = get_post( $quiz_id );
				setup_postdata( $post );
				_learning_online_setup_question( $new_question_id );
				learning_online_admin_view( 'meta-boxes/quiz/question.php', array( 'question' => $question ) );
				$response['html'] = ob_get_clean();
		...
		public static function remove_notice_popup() {
			...
			if ( isset( $_POST['action'] ) && $_POST['action'] === 'learningonline_remove_notice_popup'
				&& isset( $_POST['slug'] ) && !empty( $_POST['slug'] )
				&& isset( $_POST['user'] ) && !empty( $_POST['user'] )
			) {

				$slug = 'learningonline_notice_' . $_POST['slug'] . '_' . $_POST['user'];
		...
		public static function update_order_status () {
            global $wpdb;
            $order_id = learning_online_get_request( 'order_id' );
            $value     = learning_online_get_request( 'value' );
