...
class LP_Settings_Base {
	...
	public function __construct() {
		if ( strtolower( current_filter() ) == 'activate_learningonline/learningonline.php' ) {
			return;
		}
		...
		if ( $sections = $this->get_sections() )
			foreach ( $sections as $id => $text ) {
				$callback = apply_filters( 'learning_online_section_callback_' . $this->id . '_' . $id, array( $this, 'output_section_' . $id ) );
				if ( is_callable( $callback ) ) {
					add_action( 'learning_online_section_' . $this->id . '_' . $id, $callback );
				}
			}
		self::$current_tab = $current_tab;
		// hooks
		add_action( 'learning_online_sections_' . $this->id, array( $this, 'output_sections' ) );
		add_action( 'learning_online_settings_' . $this->id, array( $this, 'output' ) );
		add_action( 'learning_online_settings_save_' . $this->id, array( $this, 'save' ) );
	...
	public function output_sections() {
		...
		if ( $sections && sizeof( $sections ) > 1 ) {
			...
			foreach ( $sections as $name => $section ) {
				?>
				<li>
					<a href="<?php echo '?page=learning-online-settings&tab=' . $this->id . '&section=' . sanitize_title( $name ); ?>" class="<?php echo $current_section == $name ? 'current' : ''; ?>">
	...
	public function output() {
		do_action( 'learning_online_section_' . $this->id . '_' . $this->section['id'] );
	}
	...
	public function save() {
		foreach ( $_POST as $k => $v ) {
			if ( ( strpos( $k, 'learning_online_' ) === false ) || ( !apply_filters( 'learning_online_abort_update_option', true, $k ) ) )
				continue;
			update_option( $k, apply_filters( 'learning_online_update_option_value', ( $v ), $k ) );
		}
	}
	...
	public function get_field_name( $name ) {
		$field_name = apply_filters( 'learning_online_settings_field_name_' . $name, "learning_online_{$name}" );
		return $field_name;
	}
	...
	public function output_field( $options ) {
		...
		require learning_online_get_admin_view( 'settings/fields/' . $file . '.php' );
	}
	...
	public function get_option( $option_name, $default = null ) {
		if ( strstr( $option_name, '[' ) ) {
			...
			$option_value = LP()->settings->get( preg_replace( '!^learning_online_!', '', $option_name ), null );
		}
