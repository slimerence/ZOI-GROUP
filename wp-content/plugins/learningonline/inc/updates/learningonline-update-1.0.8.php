...
if ( LEARNING_ONLINE_UPDATE_DATABASE ) {
	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) === $table ) {
		...
		$query = $wpdb->prepare( "
                    ALTER TABLE {$wpdb->prefix}learningonline_user_lessons ADD COLUMN `start_time` DATETIME NOT NULL DEFAULT %s,
		...
		$query = $wpdb->prepare( "
                            UPDATE {$wpdb->prefix}learningonline_user_lessons
                            SET
		...
		$query = $wpdb->query( "
                    ALTER TABLE {$wpdb->prefix}learningonline_user_lessons DROP COLUMN `start_date`, DROP COLUMN `end_date`;
            " );
	}
	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) === $table ) {
		// delete columns start_date, end_date
		$query = $wpdb->query( "
			ALTER TABLE {$wpdb->prefix}learningonline_user_course_items DROP COLUMN `start_date`, DROP COLUMN `end_date`;
		" );
	}
	learning_online_update_log( '1.0.8', array( 'time' => time() ) );
}