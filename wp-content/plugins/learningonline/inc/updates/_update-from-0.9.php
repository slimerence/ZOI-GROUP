...
class LP_Upgrade_From_09 {
	...
	public function __construct() {
		...
		$this->learning_online_upgrade_10_page();
	}
	...
	/**
	 * Check if user trying to access LearningOnline admin menu
	 *
	 * @return bool
	 */
	private function check_admin_menu() {
		$admin_page = !empty( $_REQUEST['page'] ) ? $_REQUEST['page'] : '';
		return preg_match( '!^learning_online_!', $admin_page );
	}
				...
				if ( $result ) {
					...
					foreach ( $lesson_quiz as $obj ) {
						...
						if ( $new_obj_id = wp_insert_post( $obj ) ) {
							$wpdb->insert(
								$wpdb->prefix . 'learningonline_section_items',
								array(
	...
	private function _upgrade_unassigned_items( $force = false ) {
		...
		", '_learning_online_upgraded', 'lpr_quiz', 'lpr_lesson' );
	...
	private function _upgrade_unassigned_questions( $force = false ) {
		...
		$query   = $wpdb->prepare( "
			...
			" . ( $exclude ? "AND ID NOT IN(" . join( ',', $exclude ) . ")" : "" ) . "
			" . ( !$force ? "HAVING upgraded IS NULL" : "" ) . "
		", '_learning_online_upgraded', 'lpr_question' );
			...
			if ( !empty( $meta['answer'] ) ) {
				if ( in_array( $meta['type'], array( 'true_or_false', 'single_choice', 'multi_choice', 'sorting_choice' ) ) ) {
					...
					foreach ( $meta['answer'] as $order => $answer ) {
						...
						$wpdb->insert(
							$wpdb->prefix . 'learningonline_question_answers',
							array(
								'question_id'  => $new_id,
...
//		$this->_remove_unused_data();
		...
		if ( $ids = $wpdb->get_col( $query ) ) {
			...
			$wpdb->query( "DELETE FROM {$wpdb->posts} WHERE ID IN(" . join( ",", $ids ) . ")" );
                        ...
                        if ( $wpdb->get_var("SHOW TABLES LIKE '{$table}'") === $table ) {
                            $wpdb->query( "DELETE FROM {$wpdb->learningonline_sections}" );
                        }
                        $table = $wpdb->prefix . 'learningonline_section_items';
                        if ( $wpdb->get_var("SHOW TABLES LIKE '{$table}'") === $table ) {
                            $wpdb->query( "DELETE FROM {$wpdb->learningonline_section_items}" );
                        }
                        $table = $wpdb->prefix . 'learningonline_quiz_history';
                        if ( $wpdb->get_var("SHOW TABLES LIKE '{$table}'") === $table ) {
                            $wpdb->query( "DELETE FROM {$wpdb->learningonline_quiz_history}" );
                        }
                        $table = $wpdb->prefix . 'learningonline_user_course';
                        if ( $wpdb->get_var("SHOW TABLES LIKE '{$table}'") === $table ) {
                            $wpdb->query( "DELETE FROM {$wpdb->learningonline_user_course}" );
                        }
		}
	...
	private function _create_order_meta( $old_id, $new_id ) {
		global $wpdb;
		$keys       = array(
			'_learning_online_transaction_method'    => '_payment_method',
			'_learning_online_customer_id'           => '_user_id',
			'_learning_online_customer_ip'           => '_user_ip_address',
			'_learning_online_order_items'           => null,
			'_learning_online_transaction_method_id' => '_transaction_id'
		);
		$order_meta = self::get_post_meta( $old_id, array_keys( $keys ) );
		if ( $order_meta ) foreach ( $order_meta as $meta ) {
			if ( '_learning_online_order_items' == $meta['meta_key'] ) {
				...
				if ( isset( $order_data->products ) ) {
					foreach ( $order_data->products as $order_item ) {
						...
						if ( $new_course_id ) {
							...
							$r             = $wpdb->insert(
								$wpdb->prefix . 'learningonline_order_items',
								array(
							...
							learning_online_add_order_item_meta( $order_item_id, '_course_id', $new_course->ID );
							learning_online_add_order_item_meta( $order_item_id, '_quantity', $order_item['quantity'] );
							learning_online_add_order_item_meta( $order_item_id, '_subtotal', $order_item['product_subtotal'] );
							learning_online_add_order_item_meta( $order_item_id, '_total', $order_item['product_subtotal'] );
						}
					...
					if ( !empty( self::$courses_map[$course_id] ) && !empty( $user_meta->course_time[$course_id] ) ) {
						...
						if ( !empty( self::$course_order_map[$course_id] ) ) {
							...
							$course_order = 0;
                                                ...
                                                $user = learning_online_get_user( $user_meta->user_id );
                                                $user_item_id = learning_online_update_user_item_field( array(
...
//                                            var_dump($user_parent_items, $old_quiz_id, self::$courses_map); die();
                                        ...
                                        $user_quiz_id = learning_online_update_user_item_field( array(
						...
						if ( !empty( $user_meta->quiz_current_question[$old_quiz_id] ) ) {
							learning_online_update_user_item_meta( $user_quiz_id, 'current_question', self::$questions_map[$user_meta->quiz_current_question[$old_quiz_id]] );
						}
					}
						...
						if ( !empty( $user_meta->quiz_question_answer[$old_quiz_id] ) ) {
							...
							learning_online_update_user_item_meta( $user_quiz_id, 'question_answers', $question_answers );
						}
					}
							...
							foreach ( $user_meta->quiz_questions[$old_quiz_id] as $old_question_id ) {
								if ( !empty( self::$questions_map[$old_question_id] ) ) {
									$quiz_questions[] = self::$questions_map[$old_question_id];
                                                        ...
                                                        learning_online_update_user_item_meta( $user_quiz_id, 'question_answers', $quiz_questions );
						}
					}
				}
			}
	...
	private function _backward_compatible() {
		register_taxonomy( 'lesson-tag', array( 'lpr_lesson', LP_LESSON_CPT ),
			array(
				...
				'rewrite'           => array( 'slug' => _x( 'lesson-tag', 'Permalink Slug', 'learningonline' ), 'hierarchical' => true, 'with_front' => false ),
			)
		);
		...
		register_taxonomy( 'question-tag', array( 'lpr_question', LP_QUESTION_CPT ),
			array(
				...
				'show_in_nav_menus' => true,
				'rewrite'           => array(
					'slug'         => _x( 'question-tag', 'Permalink Slug', 'learningonline' ),
					'hierarchical' => false,
					'with_front'   => false
				),
			)
		...
		if ( $_lpr_settings_general = get_option( '_lpr_settings_general' ) ) {
			...
			foreach ( $options as $o ) {
				if ( array_key_exists( $o, $_lpr_settings_general ) ) {
					add_option( 'learning_online_' . $o, $_lpr_settings_general[$o] );
				}
			}
		}
			...
			foreach ( $payments as $payment => $options ) {
				if ( $payment == 'paypal' ) {
					if ( !empty( $options['enable'] ) && $options['enable'] == 'on' ) {
						add_option( 'learning_online_paypal_enable', 'yes' );
					} else {
						add_option( 'learning_online_paypal_enable', 'no' );
					}
					add_option( 'learning_online_paypal_email', !empty( $options['paypal_email'] ) ? $options['paypal_email'] : '' );
					add_option( 'learning_online_paypal_sandbox_email', !empty( $options['paypal_sandbox_email'] ) ? $options['paypal_sandbox_email'] : '' );
					add_option( 'learning_online_paypal_sandbox', !empty( $options['sandbox'] ) ? 'yes' : 'no' );
				}
			}
		}
		...
		if ( $_lpr_settings_emails = get_option( '_lpr_settings_emails' ) ) {
			$emails = array(
				'published_course' => array(
					'learning_online_emails_published_course',
					'published-course'
				),
				'enrolled_course'  => array(
					'learning_online_emails_enrolled_course',
					'enrolled-course'
				),
				'passed_course'    => array(
					'learning_online_emails_finished_course',
					'finished-course'
				)
			);
			...
			foreach ( $_lpr_settings_emails as $email_type => $email ) {
				if ( in_array( $email_type, array( 'published_course', 'enrolled_course', 'passed_course' ) ) ) {
					...
					if ( !empty( $email['message'] ) ) {
						...
						$templates = array( 'learningonline' );
						if ( get_template() == 'eduma' ) {
							$templates[] = 'learningonline-v1';
						}
					...
					$new_email               = wp_parse_args(
						get_option( 'learning_online_emails_general' ),
						array(
							'from_name'  => '',
							'from_email' => ''
						)
					...
					add_option( 'learning_online_emails_general', $new_email );
				}
			}
		}
	}
	...
	private function _mark_upgraded( $old, $new ) {
		update_post_meta( $old, '_learning_online_upgraded', $new );
		update_post_meta( $new, '_learning_online_upgraded_from', $old );
	}
	...
	public function do_upgrade() {
		...
		$force = learning_online_get_request( 'force' ) == 'true';
		try {
		...
		$wpdb->query( "COMMIT;" );
		update_option( 'learningonline_version', LP()->version );
		update_option( 'learningonline_db_version', LP()->version );
		update_option( '_learning_online_flush_rewrite_rules', 'yes' );
		// cui bap
		update_option( 'permalink_structure', '/%postname%/' );
		delete_transient( 'learning_online_is_old_version' );

		learning_online_update_log( '1.0.x', array( 'time' => time() ) );
		return true;
	}
	 ...
	 */
	public function learning_online_upgrade_10_page() {
		if ( empty( $_REQUEST['page'] ) || $_REQUEST['page'] != 'learning_online_upgrade_from_09' ) return;
		if ( empty( $_REQUEST['_wpnonce'] ) || !wp_verify_nonce( $_REQUEST['_wpnonce'], 'learning-online-upgrade-09' ) ) {
		...
		wp_enqueue_style( 'learning-online-upgrade-x', LP()->plugin_url( 'inc/updates/09/style.css' ), array( 'dashicons', 'install' ) );
		wp_enqueue_script( 'learning-online-upgrade-x', LP()->plugin_url( 'inc/updates/09/script.js' ), array( 'jquery' ) );

		add_action( 'learning_online_update_step_welcome', array( $this, 'update_welcome' ) );
		add_action( 'learning_online_update_step_upgraded', array( $this, 'update_upgraded' ) );
		...
		$view                = learning_online_get_admin_view( 'updates/0.9/update-wizard.php' );
		include_once $view;
		exit();
	}
	 ...
	 */
	public function learning_online_update_10_menu() {
	...
	public function update_welcome() {
		$view = learning_online_get_admin_view( 'updates/0.9/step-welcome.php' );
		include $view;
	}

	public function update_upgraded() {
		$view = learning_online_get_admin_view( 'updates/0.9/step-upgraded.php' );
		include $view;
	}
	...
	public function update_repair_database() {
		$view = learning_online_get_admin_view( 'updates/0.9/step-repair-database.php' );
		include $view;
	}
		...
		if ( $this->_current_step ) {
			if ( ( $pos = array_search( $this->_current_step, $this->_steps ) ) !== false ) {
				if ( $pos < sizeof( $this->_steps ) - 1 ) {
					$pos ++;
					return admin_url( 'admin.php?page=learning_online_upgrade_from_09&step=' . $this->_steps[$pos] );
				}
			}
		}
		return false;
