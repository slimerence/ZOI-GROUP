;(function ($) {
	$(document).ready(function () {
		$('#learning-online-upgrade-course-actions .button').click(function () {
			var $button = $(this),
			...
			if (action == 'upgrade') {
				$.ajax({
					...
					data    : {
						action: 'learning_online_upgrade_courses'
					},
					dataType: 'text',
				...
				$button.parent().hide();
				$('#learning-online-confirm-abort-upgrade-course').show();
			}
		});
		$('#learning-online-confirm-abort-upgrade-course .button').click(function () {
			var $button = $(this),
				action = $button.attr('data-action');
			if (action == 'yes') {
				$.ajax({
					url     : LP_Settings.ajax,
					data    : {
						action   : 'learning_online_hide_upgrade_notice',
						ask_again: $('#learning-online-ask-again-abort-upgrade').is(':checked') ? 'no' : 'yes'
					},
					dataType: 'text',
					success : function (response) {
						response = LP.parseJSON(response);
				...
				$('#learning-online-confirm-abort-upgrade-course').hide();
				$('#learning-online-upgrade-course-actions').show();
			}
		});
	});
...
;(function ($) {
	...
	$doc.ready(function () {
		$doc.on('click', '#button-repair-database', function (e) {
			...
			e.preventDefault();
			doRollbackDatabase();
		}).on('click', '#learning-online-update-button', function (e) {
			e.preventDefault();
