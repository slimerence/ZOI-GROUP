...
if ( LEARNING_ONLINE_UPDATE_DATABASE ) {
	...
	try {
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) === $table ) {
			$query = $wpdb->prepare( "
				INSERT INTO {$wpdb->prefix}learningonline_user_items(`user_item_id`, `user_id`, `item_id`, `item_type`, `start_time`, `end_time`, `status`, `ref_id`, `ref_type`)
				(
			...
			$query = $wpdb->prepare( "
				INSERT INTO {$wpdb->learningonline_user_itemmeta}(`learningonline_user_item_id`, `meta_key`, `meta_value`)
		...
		$query = $wpdb->prepare( "
			UPDATE {$wpdb->prefix}learningonline_user_itemmeta
			SET meta_key = CASE
		...
		if ( $item_empty_course = $wpdb->get_results( $query ) ) {
			...
			$query = $wpdb->prepare( "
				...
				FROM {$wpdb->learningonline_section_items} si
				INNER JOIN {$wpdb->learningonline_sections} s ON si.section_id = s.section_id
			...
			if ( $item_courses = $wpdb->get_results( $query ) ) {
				foreach ( $item_courses as $row ) {
					$wpdb->update(
						$wpdb->learningonline_user_items,
		...
		$table = $wpdb->prefix . 'learningonline_user_courses';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) === $table ) {
			$query = $wpdb->prepare( "
				INSERT INTO {$wpdb->learningonline_user_items}(`user_id`, `item_id`, `item_type`, `start_time`, `end_time`, `status`, `ref_id`, `ref_type`)
				SELECT `user_id`, `course_id`, %s, `start_time`, `end_time`, `status`, `order_id`, %s
				FROM {$wpdb->prefix}learningonline_user_courses
			", 'lp_course', 'lp_order' );
			$wpdb->query( $query );
		}
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) === $table ) {
			$query = $wpdb->prepare( "
				...
				FROM {$wpdb->prefix}learningonline_user_lessons
			", 'lp_lesson', '0000-00-00 00:00:00', '0000-00-00 00:00:00', 'lp_course' );
			$wpdb->query( $query );
		}
		// remove auto-increment
		//$query = "ALTER TABLE {$wpdb->prefix}learningonline_user_courses` MODIFY COLUMN `user_course_item_id` BIGINT(20) UNSIGNED NOT NULL DEFAULT 0;";

		// remove auto-increment
		$table = $wpdb->prefix . 'learningonline_user_course_items';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) === $table ) {
			$query = "ALTER TABLE {$wpdb->prefix}learningonline_user_course_items MODIFY COLUMN `user_course_item_id` BIGINT(20) UNSIGNED NOT NULL DEFAULT 0;";
			$wpdb->query( $query );
		}
		...
		foreach ( $old_tables as $old_table ) {
			$table = $wpdb->prefix . 'learningonline_' . $old_table;
			if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) === $table ) {
		...
		learning_online_update_log( '2.0', array( 'time' => time() ) );

		do_action( 'learning_online_upgrade_database', '2.0' );
	} catch ( Exception $ex ) {
		$wpdb->query( "ROLLBACK;" );
	}
}