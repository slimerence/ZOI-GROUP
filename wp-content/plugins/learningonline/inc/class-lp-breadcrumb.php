...
class LP_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'learning_online_get_breadcrumb', $this->crumbs, $this );
	}
	...
	public function generate() {
		$conditionals = array(
			...
			'is_attachment',
			'is_single',
			'learning_online_is_course_category',
			'learning_online_is_course_tag',
			'learning_online_is_courses',
			'is_page',
		...
		if ( ( !is_front_page() && !( is_post_type_archive() && get_option( 'page_on_front' ) == learning_online_get_page_id( 'courses' ) ) ) || is_paged() ) {
			foreach ( $conditionals as $conditional ) {
				if ( is_callable( $conditional ) && call_user_func( $conditional ) ) {
					$conditional = preg_replace( '/^learning_online_/', '', $conditional );
	...
	private function prepend_courses_page() {
		$permalinks      = get_option( 'learning_online_permalinks' );
	...
	private function add_crumbs_404() {
		$this->add_crumb( __( 'Error 404', 'learningonline' ) );
	}
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'lp_course' === get_post_type( $post ) ) {
			...
			if ( $terms = learning_online_get_course_terms( $post->ID, 'course_category', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
				$main_term = apply_filters( 'learning_online_breadcrumb_main_term', $terms[0], $terms );
	...
	private function add_crumbs_course_tag() {
		...
		$this->prepend_courses_page();
		$this->add_crumb( sprintf( __( 'Courses tagged &ldquo;%s&rdquo;', 'learningonline' ), $current_term->name ) );
	}
	...
	private function add_crumbs_courses() {
		...
		$_name = learning_online_get_page_id( 'courses' ) ? get_the_title( learning_online_get_page_id( 'courses' ) ) : '';
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( __( 'Posts tagged &ldquo;%s&rdquo;', 'learningonline' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( __( 'Author: %s', 'learningonline' ), $userdata->display_name ) );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( sprintf( __( 'Search results for &ldquo;%s&rdquo;', 'learningonline' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( sprintf( __( 'Page %d', 'learningonline' ), get_query_var( 'paged' ) ) );
		}
	}
}
