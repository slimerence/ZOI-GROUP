...
define( 'LEARNING_ONLINE_UPDATE_DATABASE', true );
...
class LP_Install {
	...
	public static function init() {
		add_action( 'learning_online_activate', array( __CLASS__, 'install' ) );
		...
		add_action( 'wp_ajax_learning_online_hide_upgrade_notice', array( __CLASS__, 'hide_upgrade_notice' ) );
	...
	public static function include_update() {
		...
		// Update LearningOnline from 0.9.x to 1.0
		if ( version_compare( learning_online_get_current_version(), $latest_version, '=' ) ) {
			add_action( 'admin_notices', array( __CLASS__, 'hide_other_notices' ), - 100 );
			learning_online_include( 'updates/' . self::$_update_files[$latest_version] );
		}
	}

	public static function hide_other_notices() {
		//remove_action( 'admin_notices', 'learning_online_one_click_install_sample_data_notice' );
	}

	public static function update_from_09() {

		if ( !self::_has_new_table() || version_compare( LEARNINGONLINE_VERSION, get_option( 'learningonline_db_version' ), '>' ) ) {
			self::install();
		}
		if ( !get_option( 'learningonline_version' ) || !get_option( 'learning_online_currency' ) ) {
			self::_create_options();
		}
		$ask = get_transient( 'learning_online_upgrade_courses_ask_again' );
		if ( self::_need_to_update() ) {
			// Notify for administrator
			if ( empty( $ask ) && learning_online_current_user_is( 'administrator' ) ) {
				LP_Assets::enqueue_style( 'learning-online-upgrade', LP()->plugin_url( 'inc/updates/09/style.css' ) );
				LP_Assets::enqueue_script( 'learning-online-upgrade', LP()->plugin_url( 'inc/updates/09/script.js' ) );
				$upgrade_url = wp_nonce_url( admin_url( 'options-general.php?page=learning_online_upgrade_from_09' ), 'learning-online-upgrade-09' );
				$message     = sprintf( '<p>%s</p>', __( 'It seems like you have updated LearningOnline from an older version and there are some outdated courses or data that need to be upgraded.', 'learningonline' ) );
				$message .= sprintf( '<div id="learning-online-confirm-abort-upgrade-course"><p><label><input type="checkbox" id="learning-online-ask-again-abort-upgrade" /> %s</label></p><p><button href="" class="button disabled" data-action="yes">%s</button> <button href="" class="button" data-action="no">%s</button> </p></div>', __( 'Do not ask again.', 'learningonline' ), __( 'Ok', 'learningonline' ), __( 'Cancel', 'learningonline' ) );
				$message .= sprintf( '<p id="learning-online-upgrade-course-actions"><a href="%s" class="button" data-action="upgrade">%s</a>&nbsp;<button class="button disabled" data-action="abort">%s</button></p>', $upgrade_url, __( 'Upgrade now', 'learningonline' ), __( 'No, thank!', 'learningonline' ) );

				LP_Admin_Notice::add( $message, 'error' );
			}

			// Notify for instructor
			if ( learning_online_current_user_is( 'instructor' ) ) {
				LP_Admin_Notice::add( sprintf( '<p>%s</p>', __( 'LearningOnline has upgraded and need to upgrade the database before you can work with it. Please notify the site administrator.', 'learningonline' ) ), 'error' );
			}
		}
	}

	public static function admin_menu() {
		add_dashboard_page( '', '', 'manage_options', 'learning_online_upgrade_from_09', '' );
	}

	public static function hide_upgrade_notice() {
		$ask_again  = learning_online_get_request( 'ask_again' );
		$expiration = DAY_IN_SECONDS;
		if ( $ask_again == 'no' ) {
			$expiration = 0;
		}
		set_transient( 'learning_online_upgrade_courses_ask_again', $ask_again, $expiration );
		learning_online_send_json( array( 'result' => 'success', 'message' => sprintf( '<p>%s</p>', __( 'Thank you for using LearningOnline', 'learningonline' ) ) ) );
	}
	...
	public static function get_update_versions() {
		...
		if ( !$patches = get_transient( 'learningonline_update_patches' ) ) {
			$patches = array();
			...
			if ( WP_Filesystem() ) {
				...
				foreach ( $list as $file ) {
					if ( preg_match( '!learningonline-update-([0-9.]+).php!', $file['name'], $matches ) ) {
	...
	public static function check_version() {
		if ( !defined( 'IFRAME_REQUEST' ) && ( get_option( 'learningonline_version' ) != LP()->version ) ) {
			self::install();
		}
	}
	...
	public static function update_actions() {
		if ( !empty( $_GET['upgrade_learningonline'] ) ) {
			self::update();
		}
	}
	...
	public static function db_update_notices() {
		if ( get_option( 'learningonline_db_version' ) != LP()->version ) {
		}
	}
	...
	public static function install() {
		...
		learning_online_delete_user_option( 'hide-notice-template-files' );
	...
	private static function _create_cron_jobs() {
		wp_clear_scheduled_hook( 'learning_online_cleanup_sessions' );
		wp_schedule_event( time(), apply_filters( 'learning_online_cleanup_session_recurrence', 'twicedaily' ), 'learning_online_cleanup_sessions' );
	}
	...
	public static function _search_page( $type, $types ) {
		...
		if ( empty( $pages[$type] ) ) {
			...
			$args     = array( '_learning_online_page' );
	...
	/**
	 * Remove learningonline page if total of learn page > 10
	 * @global type $wpdb
	 * @return type
	 */
	public static function _remove_pages() {
		...
		$sql       = 'SELECT * '
			...
			. ' ON p.ID=pm.post_id AND pm.meta_key="_learning_online_page" AND p.post_type="page";';
		...
		foreach ( $pages as $page ) {
			delete_option( "learning_online_{$page}_page_id" );
		}
		sleep( 5 );
		return array();
	...
	public static function create_pages() {
		...
		$pages = self::$_pages;
		foreach ( $pages as $page ) {
			$page_id = get_option( "learning_online_{$page}_page_id" );
			...
			if ( $page_id ) {
				update_option( "learning_online_{$page}_page_id", $page_id );
				update_post_meta( $page_id, '_learning_online_page', $page );
			}
		}
		flush_rewrite_rules();
	}
	...
	private function _is_old_version() {
		if ( is_null( self::$_is_old_version ) ) {
			$is_old_version = get_transient( 'learning_online_is_old_version' );

			if ( empty( $is_old_version ) ) {
				if ( !get_option( 'learningonline_db_version' ) ||
					get_posts(
						array(
				...
				set_transient( 'learning_online_is_old_version', $is_old_version );
			}
	 ...
	 * If a course has got a meta key like _learning_online_upgraded that means it is not upgraded
	 *
	 * @return mixed
	 */
	private static function _has_old_posts() {
		...
		$query = $wpdb->prepare( "
			...
			LIMIT 0, 1
		", '_learning_online_upgraded', 'lpr_course' );
	...
	private static function _has_new_table() {
		...
		", DB_NAME, '%learningonline_sections%' );
	...
	public static function update() {
		$learningonline_db_version = get_option( 'learningonline_db_version' );

		foreach ( self::$_update_files as $version => $updater ) {
			if ( version_compare( $learningonline_db_version, $version, '<' ) ) {
	...
	public static function update_db_version( $version = null ) {
		...
		update_option( 'learningonline_db_version', is_null( $version ) ? LEARNINGONLINE_VERSION : $version );
	}

	public static function update_version( $version = null ) {

		delete_option( 'learningonline_version' );
		update_option( 'learningonline_version', is_null( $version ) ? LEARNINGONLINE_VERSION : $version );
	}
	...
	private static function _create_options() {
		...
		$custom_options = array(
			'learning_online_course_base_type'   => 'custom',
			'learning_online_paypal_email'       => get_option( 'admin_email' ),
			'learning_online_paypal_enable'      => 'yes',
			'learning_online_profile_endpoints'  => 'a:4:{s:15:"profile-courses";s:7:"courses";s:15:"profile-quizzes";s:7:"quizzes";s:14:"profile-orders";s:6:"orders";s:21:"profile-order-details";s:13:"order-details";}',
			'learning_online_checkout_endpoints' => 'a:1:{s:17:"lp_order_received";s:17:"lp-order-received";}'
		);
		...
		update_option( 'learning_online_install', 'yes');
	}
	...
	private static function _get_schema() {
		...
		$table = $wpdb->prefix . 'learningonline_order_itemmeta';
		$query = '';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE {$wpdb->prefix}learningonline_order_itemmeta (
					meta_id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
					learningonline_order_item_id bigint(20) unsigned NOT NULL DEFAULT '0',
					meta_key varchar(45) NOT NULL DEFAULT '',
					meta_value longtext NOT NULL,
					PRIMARY KEY  (meta_id)
				) $collate;";
		}
		$table = $wpdb->prefix . 'learningonline_order_items';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE {$wpdb->prefix}learningonline_order_items (
		...
		$table = $wpdb->prefix . 'learningonline_question_answers';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE {$wpdb->prefix}learningonline_question_answers (
		...
		$table = $wpdb->prefix . 'learningonline_quiz_questions';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE {$wpdb->prefix}learningonline_quiz_questions (
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE {$wpdb->prefix}learningonline_review_logs (
		...
		$table = $wpdb->prefix . 'learningonline_section_items';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE {$wpdb->prefix}learningonline_section_items (
		...
		$table = $wpdb->prefix . 'learningonline_sections';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE {$wpdb->prefix}learningonline_sections (
		...
		$table = $wpdb->prefix . 'learningonline_sessions';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE  {$wpdb->prefix}learningonline_sessions (
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE {$wpdb->prefix}learningonline_user_items (
		...
		$table = $wpdb->prefix . 'learningonline_user_itemmeta';
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
			$query .= "
				CREATE TABLE {$wpdb->prefix}learningonline_user_itemmeta (
					meta_id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
					learningonline_user_item_id bigint(20) unsigned NOT NULL,
					meta_key varchar(45) NOT NULL DEFAULT '',
