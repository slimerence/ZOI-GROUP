...
class LP_User_Factory {
	...
	public static function init() {
		...
		add_action( 'learning_online_user_start_quiz', array( __CLASS__, 'start_quiz' ), 10, 4 );
		add_action( 'learning_online_user_retake_quiz', array( __CLASS__, 'retake_quiz' ), 10, 4 );
		add_action( 'learning_online_activate', array( __CLASS__, 'register_event' ), 15 );
		add_action( 'learning_online_deactivate', array( __CLASS__, 'deregister_event' ), 15 );
		add_action( 'learning_online_schedule_cleanup_temp_users', array( __CLASS__, 'schedule_cleanup_temp_users' ) );
	...
	public static function cron_schedules( $schedules ) {
		$schedules['every_three_minutes'] = array(
			...
			'display'  => __( 'Every 3 Minutes', 'learningonline' )
		);
	...
	public static function register_event() {
		if ( ! wp_next_scheduled( 'learning_online_schedule_cleanup_temp_users' ) ) {
			wp_schedule_event( time(), 'every_three_minutes', 'learning_online_schedule_cleanup_temp_users' );
		}
	}
	...
	public static function deregister_event() {
		wp_clear_scheduled_hook( 'learning_online_schedule_cleanup_temp_users' );
	}
	...
	public static function schedule_cleanup_temp_users() {
		...
		$query = $wpdb->prepare( "
			SELECT user_id
			FROM {$wpdb->prefix}learningonline_user_items c
			INNER JOIN {$wpdb->prefix}learningonline_user_itemmeta d ON c.user_item_id = d.learningonline_user_item_id AND d.meta_key = %s
			INNER JOIN {$wpdb->prefix}learningonline_user_itemmeta e ON c.user_item_id = e.learningonline_user_item_id AND e.meta_key = %s AND e.meta_value < TIMESTAMPADD(SECOND, %d, NOW())
		", 'temp_user_id', 'temp_user_time', self::$_guest_transient );

		if ( $uids = $wpdb->get_col( $query ) ) {
			$query = $wpdb->prepare( "
				DELETE a.*, b.*
				FROM {$wpdb->prefix}learningonline_user_items a
				INNER JOIN {$wpdb->prefix}learningonline_user_itemmeta b
				WHERE %d
				AND a.user_item_id = b.learningonline_user_item_id
	...
	public static function generate_guest_id() {
		...
		if ( ! $id ) {
			...
			if ( ! is_user_logged_in() ) {
				learning_online_setcookie( 'learning_online_user_guest_id', $id, time() + self::$_guest_transient );
				set_transient( 'learning_online_user_guest_' . $id, $id, self::$_guest_transient );
			}
		}
	...
	public static function get_guest_id() {
		return empty( $_COOKIE['learning_online_user_guest_id'] ) ? false : $_COOKIE['learning_online_user_guest_id'];
	}
	...
	public static function get_user_class( $the_id = 0 ) {
		...
		return apply_filters( 'learning_online_user_class', $class );
	}
	...
	public static function clear_temp_user_data( $user_login ) {
		if ( $temp_id = self::get_guest_id() ) {
			learning_online_remove_cookie( 'learning_online_user_guest_id' );
			delete_transient( 'learning_online_user_guest_' . $temp_id );
			...
			$query = $wpdb->prepare( "
				SELECT user_item_id
				FROM {$wpdb->prefix}learningonline_user_items a
				INNER JOIN {$wpdb->prefix}learningonline_user_itemmeta b ON a.user_item_id = b.learningonline_user_item_id AND b.meta_key = %s And b.meta_value = %s
			", 'temp_user_id', 'yes' );
			...
			if ( $user_item_ids ) {
				$query = $wpdb->prepare( "
					...
					FROM {$wpdb->prefix}learningonline_user_items a
					INNER JOIN {$wpdb->prefix}learningonline_user_itemmeta b
					WHERE a.user_item_id = b.learningonline_user_item_id
					AND a.user_id = %d
				", $temp_id );
				$wpdb->query( $query );
			}
	...
	private static function _update_user_item_meta( $item, $quiz_id, $course_id, $user_id ) {
		...
		learning_online_add_user_item_meta( $item_id, 'temp_user_id', 'yes' );
		learning_online_add_user_item_meta( $item_id, 'temp_user_time', date( 'Y-m-d H:i:s', time() ) );
	}
}
