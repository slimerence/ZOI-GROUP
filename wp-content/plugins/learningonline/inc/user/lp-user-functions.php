 ...
 *
 * @author  JwsTheme
 * @package LearningOnline/Functions/User
 * @version 1.0
 */
 ...
 * @param $course_id
 */
function learning_online_delete_user_data( $user_id, $course_id = 0 ) {
	global $wpdb;
...
function learning_online_get_user_item_id( $user_id, $item_id ) {
	$user_item_ids = LP_Cache::get_user_item_id( false, array() );
	if ( empty( $user_item_ids[ $user_id . '-' . $item_id ] ) ) {
		global $wpdb;
		$query                                      = $wpdb->prepare( "SELECT user_item_id FROM {$wpdb->prefix}learningonline_user_items WHERE user_id = %d AND item_id = %d ORDER BY user_item_id DESC LIMIT 0,1", $user_id, $item_id );
 ...
 * @return int
 */
function learning_online_get_current_user_id() {
 ...
 * @return LP_User
 */
function learning_online_get_current_user( $user_id = 0 ) {
 ...
 */
function learning_online_get_user( $user_id, $force = false ) {
 ...
 * @return void
 */
function learning_online_add_user_roles() {
...
add_action( 'learning_online_ready', 'learning_online_add_user_roles' );

function learning_online_get_user_questions( $user_id = null, $args = array() ) {
	if ( ! $user_id ) {
		$user_id = get_current_user_id();
	}

	return learning_online_get_user( $user_id )->get_questions( $args );
}
 ...
 * @return bool|string
 */
function learning_online_current_user_is( $check_type = null ) {
	global $current_user;
...
function learning_online_user_has_roles( $roles, $user_id = null ) {
	$has_role = false;
	if ( ! $user_id ) {
 ...
 */
function learning_online_edit_admin_bar() {
	global $wp_admin_bar;
	if ( ( $profile = learning_online_get_page_id( 'profile' ) ) && get_post_type( $profile ) == 'page' && get_post_status( $profile ) != 'trash' && ( LP()->settings->get( 'admin_bar_link' ) == 'yes' ) ) {
		...
		$course_profile['href']           = learning_online_user_profile_link( $user_id, $first_tab );
...
add_action( 'admin_bar_menu', 'learning_online_edit_admin_bar' );


function learning_online_current_user_can_view_profile_section( $section, $user ) {
	$current_user = wp_get_current_user();
	$view         = true;
	...
	return apply_filters( 'learning_online_current_user_can_view_profile_section', $view, $section, $user );
}

function learning_online_profile_tab_courses_content( $current, $tab, $user ) {
	learning_online_get_template( 'profile/tabs/courses.php', array(
		'user'    => $user,
		'current' => $current,
		'tab'     => $tab
	) );
...
function learning_online_profile_tab_quizzes_content( $current, $tab, $user ) {
	learning_online_get_template( 'profile/tabs/quizzes.php', array(
		'user'    => $user,
		'current' => $current,
		'tab'     => $tab
	) );
...
function learning_online_profile_tab_orders_content( $current, $tab, $user ) {
	learning_online_get_template( 'profile/tabs/orders.php', array(
		'user'    => $user,
		'current' => $current,
		'tab'     => $tab
	) );
...
function learning_online_get_profile_user() {
	global $wp_query;
...
//function learning_online_update_user_lesson_start_time() {
//	global $wpdb;
...
//		return;
//	}
//        $table = $wpdb->prefix . 'learningonline_user_lessons';
//        if ( $wpdb->get_var("SHOW TABLES LIKE '{$table}'") !== $table ) {
//            return;
//        }
//	$query = $wpdb->prepare( "
//		SELECT user_lesson_id FROM {$wpdb->prefix}learningonline_user_lessons WHERE user_id = %d AND lesson_id = %d AND course_id = %d
//	", get_current_user_id(), $lesson->id, $course->id );
//	if ( $wpdb->get_row( $query ) ) {
//		return;
//	}
//	$wpdb->insert(
//		$wpdb->prefix . 'learningonline_user_lessons',
//		array(
//			'user_id'    => get_current_user_id(),
//			'lesson_id'  => $lesson->id,
...
//}
//
//add_action( 'learning_online_course_content_lesson', 'learning_online_update_user_lesson_start_time' );

/**
 * Add instructor registration button to register page and admin bar
 */
function learning_online_user_become_teacher_registration_form() {
    ...
    <p>
        <label for="become_teacher">
			...
			<?php _e( 'Want to be an instructor?', 'learningonline' ) ?>
        </label>
    </p>
	<?php
}

add_action( 'register_form', 'learning_online_user_become_teacher_registration_form' );
 ...
 * @param $user_id
 */
function learning_online_update_user_teacher_role( $user_id ) {
	if ( LP()->settings->get( 'instructor_registration' ) != 'yes' ) {
		return;
	}
...
add_action( 'user_register', 'learning_online_update_user_teacher_role', 10, 1 );
 ...
 * @return mixed
 */
function learning_online_update_user_item_field( $fields, $where = false ) {
	global $wpdb;
	...
	if ( $where && empty( $where['user_id'] ) ) {
		$where['user_id'] = learning_online_get_current_user_id();
	}
	$where_format = array();
 ...
 */
function learning_online_get_user_item( $where, $single = true ) {
	global $wpdb;
 ...
 * @return mixed
 */
function learning_online_get_user_item_meta( $user_item_id, $meta_key, $single = true ) {
	return get_metadata( 'learningonline_user_item', $user_item_id, $meta_key, $single );
}
 ...
 * @return false|int
 */
function learning_online_add_user_item_meta( $user_item_id, $meta_key, $meta_value, $prev_value = '' ) {
	return add_metadata( 'learningonline_user_item', $user_item_id, $meta_key, $meta_value, $prev_value );
}
 ...
 * @return bool|int
 */
function learning_online_update_user_item_meta( $user_item_id, $meta_key, $meta_value, $prev_value = '' ) {
	return update_metadata( 'learningonline_user_item', $user_item_id, $meta_key, $meta_value, $prev_value );
}
 ...
 * @param $_meta_value
 */
function _learning_online_update_created_time_user_item_meta( $meta_id, $object_id, $meta_key, $_meta_value ) {
	global $wpdb;
	$wpdb->update(
		$wpdb->learningonline_user_itemmeta,
...
///add_action( 'added_learningonline_user_item_meta', '_learning_online_update_created_time_user_item_meta', 10, 4 );
 ...
 * @param $_meta_value
 */
function _learning_online_update_updated_time_user_item_meta( $meta_id, $object_id, $meta_key, $_meta_value ) {
	global $wpdb;
	$wpdb->update(
		$wpdb->learningonline_user_itemmeta,
...
//add_action( 'updated_learningonline_user_item_meta', '_learning_online_update_updated_time_user_item_meta', 10, 4 );
 ...
 * @return bool|mixed
 */
function learning_online_user_has_quiz_status( $status, $quiz_id = 0, $user_id = 0, $course_id = 0 ) {
	$user = learning_online_get_user( $user_id );

	return $user->has_quiz_status( $status, $quiz_id, $course_id );
}

//add_action( 'init', 'learning_online_user_update_user_info' );

function learning_online_user_update_user_info() {
	global $wp, $wpdb;
	$user             = learning_online_get_current_user();
	$user_id          = learning_online_get_current_user_id();
	$message_template = '<div class="learning-online-message %s">'
	                    . '<p>%s</p>'
	                    . '</div>';
...
#
		...
		$ppdir  = $upload['basedir'] . DIRECTORY_SEPARATOR . 'learning-online-profile';
		if ( ! is_dir( $ppdir ) ) {
			mkdir( $ppdir );
		}
		...
		$lp_profile_url = $upload['baseurl'] . '/learning-online-profile/' . $user_id . '/';
#
# CREATE SOME DIRECTORY
# - - - - - - - - - - - - - - - - - - - -
		...
		if ( isset( $_POST['sub_action'] ) && 'upload_avatar' === $_POST['sub_action'] && isset( $_FILES['image'] ) ) {
			...
			if ( ( ! empty( $_FILES["image"] ) ) && ( $_FILES['image']['error'] == 0 ) ) {
				...
				if ( ! in_array( $image_type, $mine_types ) ) {
					$_message = __( 'Only', 'learningonline' ) . ' <strong>' . implode( ',', $image_exts ) . '</strong> ' . __( 'images accepted for upload', 'learningonline' );
				...
				if ( $image_size > $image_size_limit * 1048576 ) {
					$message = __( 'Images must be under', 'learningonline' ) . ' ' . $image_size_limit . __( 'MB in size', 'learningonline' );
					$return  = array(
						'return'  => false,
						'message' => $message
					);
				...
				$message = __( 'Please select an image for upload', 'learningonline' );
				$return  = array(
					'return'  => false,
					'message' => $message
				);
			...
			if ( isset( $_FILES['image']['name'] ) ) {
				...
				$_message = $uploaded ? __( 'Image is uploaded success', 'learningonline' ) : __( 'Error on upload image', 'learningonline' );
...
#	
		if ( isset( $_POST['sub_action'] ) && 'crop_avatar' === $_POST['sub_action'] && isset( $_POST['avatar_filename'] ) ) {
			...
			if ( is_wp_error( $editor ) ) {
				learning_online_add_message( __( 'Thumbnail of image profile not created', 'learningonline' ) );
			} else {
				# Calculator new width height
				...
				if ( is_wp_error( $saved ) ) {
					$_message               = __( 'Error on crop user picture profile ', 'learningonline' );
					$message                = sprintf( $message_template, 'error', $_message );
					$res['return']          = false;
					$res['message']         = $message;
					$res['avatar_filename'] = '';
					...
					if ( file_exists( $avatar_filepath ) ) {
						...
						if ( is_wp_error( $editor2 ) ) {
							$_message       = __( 'Thumbnail of image profile not created', 'learningonline' );
							$message        = sprintf( $message_template, 'error', $_message );
							$res['message'] .= $message;
						} else {
					...
					$_message               = __( 'Profile picture is changed', 'learningonline' );
					$message                = sprintf( $message_template, 'success', $_message );
					$res['return']          = true;
					$res['message']         .= $message;
...
#	
		...
		if ( ! empty( $_POST['pass0'] ) && ! empty( $_POST['pass1'] ) && ! empty( $_POST['pass1'] ) ) {
			...
			if ( ! $check_old_pass ) {
				$_message               = __( 'Old password incorrect!', 'learningonline' );
				$message                = sprintf( $message_template, 'error', $_message );
				$return['return']       = false;
				...
				if ( $new_pass != $new_pass2 ) {
					$_message               = __( 'Confirmation password incorrect!', 'learningonline' );
					$message                = sprintf( $message_template, 'error', $_message );
					$return['return']       = false;
					$return['message']      = $message;
					$return['redirect_url'] = '';
		...
		if ( $res ) {
			$_message               = __( 'Your change is saved', 'learningonline' );
			$message                = sprintf( $message_template, 'success', $_message );
			$return['return']       = true;
			$return['message']      = $message;
			$return['redirect_url'] = '';
			...
			$_message               = __( 'Error on update your profile info', 'learningonline' );
			$message                = sprintf( $message_template, 'error', $_message );
			$return['return']       = false;
			$return['message']      = $message;
			$return['redirect_url'] = '';
		...
		$current_url = learning_online_get_page_link( 'profile' ) . $user->user_login . '/edit';
		wp_redirect( $current_url );
		exit();
	}
}

if ( ! function_exists( 'learning_online_pre_get_avatar_callback' ) ) {
	/**
	 * @param        $avatar
	 ...
	 * @return string|void
	 */
	function learning_online_pre_get_avatar_callback( $avatar, $id_or_email = '', $size ) {
...
function learning_online_user_profile_picture_upload_dir( $width_user = true ) {
	static $upload_dir;
	if ( ! $upload_dir ) {
		$upload_dir = wp_upload_dir();
		$subdir     = apply_filters( 'learning_online_user_profile_folder', 'learning-online-profile', $width_user );
		if ( $width_user ) {
...
add_action( 'learning_online_before_purchase_course_handler', '_learning_online_before_purchase_course_handler', 10, 2 );
function _learning_online_before_purchase_course_handler( $course_id, $cart ) {
	// Redirect to login page if user is not logged in
	if ( ! is_user_logged_in() ) {
		$return_url = add_query_arg( $_POST, get_the_permalink( $course_id ) );
		$return_url = apply_filters( 'learning_online_purchase_course_login_redirect_return_url', $return_url );
		$redirect   = apply_filters( 'learning_online_purchase_course_login_redirect', learning_online_get_login_url( $return_url ) );
		if ( $redirect !== false ) {
			learning_online_add_message( __( 'Please login to enroll this course', 'learningonline' ) );
		...
		if ( $user->has_finished_course( $course_id ) ) {
			learning_online_add_message( __( 'You have already finished course', 'learningonline' ) );
			$redirect = true;
		} elseif ( $user->has_purchased_course( $course_id ) ) {
			learning_online_add_message( __( 'You have already enrolled in this course', 'learningonline' ) );
			$redirect = true;
		}
		if ( $redirect ) {
...
function learning_online_user_is( $role, $user_id = 0 ) {
	if ( ! $user_id ) {
		$user = learning_online_get_current_user();
	} else {
		$user = learning_online_get_user( $user_id );
...
//function learning_online_profile_tab_endpoints_edit_profile( $endpoints ) {
//	$endpoints['edit'] = 'edit';
//	print_r($endpoints);
//	return $endpoints;
//}
//
//add_filter( 'learning_online_profile_tab_endpoints', 'learning_online_profile_tab_endpoints_edit_profile' );

function learning_online_profile_tab_edit_content( $current, $tab, $user ) {
	learning_online_get_template( 'profile/tabs/edit.php', array( 'user' => $user, 'current' => $current, 'tab' => $tab ) );
}

function _learning_online_redirect_logout_redirect() {
...
function learning_online_get_profile_endpoints() {
	...
	return apply_filters( 'learning_online_profile_tab_endpoints', $endpoints );
}

add_action( 'wp_logout', '_learning_online_redirect_logout_redirect' );

function learning_online_update_user_option( $name, $value, $id = 0 ) {
	if ( ! $id ) {
		$id = get_current_user_id();
	}
	$key              = 'learningonline_user_options';
 ...
 * @return bool
 */
function learning_online_delete_user_option( $name, $id = 0 ) {
	if ( ! $id ) {
		$id = get_current_user_id();
	}
	$key     = 'learningonline_user_options';
 ...
 * @return bool
 */
function learning_online_get_user_option( $name, $id = 0 ) {
	if ( ! $id ) {
		$id = get_current_user_id();
	}
	$key     = 'learningonline_user_options';
 ...
 * @return array
 */
function learning_online_get_display_name_publicly( $user_info ) {
	...
	return apply_filters( 'learning_online_display_name_publicly', $public_display );
}

/**
 * Check and update user information from request in user profile page
 */
function learning_online_update_user_profile() {
	if ( ! LP()->is_request( 'post' ) ) {
		return;
	}
	$nonce = learning_online_get_request( 'profile-nonce' );
	if ( ! wp_verify_nonce( $nonce, 'learning-online-update-user-profile-' . get_current_user_id() ) ) {
		return;
	}
	$section = learning_online_get_request( 'lp-profile-section' );
	do_action( 'learning_online_update_user_profile_' . $section );
	do_action( 'learning_online_update_user_profile', $section );
}

add_action( 'init', 'learning_online_update_user_profile' );

/**
 * Update user avatar
 */
function learning_online_update_user_profile_avatar() {
	$upload_dir = learning_online_user_profile_picture_upload_dir();
	if ( learning_online_get_request( 'lp-user-avatar-custom' ) != 'yes' ) {
		delete_user_meta( get_current_user_id(), '_lp_profile_picture' );
	} else {
		$data = learning_online_get_request( 'lp-user-avatar-crop' );
	...
	learning_online_add_message( __( 'Your avatar updated', 'learningonline' ) );
	wp_redirect( learning_online_get_current_url() );
	exit;
}

add_action( 'learning_online_update_user_profile_avatar', 'learning_online_update_user_profile_avatar' );

/**
 * Update user basic information
 */
function learning_online_update_user_profile_basic_information() {
	$user_id     = learning_online_get_current_user_id();
	$update_data = array(
		'ID'           => $user_id,
		'first_name'   => filter_input( INPUT_POST, 'first_name', FILTER_SANITIZE_STRING ),
	...
	$res         = wp_update_user( $update_data );
	if ( $res ) {
		$message = __( 'Your change is saved', 'learningonline' );
	} else {
		$message = __( 'Error on update your profile info', 'learningonline' );
	}
...
add_action( 'learning_online_update_user_profile_basic-information', 'learning_online_update_user_profile_basic_information' );

/**
 * Change password
 */
function learning_online_update_user_profile_change_password() {
	# check and update pass word
	// check old pass
	$old_pass       = filter_input( INPUT_POST, 'pass0' );
	$check_old_pass = false;
	...
	if ( ! $check_old_pass ) {
		$message = __( 'Old password incorrect!', 'learningonline' );
	} else {
		// check new pass
		$new_pass  = filter_input( INPUT_POST, 'pass1' );
		$new_pass2 = filter_input( INPUT_POST, 'pass2' );
		if ( $new_pass != $new_pass2 ) {
			$message = __( 'Confirmation password incorrect!', 'learningonline' );
		} else {
			$update_data = array(
				'user_pass' => $new_pass,
				'ID'        => learning_online_get_current_user_id()
			);
			if ( wp_update_user( $update_data ) ) {
				$message = __( 'Your password updated', 'learningonline' );
			} else {
				$message = __( 'Change your password failed', 'learningonline' );
			}
		}
	}
	learning_online_add_message( $message );
...
add_action( 'learning_online_update_user_profile_change-password', 'learning_online_update_user_profile_change_password' );

function learning_online_get_avatar_thumb_size() {
 ...
 * @return mixed
 */
function learning_online_get_user_courses_info( $user_id, $course_ids ) {
	global $wpdb;
...
function learning_online_set_user_cookie_for_guest() {
	if ( learning_online_is_course() && ! is_admin() ) {
...
add_action( 'wp', 'learning_online_set_user_cookie_for_guest' );