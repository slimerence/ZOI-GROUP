	...
	class LP_Profile {
		...
		public function get_user() {
			if ( is_numeric( $this->_user ) ) {
				$this->_user = learning_online_get_user( $this->_user );
			} elseif ( empty( $this->_user ) ) {
				$this->_user = learning_online_get_current_user();
			}
			return $this->_user;
		}
		...
		public function get_tabs() {
			...
			$defaults = array(
				...
				$course_endpoint => array(
					...
					'base'     => 'courses',
					'callback' => 'learning_online_profile_tab_courses_content'
				)
			);
			...
			if ( $this->_user->id == get_current_user_id() ) {
				$defaults[$order_endpoint] = array(
					...
					'base'     => 'orders',
					'callback' => 'learning_online_profile_tab_orders_content'
				);
			}

			$tabs = apply_filters( 'learning_online_user_profile_tabs', $defaults, $this->_user );
			if ( $this->_user->id == get_current_user_id() ) {
				$tabs['settings'] = array(
					'title'    => apply_filters( 'learning_online_user_profile_tab_edit_title', __( 'Settings', 'learningonline' ) ),
					'base'     => 'settings',
					'callback' => 'learning_online_profile_tab_edit_content'
				);
			}
