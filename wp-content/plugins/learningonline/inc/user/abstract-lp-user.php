...
class LP_Abstract_User {
	...
	public function set_course( $the_course = 0 ) {
		...
		$course = learning_online_get_course( $the_course );
		if ( ! $course || ! $course->id ) {
			return;
		}
	...
	private function _get_course_id( $course_id ) {
		if ( ! $course_id && learning_online_is_course() ) {
	...
	private function _create_quiz_history( $quiz_id, $course_id = 0/* added 1.0.4 */ ) {
		...
		if ( ! $course_id ) {
			throw new Exception( __( 'Create quiz history need pass course ID', 'learningonline' ) );
		}
		if ( empty( $this->_quiz_history_id ) ) {
			...
			$wpdb->insert(
				...
				apply_filters( 'learning_online_user_quiz_history_data', array(
					'user_id'    => $this->id,
					'item_id'    => $quiz_id,
					'item_type'  => 'lp_quiz',
					'ref_id'     => $course_id,
			 ...
			 */
			@$wpdb->update(
				$wpdb->prefix . 'learningonline_user_items', //quizzes,
				array(
					'ref_id' => $course_id
				),
			...
			$user_quiz_data    = apply_filters(
				'learning_online_user_quiz_data',
				array(
			...
			foreach ( $user_quiz_data as $key => $value ) {
				...
				learning_online_add_user_quiz_meta( $user_quiz_id, $key, $value );
			}

			do_action( 'learning_online_add_user_quiz_meta', $user_quiz_id, $this );
		} else {
	...
	public function start_quiz( $quiz_id, $course_id = 0 ) {
		...
		if ( ! apply_filters( 'learning_online_before_user_start_quiz', true, $quiz_id, $course_id, $this->id ) ) {
			return false;
		}
		$history = $this->get_quiz_history( $quiz_id, $course_id, null, true );
		if ( ! $history ) {
			$return = learning_online_update_user_item_field(
				array(
					'status'     => 'started',
					'start_time' => current_time( 'mysql' ),
					'item_id'    => $quiz_id,
		...
		if ( $return ) {
			$item = learning_online_get_user_item(
				array( 'item_id' => $quiz_id, 'ref_id' => $course_id, 'user_id' => $this->id )
			);
		} else {
			$item = false;
		}
		do_action( 'learning_online_user_start_quiz', $item, $quiz_id, $course_id, $this->id );
...
//			$current_time = current_time( 'timestamp' );
			$current_time   = learning_online_get_current_time();
		...
		return apply_filters( 'learning_online_user_quiz_time_remaining', $remaining, $quiz_id, $course_id, $this->id );
	}
	...
	public function get_current_question_id( $quiz_id = 0 ) {
		...
		if ( ! $current ) {
			...
			if ( $history ) {
				...
				$current = learning_online_get_user_quiz_meta( $keys[0], 'current_question' );
			}
		}
	...
	public function finish_quiz( $quiz_id, $course_id, $args = '' ) {
		if ( ! apply_filters( 'learning_online_before_user_finish_quiz', true, $quiz_id, $course_id, $this->id ) ) {
			return false;
		}
		...
		if ( $progress ) {
			...
			$updated = learning_online_update_user_item_field(
				array(
					'status'   => 'completed',
			...
			if ( $updated ) {
				if ( is_array( $args ) ) {
					foreach ( $args as $k => $v ) {
						learning_online_update_user_item_meta( $progress->history_id, $k, $v );
					}
				}
		...
		do_action( 'learning_online_user_finish_quiz', $quiz_id, $course_id, $this->id );
	...
	public function retake_quiz( $quiz_id, $course_id ) {
		if ( ! apply_filters( 'learning_online_before_user_retake_quiz', true, $quiz_id, $course_id, $this->id ) ) {
			return false;
		}
		$course_id = $this->_get_course_id( $course_id );
		$response  = false;
		...
		if ( $return ) {
			...
			learning_online_update_user_item_meta( $return, 'questions', $questions );
			learning_online_update_user_item_meta( $return, 'current_question', $question );
			learning_online_update_user_item_meta( $return, 'question_answers', array() );
			LP_Cache::flush();
			$response = $this->get_quiz_results( $quiz_id, $course_id, true );
		}
		do_action( 'learning_online_user_retake_quiz', $response, $quiz_id, $course_id, $this->id );
	...
	public function get_quiz_status( $quiz_id, $course_id = 0, $force = false ) {
		...
		$cached = LP_Cache::get_quiz_status( false, array() );// (array) wp_cache_get( 'user-quiz-statuses', 'learningonline' );
		if ( ( ! array_key_exists( $this->id . '-' . $course_id . '-' . $quiz_id, $cached ) || $force ) && $quiz_ids = $this->get_course_items( 'lp_quiz', 'key' ) ) {
			...
			//wp_cache_set( 'user-quiz-statuses', $cached, 'learningonline' );
			LP_Cache::set_quiz_status( $this->id . '-' . $course_id . '-' . $quiz_id, $cached[ $this->id . '-' . $course_id . '-' . $quiz_id ] );
		}

		//learning_online_debug( __FILE__, current_filter(), get_the_ID(), $cached, $quiz_ids );
		$status = ! empty( $cached[ $this->id . '-' . $course_id . '-' . $quiz_id ] ) ? $cached[ $this->id . '-' . $course_id . '-' . $quiz_id ] : false;

		return apply_filters( 'learning_online_user_quiz_status', $status, $quiz_id, $this->id, $course_id );
	}
	...
	public function get_lesson_status( $lesson_id, $course_id = 0, $force = false ) {
		...
		$cached = (array) wp_cache_get( 'user-lesson-statuses', 'learningonline' );
		if ( ( ! array_key_exists( $this->id . '-' . $course_id . '-' . $lesson_id, $cached ) || $force ) && $lesson_ids = $this->get_course_items( 'lp_lesson', 'key' ) ) {
			...
			wp_cache_set( 'user-lesson-statuses', $cached, 'learningonline' );
			//learning_online_debug( __FILE__, $query, $cached );
		}

		return apply_filters( 'learning_online_user_lesson_status', $cached[ $this->id . '-' . $course_id . '-' . $lesson_id ], $lesson_id, $this->id, $course_id );
	}

	public function get_item_status( $item_id, $course_id = 0, $force = false ) {
		$course_id = $this->_get_course_id( $course_id );

		_learning_online_parse_user_item_statuses( $this->id, $course_id, $force );
		...
		if ( ( ! array_key_exists( $key, $item_statuses ) || $force ) ) {
			if ( $item_ids = $this->get_course_items( '', 'key', $course_id ) ) {
				...
				$query = $wpdb->prepare( "
                    ...
                    FROM {$wpdb->prefix}learningonline_user_items o
                    WHERE user_item_id = (SELECT MAX(user_item_id) FROM {$wpdb->prefix}learningonline_user_items s2 WHERE s2.item_id = o.item_id)
				...
				if ( $rows = $wpdb->get_results( $query ) ) {
					foreach ( $rows as $row ) {
						$item_statuses[ $this->id . '-' . $course_id . '-' . $row->item_id ] = learning_online_validate_item_status( $row );
					}
				}
			}
		...
		return apply_filters( 'learning_online_user_course_item_status', $item_statuses[ $key ], $item_id, $course_id, $this->id );
	}
	...
	public function has_quiz_status( $statuses, $quiz_id, $course_id = 0, $force = false ) {
		...
		return apply_filters( 'learning_online_user_has_quiz_status', in_array( $status, $statuses ), $statuses, $status, $quiz_id, $course_id, $this->id );
	}
	...
	public function get_quiz_last_results( $quiz_id ) {
		...
		return apply_filters( 'learning_online_user_quiz_last_results', $results, $quiz_id, $this );
	}
	...
	public function get_quiz_info( $quiz_id, $course_id = 0, $field = null ) {
		...
		if ( empty( $quizzes[ $quiz_id ] ) ) {
			global $wpdb;
			$table     = $wpdb->prefix . 'learningonline_user_items';//{$wpdb->learningonline_user_quizzes}
			$query     = $wpdb->prepare( "
				SELECT *
				FROM $table
				WHERE user_id = %d
		...
		if ( $field ) {
			...
			return apply_filters( 'learning_online_user_quiz_' . $field, $info, $this );
		} else {
			$info = $quizzes[ $quiz_id ];

			return apply_filters( 'learning_online_user_quiz_info', $info, $this );
		}
	...
	public function get_quiz_history( $quiz_id, $course_id = 0, $history_id = null, $force = false ) {
		...
		$cached = LP_Cache::get_quiz_history( false, array() );// wp_cache_get( 'user-quiz-history', 'learningonline' );

		if ( ( ! array_key_exists( $key, $cached ) || $force ) && $quizzes && in_array( $quiz_id, $quizzes ) ) {
			global $wpdb;
			$t1             = $wpdb->prefix . 'learningonline_user_items'; //{$wpdb->learningonline_user_quizzes}
			$t2             = $wpdb->prefix . 'learningonline_user_itemmeta'; //{$wpdb->learningonline_user_quizzes}
		...
		return apply_filters( 'learning_online_user_quiz_history', isset( $cached[ $key ] ) ? $cached[ $key ] : array(), $this, $quiz_id );
	}
	...
	private function _get_quiz_meta( $user_item_id ) {
		...
		$query = $wpdb->prepare( "
			SELECT learningonline_user_item_id as user_item_id, meta_key, meta_value, item_id
			FROM {$wpdb->prefix}learningonline_user_itemmeta im
			INNER JOIN {$wpdb->prefix}learningonline_user_items i ON i.user_item_id = im.learningonline_user_item_id
			WHERE learningonline_user_item_id IN(" . join( ',', $in ) . ")
		", $user_item_id );
	...
	public function get_quiz_progress( $quiz_id, $course_id = 0 ) {
		 ...
		 * }
		 * return apply_filters( 'learning_online_user_quiz_progress', $progress, $quiz_id, $course_id, $this->id );
		 *
		 **/
	}
	...
	public function get_current_quiz_question( $quiz_id, $course_id = 0 ) {
		...
		$current_question_id = $history ? learning_online_get_user_item_meta( $history->history_id, 'lp_current_question_after_close', true ) : array();
		if ( ! empty( $current_question_id ) ) {
			$question_id = $current_question_id;
		}

		return apply_filters( 'learning_online_user_current_quiz_question', absint( $question_id ), $quiz_id, $course_id, $this->id );
	}
	...
	public function get_finished_courses() {
		...
		$query = $wpdb->prepare( "
			...
			FROM {$wpdb->posts} p
			INNER JOIN {$wpdb->prefix}learningonline_user_items uc ON p.ID = uc.item_id
			WHERE uc.user_id = %d
			AND uc.status = %s
		", $this->id, 'finished' );

		return apply_filters( 'learning_online_user_finished_courses', $wpdb->get_results( $query ) );
	}
	...
	public function get_posts( $args = array() ) {
		...
		$args  = apply_filters( 'learning_online_get_user_posts', $args, $this );
	...
	public function get_quizzes( &$args = array(), $force = false ) {
		...
		if ( empty( $quizzes[ $key ] ) || $force ) {
			...
			$query = $wpdb->prepare( "
				...
				FROM {$wpdb->posts} q
				INNER JOIN {$wpdb->prefix}learningonline_user_items uq ON uq.item_id = q.ID
		...
		return apply_filters( 'learning_online_get_user_quizzes', $quizzes[ $key ]['rows'], $args );
	}
	...
	public function get_lessons( $args = array(), $force = false ) {
		...
		return apply_filters( 'learning_online_get_user_lessons', $lessons );
	}
	...
	public function can( $role ) {
		...
		if ( is_callable( $callback ) ) {
			...
			throw new Exception( sprintf( __( 'The role %s for user doesn\'t exists', 'learningonline' ), $role ) );
		}
	}
	...
	public function can_purchase_course( $course_id ) {
		...
		return apply_filters( 'learning_online_user_can_purchase_course', $purchasable, $this, $course_id );
	}
	...
	public function can_enroll_course( $course_id ) {
		...
		$enrollable = apply_filters( 'learning_online_user_can_enroll_course', $enrollable, $this, $course_id );
	...
	public function can_view_item( $item_id, $course_id = 0 ) {
		...
		return apply_filters( 'learning_online_user_can_view_item', $return, $item_id, $course_id, $this->id );
	}
	...
	public function can_edit_item( $item_id, $course_id = 0 ) {
		...
		if ( ! $return ) {
			...
			$course_author = learning_online_get_course_user( $course_id );
			if ( $course_author && $course_author->id == $this->id ) {
				$return = true;
			}
		}

		return apply_filters( 'learning_online_user_can_edit_item', $return, $item_id, $course_id, $this->id );
	}
	...
	public function can_view_lesson( $lesson_id, $course_id = 0 ) {
		...
		return apply_filters( 'learning_online_user_view_lesson', $view, $lesson_id, $this->id, $course_id );
	}
	...
	public function can_view_quiz( $quiz_id, $course_id = 0 ) {
		...
		return apply_filters( 'learning_online_user_view_quiz', $view, $quiz_id, $this->id, $course_id );
	}
	...
	public function can_retake_quiz( $quiz_id, $course_id = 0, $force = false ) {
		...
		return apply_filters( 'learning_online_user_can_retake_quiz', $can, $quiz_id, $this->id, $course_id );
	}
	...
	public function can_finish_course( $course_id ) {
		...
		return apply_filters( 'learning_online_user_can_finish_course', $return, $course_id, $this->id );
	}
	...
	public function can_retake_course( $course_id, $force = false ) {
		...
		return apply_filters( 'learning_online_user_can_retake_course', $can, $course->id, $this->id );
	}
	...
	public function get_incomplete_items( $course_id ) {
		...
		$query    = $wpdb->prepare( "
			SELECT user_item_id
			FROM {$wpdb->learningonline_user_items}
			WHERE user_id = %d
		...
		return apply_filters( 'learning_online_user_incomplete_items', $item_ids, $course_id, $this->id );
	}
	...
	public function finish_course( $course_id ) {
		...
		if ( $course = LP_Course::get_course( $course_id ) ) {
			if ( ! $this->can( 'finish-course', $course_id ) && 1 == 0 ) {
				...
				if ( $return ) {
					do_action( 'learning_online_user_finish_course', $course_id, $this->id, $return );
				}
			}
		}

		return apply_filters( 'learning_online_user_finish_course_data', $return, $course_id, $this->id );
	}
	...
	public function get_orders( $last_order = true ) {
		$my_orders = _learning_online_get_user_course_orders( $this->id );
	...
	public function has_enrolled_course( $course_id, $force = false ) {
		...
		return apply_filters( 'learning_online_user_has_enrolled_course', $enrolled, $this, $course_id );
	}
	...
	public function has_finished_course( $course_id, $force = false ) {
		...
		if ( ! empty( $item_statuses ) && array_key_exists( $key, $item_statuses ) && ! $force ) {
			...
			global $wpdb;
			$query                 = $wpdb->prepare( "SELECT status FROM {$wpdb->prefix}learningonline_user_items where user_id=%d and item_id=%d", $this->id, $course_id );
			$finished              = $wpdb->get_var( $query ) == 'finished' ? 'yes' : 'no';
			$item_statuses[ $key ] = $finished;
			LP_Cache::set_item_statuses( $key, $finished );
		}

		return apply_filters( 'learning_online_user_has_finished_course', $finished == 'yes', $this, $course_id );
		...
		if ( empty( $finished_courses[ $course_id ] ) || $force ) {
			...
			$query                          = $wpdb->prepare( "
				SELECT status
				FROM {$wpdb->prefix}learningonline_user_items uc
		...
		return apply_filters( 'learning_online_user_has_finished_course', $finished_courses[ $course_id ] == 'yes', $course_id, $this->id );
	}
	...
	public function has_passed_course( $course_id ) {
		...
		return apply_filters( 'learning_online_user_has_passed_course', $results * 100 >= $course->passing_condition ? $results : false, $course_id, $this );
	}
	...
	public function has_started_quiz( $quiz_id, $course_id = 0 ) {
		...
		return apply_filters( 'learning_online_user_started_quiz', $started, $quiz_id, $course_id, $this->id );
	}
	...
	public function has_completed_quiz( $quiz_id, $course_id = 0 ) {
		...
		return apply_filters( 'learning_online_user_has_completed_quiz', $completed, $quiz_id, $this );
	}
	...
	public function current_quiz_status( $quiz_id, $course_id = 0, $force = false ) {
		...
		global $wpdb;
		//$key_format = '%d-%d';
		$cached = (array) wp_cache_get( 'user-quiz-statuses', 'learningonline' );
		if ( ! array_key_exists( $this->id . '-' . $course_id . '-' . $quiz_id, $cached ) || $force ) {
			$query                                = $wpdb->prepare( "
				SELECT uq.item_id as id, uqm.meta_value as `status`
				FROM {$wpdb->prefix}learningonline_user_itemmeta uqm
				INNER JOIN {$wpdb->prefix}learningonline_user_items uq ON uq.user_item_id = uqm.learningonline_user_item_id AND uqm.meta_key = %s
				WHERE uq.user_id = %d
	...
	public function count_retaken_quiz( $quiz_id, $course_id = 0, $force = false ) {
		...
		return apply_filters( 'learning_online_user_count_retaken_quiz', $count, $quiz_id, $course_id, $this->id );

		// Get data from cache
		$cached = (array) wp_cache_get( 'user-count-retaken-quiz', 'learningonline' );

		// If no data exists in cache or force to get it from database
		if ( ! array_key_exists( $this->id . '-' . $course_id . '-' . $quiz_id, $cached ) || $force ) {
			$query = $wpdb->prepare( "
				SELECT count(user_item_id) - 1 FROM {$wpdb->prefix}learningonline_user_items
				WHERE user_id = %d
				AND item_id = %d
				AND ref_id = %d
				AND item_type = %s
			...
			wp_cache_set( 'user-count-retaken-quiz', $cached, 'learningonline' );
		} else {
	...
	public function count_retaken_course( $course_id = 0, $force = false ) {
		...
		// Get data from cache
		$cached = (array) wp_cache_get( 'user-count-retaken-course', 'learningonline' );
		...
		if ( ! array_key_exists( $this->id . '-' . $course_id, $cached ) || $force ) {
			...
			wp_cache_set( 'user-count-retaken-course', $cached, 'learningonline' );
		} else {
			$count = $cached[ $this->id . '-' . $course_id ];
		}

		return apply_filters( 'learning_online_user_count_retaken_course', $count, $course_id, $this->id );
	}
	...
	public function retake_course( $course_id ) {
		...
		$check = apply_filters( 'learning_online_before_retake_course', true, $course_id, $this->id );
		if ( ! $check ) {
			return false;
		}
		if ( $wpdb->insert(
			...
			$wpdb->query(
				$wpdb->prepare( "
					DELETE FROM {$wpdb->prefix}learningonline_user_items
					WHERE user_id = %d
					AND ref_id = %d
					AND ref_type = %s
				", $this->id, $course_id, 'lp_course' )
			);
			do_action( 'learning_online_user_retaken_course', $course_id, $this->id, $inserted );

		} else {
			learning_online_debug( $wpdb );
			do_action( 'learning_online_user_retake_course_failed', $course_id, $this->id, $inserted );
		}
	...
	public function complete_lesson( $lesson_id, $course_id = 0 ) {
		global $wpdb;
		do_action( 'learning_online_before_user_complete_lesson', $lesson_id, $this );
		$course_id = $this->_get_course_id( $course_id );
		...
		if ( $item = $this->is_exists_lesson( $lesson_id, $course_id ) ) {
			...
			if ( $item->status !== 'completed' ) {
				...
				$result = new WP_Error( 'lesson-completed', __( 'You have already completed this lesson', 'learningonline' ) );
			}
		} else {
			$wpdb->insert(
				$wpdb->prefix . 'learningonline_user_items',
				array(
					...
					'end_time'   => current_time( 'mysql' ),
					'status'     => 'completed',
					'parent_id'  => learning_online_get_user_item_id( $this->id, $course_id )
				),
		...
		do_action( 'learning_online_user_complete_lesson', $lesson_id, $result, $this->id );
	...
	public function has_completed_lesson( $lesson_id, $course_id = null, $force = false ) {
		...
		if ( ! $cached/* && !array_key_exists( $lesson_id, $lessons )*/ || $force ) {
			global $wpdb;
			$query = $wpdb->prepare( "
				SELECT item_id, status FROM {$wpdb->prefix}learningonline_user_items
				WHERE user_id = %d
					AND item_type = %s
			", $this->id, 'lp_lesson' );
		...
		return apply_filters( 'learning_online_user_has_completed_lesson', $completed, $lesson_id, $this );
	}
	...
	public function get_course_info( $course_id, $field = null, $force = false ) {
		...
		if ( $course_id && ! array_key_exists( $course_id, $user_course_info[ $this->id ] ) || $force ) {
			if ( $result = learning_online_get_user_courses_info( $this->id, array( $course_id ) ) ) {
		...
		return apply_filters( 'learning_online_user_course_info', $info, $this, $course_id );
	}
	...
	public function get_course_status( $course_id ) {
		return apply_filters( 'learning_online_user_course_status', $this->get_course_info( $course_id, 'status' ), $this->id );
	}
	...
	public function evaluate_quiz_results( $quiz_id, $progress ) {
		...
		return apply_filters( 'learning_online_evaluate_quiz_results', $results, $quiz_id, $this->id );
	}
	...
	private function _calculate_quiz_time( $quiz, $progress ) {
		...
		if ( $end < $start ) {
			...
			//learning_online_update_user_quiz_meta( $progress->history_id, 'end', $end );
		}
	...
	public function has_purchased_course( $course_id ) {
		...
		return apply_filters( 'learning_online_user_has_purchased_course', $this->get_order_status( $course_id ) == 'lp-completed', $course_id, $this->id );
	}
	...
	public function has_ordered_course( $course_id ) {
		return apply_filters( 'learning_online_user_has_ordered_course', $this->get_course_order( $course_id ), $course_id, $this->id );
	}
	...
	public function get_order_status( $course_id ) {
		...
		return apply_filters( 'learning_online_user_has_ordered_course', $order ? $order->post_status : false, $course_id, $this->id );
	}
	...
	public function has_completed_item( $item, $course_id = 0, $force = false ) {
		...
		return apply_filters( 'learning_online_user_has_completed_item', $return, $item );
	}
		...
		if ( $course->id ) {
			...
			if ( $course_info ) {
				...
				if ( $start_time + $course_duration > $now ) {
					...
					$remain = learning_online_seconds_to_weeks( $remain );
				}
			}
		}
	...
	private function _parse_item_order_of_course( $course_id ) {
		...
		if ( $items ) {
			...
			$query = $wpdb->prepare( "
				...
				FROM {$wpdb->posts} o
				INNER JOIN {$wpdb->postmeta} om ON om.post_id = o.ID AND om.meta_key = %s AND om.meta_value = %d
				INNER JOIN {$wpdb->learningonline_order_items} oi ON o.ID = oi.order_ID
				INNER JOIN {$wpdb->learningonline_order_itemmeta} oim ON oim.learningonline_order_item_id= oi.order_item_id AND oim.meta_key = %s
				INNER JOIN {$wpdb->posts} c ON c.ID = oim.meta_value
				INNER JOIN {$wpdb->learningonline_sections} s ON s.section_course_id = c.ID
				INNER JOIN {$wpdb->learningonline_section_items} si ON si.section_id = s.section_id WHERE si.item_id IN (" . join( ',', $items ) . ")
				AND o.post_status = %s
	...
	public function enroll( $course_id, $ref_id = 0 ) {
		...
		$check = apply_filters( 'learning_online_before_enroll_course', true, $this, $course_id );
		if ( ! $check ) {
			return false;
		}
		...
		if ( $course->is_free() ) {
			# 1 create order
			$order_data = array(
				'status'      => apply_filters( 'learning_online_default_enroll_order_status', 'completed' ),
			...
			$order_item = learning_online_add_order_item( $order->id, $item );
			$ref_id     = $order->id;

			# 3 add order itemmeta
			learning_online_add_order_item_meta( $order_item, '_course_id', $course->id );
			learning_online_add_order_item_meta( $order_item, '_quantity', 1 );
			learning_online_add_order_item_meta( $order_item, '_subtotal', 0 );
			learning_online_add_order_item_meta( $order_item, '_total', 0 );
		...
		if ( $wpdb->insert(
			...
			do_action( 'learning_online_user_enrolled_course', $course_id, $this->id, $inserted );

		} else {
			learning_online_debug( $wpdb );
			do_action( 'learning_online_user_enroll_course_failed', $this, $course_id, $inserted );
		}
	...
	public function get_questions( $args = array() ) {
		...
		return apply_filters( 'learning_online_user_questions', $questions[ $key ], $this );
	}
		...
		if ( empty( $courses[ $key ] ) ) {
			...
			$query = $wpdb->prepare( "
				SELECT SQL_CALC_FOUND_ROWS * FROM(
					...
					FROM {$wpdb->posts} c
					LEFT JOIN {$wpdb->prefix}learningonline_user_items uc ON c.ID = uc.item_id AND uc.user_id = %d
					WHERE post_type = %s
					...
					SELECT c.*, uc.status as course_status
					FROM {$wpdb->posts} c
					INNER JOIN {$wpdb->prefix}learningonline_user_items uc ON c.ID = uc.item_id
					WHERE uc.user_id = %d
						AND c.post_type = %s
						AND c.post_status = %s
				) a GROUP BY a.ID
			...
			if ( $data['rows'] ) {
				...
				learning_online_setup_user_course_data( $this->id, $course_ids );
				learning_online_get_user_courses_info( $this->id, $course_ids );
				_learning_online_count_users_enrolled_courses( $course_ids );
	...
	public function _get_orders() {
		...
		return apply_filters( 'learning_online_user_orders', $orders, $this->id );
	}
	...
	public function get_quiz_by_question( $question_id ) {
		...
		$query = $wpdb->prepare( "
			SELECT quiz_id
			FROM {$wpdb->prefix}learningonline_user_items uq
			INNER JOIN {$wpdb->prefix}learningonline_user_itemmeta uqm ON uqm.learningonline_user_item_id = uq.user_item_id AND uqm.meta_key = %s AND uqm.meta_value LIKE %s
	...
	public function get_purchased_courses( $args = array() ) {
		...
		if ( empty( $courses[ $key ] ) ) {
			...
			$query = $wpdb->prepare( "
				...
				FROM {$wpdb->posts} o
				INNER JOIN {$wpdb->prefix}learningonline_order_items oi ON oi.order_id = o.ID
				INNER JOIN {$wpdb->prefix}learningonline_order_itemmeta oim ON oim.learningonline_order_item_id = oi.order_item_id AND oim.meta_key = %s
				INNER JOIN {$wpdb->posts} c ON c.ID = oim.meta_value
				INNER JOIN {$wpdb->postmeta} om ON om.post_id = o.ID AND om.meta_key = %s
	...
	public function has_checked_answer( $question_id, $quiz_id, $course_id = 0 ) {
		...
		$checked = (array) learning_online_get_user_item_meta( $history->history_id, 'question_checked', true );
	...
	public function get_quiz_graduation( $quiz_id, $course_id = 0, $check_completed = true ) {
		...
		return apply_filters( 'learning_online_user_quiz_graduation', $grade, $quiz_id, $course_id );
	}
	...
	public function get_upload_profile_src( $size = '' ) {
		if ( empty( $this->uploaded_profile_src ) ) {
			if ( $profile_picture = $this->profile_picture ) {
				$upload    = learning_online_user_profile_picture_upload_dir();
	...
	public function get_profile_picture( $type = '', $size = 96 ) {
		if ( $type == 'gravatar' ) {
			remove_filter( 'pre_get_avatar', 'learning_online_pre_get_avatar_callback', 1, 5 );
		}
		...
		if ( $type == 'gravatar' ) {
			add_filter( 'pre_get_avatar', 'learning_online_pre_get_avatar_callback', 1, 5 );
		}
	...
	public function can_access_course( $course_id ) {
		return apply_filters( 'learning_online_user_can_access_course', $this->get_order_status( $course_id ) == 'lp-completed', $course_id, $this->id );
	}
	...
	public function can_do_quiz( $quiz_id, $course_id = 0 ) {
		...
		return apply_filters( 'learning_online_user_can_do_quiz', $can, $quiz_id, $this->id, $course_id );
	}
	...
	public function get_course_grade( $course_id ) {
		...
		return apply_filters( 'learning_online_user_course_grade', $grade, $this->id, $course_id );
	}
