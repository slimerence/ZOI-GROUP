...
class LP_Order {
	...
	public function update_status( $new_status = 'pending', $force = false ) {
		...
		if ( ( ( $new_status !== $old_status ) || $force ) && in_array( $new_status, array_keys( learning_online_get_order_statuses( false ) ) ) ) {
			// Update the order
			global $wpdb;
			$updated = $wpdb->update( $wpdb->posts, array( 'post_status' => 'lp-' . $new_status ), array( 'ID' => $this->id ), array( '%s' ) );

			if ( $updated === false ) {
				throw new Exception( __( 'Error! Update order failed', 'learningonline' ) );
				return false;
			}
			...
			// Status was changed
			do_action( 'learning_online_order_status_' . $new_status, $this->id );
			do_action( 'learning_online_order_status_' . $old_status . '_to_' . $new_status, $this->id );
			do_action( 'learning_online_order_status_changed', $this->id, $old_status, $new_status );
			...
			// backward compatible
			do_action( 'learning_online_update_order_status', $new_status, $this->id );
		}
	}
	...
	public function get_order_number() {
		return apply_filters( 'learning_online_get_order_number', '#' . sprintf( "%'.010d", $this->id ), $this );
	}
	...
	public function get_order_status() {
		...
		return apply_filters( 'learning_online_get_order_status', $status, $this );
	}
	...
	public function get_order_status_html() {
		...
		if ( !empty( $statuses[$this->post_status] ) ) {
			...
			$status = __( 'Removed', 'learningonline' );
		} else {
			$status = ucfirst( $this->post_status );
		}
		$class = 'order-status order-status-' . sanitize_title( $status );
		$html  = sprintf( '<span class="%s">%s</span>', apply_filters( 'learning_online_order_status_class', $class, $status, $this ), $status, $this );
		return apply_filters( 'learning_online_order_status_html', $html, $this );
	}
	...
	public function payment_complete( $transaction_id = '' ) {
		do_action( 'learning_online_pre_payment_complete', $this->id );

		LP()->session->order_awaiting_payment = null;

		$valid_order_statuses = apply_filters( 'learning_online_valid_order_statuses_for_payment_complete', array( 'pending', 'processing', 'on-hold' ), $this );
		...
		if ( $this->id && $this->has_status( $valid_order_statuses ) ) {
			...
			do_action( 'learning_online_payment_complete', $this->id );
		} else {
			do_action( 'learning_online_payment_complete_order_status_' . $this->get_status(), $this->id );
		}
	...
	public function get_checkout_order_received_url() {
		$received_url = learning_online_get_endpoint_url( 'lp-order-received', $this->id, learning_online_get_page_link( 'checkout' ) );

		$received_url = add_query_arg( 'key', $this->order_key, $received_url );

		return apply_filters( 'learning_online_get_checkout_order_received_url', $received_url, $this );
	}
	...
	public function get_customer_name() {
		...
		$customer_name = '';
		if ( !$user->is_exists() ) {
			$customer_name = apply_filters( 'learning_online_order_customer_name', __( '[Guest]', 'learningonline' ) );
		} else {
	...
	public function get_items() {
		...
		$query = $wpdb->prepare( "
			...
			FROM {$wpdb->learningonline_order_items}
		...
		return apply_filters( 'learning_online_order_get_items', $items, $this );
	}

	public function get_item_meta( &$item ) {
		if ( $metas = get_metadata( 'learningonline_order_item', $item['id'] ) ) {
	...
	public function remove_order_items() {
		...
		$wpdb->query(
			$wpdb->prepare( "
				...
				USING {$wpdb->learningonline_order_itemmeta} itemmeta
				INNER JOIN {$wpdb->learningonline_order_items} items
				WHERE itemmeta.learningonline_order_item_id = items.order_item_id
		...
		$wpdb->query(
			$wpdb->prepare( "
				DELETE FROM {$wpdb->learningonline_order_items}
				WHERE order_id = %d",
				$this->id
			)
		);
		$wpdb->query( $wpdb->prepare( "ALTER TABLE {$wpdb->learningonline_order_itemmeta} AUTO_INCREMENT = %d", 1 ) );
		$wpdb->query( $wpdb->prepare( "ALTER TABLE {$wpdb->learningonline_order_items} AUTO_INCREMENT = %d", 1 ) );
	}
	...
	public function add_item( $item, $quantity = 1, $meta = array() ) {
		...
		$item_id = learning_online_add_order_item( $this->id, $item );
		if ( !$item_id ) {
			return false;
		}
		learning_online_add_order_item_meta( $item_id, '_course_id', $item['item_id'] );
		...
		learning_online_add_order_item_meta( $item_id, '_data', $item['data'] );

		return $item_id;
	}

	public function get_user( $field = '' ) {
		$user = learning_online_get_user( $this->user_id );
		if ( $field ) {
			return $user->{$field};
		}
		return $user;
	...
	public function get_formatted_order_subtotal() {
		$currency_symbol = learning_online_get_currency_symbol( $this->order_currency );
		return learning_online_format_price( $this->order_subtotal, $currency_symbol );
	}

	public function get_formatted_order_total() {
		$currency_symbol = learning_online_get_currency_symbol( $this->order_currency );
		return learning_online_format_price( $this->order_total, $currency_symbol );
	}
	...
	public function get_payment_method_title() {
		...
		return apply_filters( 'learning_online_display_payment_method_title', $title, $this->payment_method );
	}

	public function get_view_order_url() {
		global $wp_query;
		$view_order_url = learning_online_get_endpoint_url( 'view-order', $this->id, learning_online_get_page_link( 'profile' ) );
		//
		...
		if ( get_option( 'permalink_structure' ) ) {
			$view_order_url = learning_online_get_page_link( 'profile' ) . $user->user_login . '/' . $view_order_endpoint . '/' . $this->id . '/';
		} else {
			$args         = array(
			...
			$view_order_url = add_query_arg(
				$args,
				learning_online_get_page_link( 'profile' )
			);
		}

		return apply_filters( 'learning_online_view_order_url', $view_order_url, $this );
	}
	...
	public function get_cancel_order_url( $force = false ) {
		...
		return apply_filters( 'learning_online_cancel_order_url', wp_nonce_url( $url, 'cancel-order', 'lp-nonce' ) );
	}
	...
	public function add_note( $note = null ) {
		if ( is_user_logged_in() ) {
			...
			$comment_agent        = 'LearningOnline';
			$comment_type         = 'lp_order_note';
			$comment_parent       = 0;
			$comment_approved     = 1;

			$commentdata = apply_filters(
				'learning_online_new_order_note_data',
	...
	public function get_user_name() {
		return apply_filters( 'learning_online_order_user_name', sprintf( _x( '%1$s', 'full name', 'learningonline' ), $this->get_user( 'user_login' ) ) );
	}
	...
	public function is_multi_users() {
		$multiple = $this->get_status() == 'auto-draft' && 'yes' == learning_online_get_request( 'multi-users' );
	...
	public function print_users() {
		if ( $user_ids = get_post_meta( $this->id, '_user_id' ) ) {
			...
			_e( 'No user assigned', 'learningonline' );
		}
	}
	...
	public function get_user_email() {
		$email = false;
		if ( $user = learning_online_get_user( $this->user_id ) ) {
