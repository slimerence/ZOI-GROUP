<?php
/**
 * Define base class of LearningOnline widgets and helper functions
 */
...
if ( !class_exists( 'LP_Widget' ) ) {
	...
	class LP_Widget extends WP_Widget {
		 ...
		 * @var string
		 */
		private $_name_prefix = 'LearningOnline - ';
		...
		public function widget( $args, $instance ) {
			...
			if ( !apply_filters( 'learning_online_widget_display_content', true, $this ) ) {
				return;
			}

			if ( !apply_filters( 'learning_online_widget_display_content-' . $this->id_base, true, $this ) ) {
				return;
			}
			$this->before_widget();
			$this->show();
		...
		public function show() {
			printf( __( 'Function %s should be overwritten in child class', 'learningonline' ), __FUNCTION__ );
		}
...
//            var_dump($this->instance);
		...
		public function get_template() {
			learning_online_get_widget_template( $this->get_slug(), 'default.php' );
		}
 ...
 * @return string
 */
function learning_online_get_widget_template_path( $slug ) {
	return LP_WIDGET_PATH . "/{$slug}/tmpl/";
}

function learning_online_get_widget_theme_template_path( $slug ) {
 ...
 * @param array $args
 */
function learning_online_get_widget_template( $slug, $template_name = 'default.php', $args = array() ) {
	//$template_path = learning_online_get_widget_template_path( $slug );
	$template = "widgets/{$slug}/" . ( $template_name ? $template_name : 'default.php' );
	die();
	learning_online_get_template( $template );// $template_name ? $template_name : 'default.php', $args, learning_online_template_path() . "/widgets/{$slug}", LP_PLUGIN_PATH . "/widgets/{$slug}" );
}
 ...
 * @return string
 */
function learning_online_locate_widget_template( $slug, $template_name = 'default.php' ) {
	//$template_path = learning_online_get_widget_template_path( $slug );
	$template = "widgets/{$slug}/" . ( $template_name ? $template_name : 'default.php' );
	return learning_online_locate_template( $template );
}