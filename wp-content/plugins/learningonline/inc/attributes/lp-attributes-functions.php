 ...
 */
function learning_online_get_attributes() {
 ...
 * @return array|int|WP_Error
 */
function learning_online_get_attribute_terms( $attribute ) {
	$terms = array();
	if ( is_array( $attribute ) ) {
		foreach ( $attribute as $attr ) {
			$_terms = learning_online_get_attribute_terms( $attr );
			if ( is_array( $terms ) ) {
 ...
 * @return array|bool
 */
function learning_online_delete_attribute_terms( $attribute ) {
	$deleted_terms = array();
	if ( !$terms = learning_online_get_attribute_terms( $attribute ) ) {
		return false;
	}
	...
	do_action( 'learning_online_deleted_attribute_terms', $attribute, $deleted_terms );
 ...
 * @return mixed
 */
function learning_online_get_course_attributes( $course_id ) {
 ...
 */
function learning_online_add_course_attribute_value( $name, $taxonomy ) {
 ...
 * @return array|bool|WP_Error
 */
function learning_online_add_course_attribute( $name ) {
 ...
 */
function learning_online_add_attribute_to_course( $course_id, $taxonomy ) {
	if ( !$term = get_term_by( 'slug', $taxonomy, LP_COURSE_ATTRIBUTE ) ) {
		return false;
	}
	...
	$attribute               = apply_filters( 'learning_online_update_course_attribute_data', array(
		'name' => $term->slug
	), $course_id, $term );
	$attributes[$term->slug] = $attribute;

	$attributes = apply_filters( 'learning_online_update_course_attributes_data', $attributes, $course_id, $term );
...
add_filter( 'pre_get_posts', function ( $q ) {
	...
	if ( $attribute_taxonomies = learning_online_get_attributes() ) {
		$attribute_operator = 'and' === strtolower( learning_online_get_request( 'attribute_operator' ) ) ? 'AND' : 'OR';
		$value_operator     = 'and' === strtolower( learning_online_get_request( 'value_operator' ) ) ? 'AND' : 'IN';
		$tax_query          = array(
