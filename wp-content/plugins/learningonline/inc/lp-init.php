...
add_action( 'init', '_learning_online_upgrade_table' );
/**
 * Add column parent_id into user_tables if it does not exists
 *
 * TODO: remove in next version
 */
function _learning_online_upgrade_table() {
	if ( version_compare( LEARNINGONLINE_VERSION, '2.1.0', '<' ) ) {
		global $wpdb;
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}learningonline_user_items'" ) != $wpdb->prefix . "learningonline_user_items" ) {
			return;
		}

		$query = "SHOW COLUMNS FROM {$wpdb->prefix}learningonline_user_items LIKE 'parent_id'";
		if ( $row = $wpdb->get_var( $query ) ) {
			update_option( 'learning_online_upgrade_table_20', 'yes' );
			return;
		}

		$query = "ALTER TABLE {$wpdb->prefix}learningonline_user_items ADD COLUMN `parent_id` BIGINT(20) UNSIGNED NOT NULL DEFAULT 0 AFTER `ref_type`;";
		if ( $wpdb->query( $query ) ) {
			update_option( 'learning_online_upgrade_table_20', 'yes' );
		}
	}
...
add_action( 'learning_online_parse_query', '_learning_online_setup_user_course_data' );
function _learning_online_setup_user_course_data( $query ) {
	learning_online_setup_user_course_data( get_current_user_id(), $query->query_vars['course_id'] );
}

/**
 * Cache static pages
 */
function learning_online_setup_pages() {
	global $wpdb;
	static $pages = false;
	if ( $pages == false ) {
		...
		foreach ( $pages as $page ) {
			$id = get_option( 'learning_online_' . $page . '_page_id' );
			if ( $id ) {
				$page_ids[] = $id;
			}
		}
 ...
 * @return bool|int
 */
function learning_online_setup_course_data( $the_course ) {
	global $wp_query;
	$course = false;
	$post   = false;
	if ( is_numeric( $the_course ) ) {
		...
		if ( !$post ) {
			$post = learning_online_get_post_by_name( $the_course, 'lp_course' );
		}
	}
	...
	_learning_online_get_course_curriculum( $post->ID );
	do_action( 'learning_online_setup_course_data_' . $post->ID );

	return $post->ID;
}

function _learning_online_count_users_enrolled_courses( $course_ids ) {
	global $wpdb;
	...
	if ( $course_ids ) {
		...
		$query    = $wpdb->prepare( "
			...
			FROM {$wpdb->posts} o
			INNER JOIN {$wpdb->learningonline_order_items} oi ON oi.order_id = o.ID
			INNER JOIN {$wpdb->learningonline_order_itemmeta} oim ON oim.learningonline_order_item_id = oi.order_item_id
			AND oim.meta_key = %s
...
function _learning_online_get_courses_curriculum( $course_ids, $force = false, $parse_items = true ) {
	global $wpdb;
	...
	$query = $wpdb->prepare( "
		...
		INNER JOIN {$wpdb->prefix}learningonline_section_items si ON si.item_id = p.ID
		INNER JOIN {$wpdb->prefix}learningonline_sections s ON s.section_id = si.section_id
		WHERE s.section_id IN(
			SELECT cc.section_id
				FROM {$wpdb->prefix}posts p
				INNER JOIN {$wpdb->prefix}learningonline_sections cc ON p.ID = cc.section_course_id
		...
		foreach ( $course_ids as $course_id ) {
			...
			if ( $quiz_ids ) {
				...
				if ( $fetched_posts ) {
					...
					if ( $quiz_ids ) {
						$question_ids = _learning_online_get_quiz_questions( $quiz_ids );
						if ( $question_ids ) {
 ...
 * @return array
 */
function _learning_online_get_course_curriculum( $course_id, $force = false ) {
	//return learning_online_get_course_curriculumx( $course_id, $force );
	$curriculum = LP_Cache::get_course_curriculum( $course_id );
	if ( ( $curriculum == false ) || $force ) {
		$curriculum = _learning_online_get_courses_curriculum( array( $course_id ), $force );
 ...
 * @return array
 */
function _learning_online_get_quiz_questions( $quiz_ids ) {
	global $wpdb;
	...
	$query          = $wpdb->prepare( "
		...
		FROM {$wpdb->prefix}learningonline_quiz_questions qq
	...
	if ( $questions = $wpdb->get_results( $query, OBJECT_K ) ) {
		...
		$query        = $wpdb->prepare( "
					...
					INNER JOIN {$wpdb->prefix}learningonline_quiz_questions qq ON qq.question_id = p.ID
					RIGHT JOIN {$wpdb->prefix}learningonline_question_answers qa ON qa.question_id = p.ID
 ...
 * @param $course_id
 */
function learning_online_setup_user_course_data( $user_id, $course_id, $force = false ) {
	if ( is_array( $course_id ) ) {
		_learning_online_get_courses_curriculum( $course_id );
		foreach ( $course_id as $cid ) {
			learning_online_setup_user_course_data( $user_id, $cid, $force );
		}
		return;
	}
	if ( !did_action( 'learning_online_setup_course_data_' . $course_id ) ) {
		learning_online_setup_course_data( $course_id );
	}
	 ...
	 * Get user orders
	 */
	_learning_online_get_user_course_orders();
	_learning_online_parse_user_item_statuses( $user_id, $course_id );
	global $lp_query;
	if ( !empty( $lp_query->query_vars['lesson'] ) && !empty( $item_statuses[$user_id . '-' . $course_id . '-' . $course_id] ) && $item_statuses[$user_id . '-' . $course_id . '-' . $course_id] != 'finished' ) {
		$user_item_id = learning_online_get_user_item_id( $user_id, $course_id );
		$lesson       = learning_online_get_post_by_name( $lp_query->query_vars['lesson'], LP_LESSON_CPT );
		if ( empty( $item_statuses[$user_id . '-' . $course_id . '-' . $lesson->ID] ) ) {
			learning_online_update_user_item_field( array(
				'user_id'    => $user_id,
				'item_id'    => $lesson->ID,
...
function _learning_online_parse_user_item_statuses( $user_id, $course_id, $force = false ) {
	if ( did_action( "learning_online_parse_user_item_statuses_{$user_id}_{$course_id}" ) && !$force ) {
		return;
	}
	global $wpdb;
	if ( !$course_id ) {
	...
	if ( $item_ids ) {
		...
		$query = $wpdb->prepare( "
			...
			FROM (
				SELECT * FROM {$wpdb->prefix}learningonline_user_items t1
				WHERE user_id = %d
				AND item_id = %d
				UNION
				(
					SELECT * FROM {$wpdb->prefix}learningonline_user_items t2
					WHERE user_id = %d AND ref_id = %d
					AND user_item_id = (
						SELECT MAX(user_item_id) FROM {$wpdb->prefix}learningonline_user_items
						WHERE user_id = %d and ref_id = %d
			...
			LEFT JOIN {$wpdb->prefix}learningonline_user_itemmeta ui ON ui.learningonline_user_item_id = X.user_item_id AND ui.meta_key = %s
			ORDER BY user_item_id ASC
		", $user_id, $course_id, $user_id, $course_id, $user_id, $course_id, '_quiz_grade' );
	} else {
		$query = $wpdb->prepare( "
			SELECT ui.*, uim.meta_value as grade FROM {$wpdb->prefix}learningonline_user_items ui
			LEFT JOIN {$wpdb->prefix}learningonline_user_itemmeta uim ON uim.learningonline_user_item_id = ui.user_item_id AND uim.meta_key = %s
			WHERE user_id = %d
			AND item_id = %d
	...
	if ( $items ) {
		foreach ( $items as $item ) {
			$item_statuses[$user_id . '-' . $course_id . '-' . $item->item_id] = learning_online_validate_item_status( $item );
			if ( !empty($item->grade) ) {
	...
	do_action( "learning_online_parse_user_item_statuses", $user_id, $course_id );
	do_action( "learning_online_parse_user_item_statuses_{$user_id}_{$course_id}" );
}

function learning_online_validate_item_status( $item ) {
	...
	if ( $item->item_type == LP_QUIZ_CPT && $item->status == 'completed' && is_null( $item->grade ) ) {
		$user  = learning_online_get_user( $item->user_id );
		...
		learning_online_update_user_item_meta( $item->user_item_id, '_quiz_grade', $grade );
	}
 ...
 * @return array
 */
function _learning_online_get_user_course_orders( $user_id = 0, $force = false ) {
	global $wpdb;
	if ( !$user_id ) {
	...
	if ( !array_key_exists( $user_id, $data ) || $force ) {
		...
		$query   = $wpdb->prepare( "
			...
			FROM {$wpdb->prefix}learningonline_order_items oi
			INNER JOIN {$wpdb->prefix}learningonline_order_itemmeta oim ON oim.learningonline_order_item_id = oi.order_item_id AND meta_key = %s
...
function _learning_online_get_user_profile_orders( $user_id = 0, $paged = 1, $limit = 10 ) {
	global $wpdb;
	if ( !$user_id ) {
		$user_id = learning_online_get_current_user_id();
	}
	...
	if ( !array_key_exists( $user_id, $data ) ) {
		...
		$results       = array();
		$statuses      = learning_online_get_order_statuses( true, true );
 ...
 * @return array
 */
function _learning_online_setup_question( $id ) {
	global $wpdb;
	settype( $id, 'array' );
	$query     = $wpdb->prepare( "
...
learning_online_setup_pages();

#
# stop support comment for 
#
//add_action( 'init', 'learning_online_remove_course_comment' );
//
//function learning_online_remove_course_comment() {
