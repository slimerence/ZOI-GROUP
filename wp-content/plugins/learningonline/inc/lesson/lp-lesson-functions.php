...
add_action( 'comment_form', 'learning_online_lesson_comment_form_fields' );
function learning_online_lesson_comment_form_fields( $post_id ) {
	if ( empty( $_REQUEST['content-item-only'] ) ) {
		return;
	}
	?>
	<input type="hidden" name="content-item-only-redirect" value="<?php echo learning_online_get_current_url(); ?>" />
	<input type="hidden" name="content-item-only" value="yes" />
	<?php
}

add_filter( 'comment_post_redirect', 'learning_online_get_only_content_permalink', 10, 2 );

if ( !function_exists( 'learning_online_get_only_content_permalink' ) ) {
	function learning_online_get_only_content_permalink( $redirect, $comment ) {
...
function learning_online_lesson_comment_form( $lesson_id, $course_id, $deprecated = null ) {
	global $post;
 ...
 */
add_action( 'delete_post', 'learning_online_lesson_before_delete_post', 10, 2 );
function learning_online_lesson_before_delete_post( $post_id, $force=false ) {
	global $wpdb;
	if( 'lp_lesson' === get_post_type( $post_id ) ) {
		$sql = 'DELETE FROM `'.$wpdb->prefix.'learningonline_section_items` WHERE `item_id` = '.$post_id.' AND `item_type` = "lp_lesson"';
		$wpdb->query($sql);
	}
}
