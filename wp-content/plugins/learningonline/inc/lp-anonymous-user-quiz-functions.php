...
class LP_Anonymous_User_Quiz {
	function __construct() {
		add_action( 'learning_online_after_reset_user_quiz', array( $this, 'after_reset_user_quiz' ), 99, 2 );
		add_filter( 'learning_online_get_question_answers', array( $this, 'anonymous_get_question_answers' ), 99, 3 );
		add_filter( 'learning_online_get_question_position', array( $this, 'anonymous_get_question_position' ), 99, 4 );
		add_action( 'learning_online_before_nav_question_form', array( $this, 'before_nav_question_form' ), 99, 2 );
		add_filter( 'learning_online_before_user_start_quiz', array( $this, 'do_quiz_for_anonymous_user' ), 999, 3 );
		add_filter( 'learning_online_user_has_completed_quiz', array( $this, 'user_has_completed_quiz' ), 999, 3 );

		add_filter( 'learning_online_user_has_started_quiz', array( $this, 'user_has_started_quiz' ), 999, 3 );
		add_action( 'learning_online_submit_answer', array( $this, 'submit_answer' ), 999, 5 );
		add_filter( 'learning_online_anonymous_user_can_retake_quiz', array( $this, 'user_can_retake_quiz' ), 999, 2 );
		add_action( 'learning_online_content_quiz_after_title_element', array( $this, '_debug' ) );
		add_filter( 'learning_online_user_has_passed_course', array( $this, 'user_has_passed_course' ), 999, 3 );
		add_filter( 'learning_online_get_user_quiz_time', array( $this, 'user_quiz_time' ), 999, 3 );

		add_filter( 'learning_online_get_user_quiz_questions', array( $this, 'user_quiz_questions' ), 10, 3 );
		add_filter( 'learning_online_user_quiz_start_time', array( $this, 'quiz_start_time' ), 10, 3 );
		//add_filter( 'learning_online_reset_user_quiz', array( $this, 'reset_user_quiz'), 999, 3 );

	}

	function quiz_start_time( $time, $quiz_id, $user_id ) {
		$course_id = learning_online_get_course_by_quiz( $quiz_id );
	...
	function user_quiz_time( $time, $quiz_id, $user_id ) {
		if ( $this->is_public_quiz( learning_online_get_course_by_quiz( $quiz_id ) ) ) {
	...
	function user_has_passed_course( $user_passed, $course_id, $user_id ) {
		if ( $this->is_public_quiz( $course_id ) ) {
			if ( ( get_post_meta( $course_id, '_lpr_course_final', true ) == 'yes' ) && ( $quiz = lpr_get_final_quiz( $course_id ) ) ) {
				$passed            = learning_online_quiz_evaluation( $quiz, $user_id );
				$passing_condition = learning_online_get_course_passing_condition( $course_id );
	...
	function is_public_quiz( $course_id ) {
		return ( false == learning_online_course_enroll_required( $course_id ) && !is_user_logged_in() );
	}
	...
	function do_quiz_for_anonymous_user( $continue, $quiz_id, $user_id ) {
		...
		if ( $this->is_public_quiz( $course_id ) ) {
			...
			$session->set(
				'anonymous_quiz',
				array(
					'questions' => array_values( learning_online_get_quiz_questions( $quiz_id ) ),
					'finished'  => 0,
					'answers'   => array(),
					'start'     => time(),
					'end'       => null
			...
			$continue = false;
			do_action( 'learning_online_user_start_quiz', $quiz_id, $user_id );
		}
		return $continue;
	}
