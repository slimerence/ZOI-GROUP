...
class LP_Page_Controller {
	...
	public function __construct() {
		...
		//add_action( 'learning_online_before_template_part', array( $this, 'before_template_part' ), 10, 4 );
		add_shortcode( 'learning_online_archive_course', array( $this, 'archive_content' ) );
	...
	public function template_loader( $template ) {
		...
		$file           = '';
		$find           = array();
		$theme_template = learning_online_template_path();
		...
		if ( is_post_type_archive( 'lp_course' ) ) {
			if ( ( $page_id = learning_online_get_page_id( 'courses' ) ) && ( empty( $wp_query->queried_object_id ) || ! empty( $wp_query->queried_object_id ) && $page_id != $wp_query->queried_object_id ) ) {
				$redirect = trailingslashit( learning_online_get_page_link( 'courses' ) );
				...
				if ( $page_id != get_option( 'page_on_front' ) && ! learning_online_is_current_url( $redirect ) ) {
		...
		if ( learning_online_is_profile() ) {
			$current_tab = learning_online_get_current_profile_tab( false );
			if ( $current_tab && ! learning_online_profile_tab_exists( $current_tab ) ) {
				global $wp;
				if ( empty( $wp->query_vars['view'] ) ) {
					wp_redirect( learning_online_get_page_link( 'profile' ) );
					exit();
				}
			}
		}
		$queried_object_id = ! empty( $wp_query->queried_object_id ) ? $wp_query->queried_object_id : 0;
		if ( ( $page_id = learning_online_get_page_id( 'taken_course_confirm' ) ) && is_page( $page_id ) && $page_id == $queried_object_id ) {
			if ( ! learning_online_user_can_view_order( ! empty( $_REQUEST['order_id'] ) ? $_REQUEST['order_id'] : 0 ) ) {
				learning_online_is_404();
			}
			$post->post_content = '[learning_online_confirm_order]';
		} elseif ( ( $page_id = learning_online_get_page_id( 'become_a_teacher' ) ) && is_page( $page_id ) && $page_id == $queried_object_id ) {
			$post->post_content = '[learning_online_become_teacher_form]';
		} else {
			if ( learning_online_is_courses() || learning_online_is_course_tag() || learning_online_is_course_category() || learning_online_is_search() ) {
		...
		if ( $file ) {
			...
			if ( ! $template && ! in_array( $file, array( 'single-course.php', 'archive-course.php' ) ) ) {
				$template = learning_online_plugin_path( 'templates/' ) . $file;
			}
		}
		if ( ! $template ) {
			...
			} elseif ( learning_online_is_courses() || learning_online_is_course_tag() || learning_online_is_course_category() || learning_online_is_search() ) {
	...
	public function archive_content() {
		ob_start();
		learning_online_get_template( 'content-archive-course.php' );
	...
	public function template_loader2( $template ) {
		define( 'LEARNINGONLINE_IS_COURSES', learning_online_is_courses() );
		define( 'LEARNINGONLINE_IS_TAG', learning_online_is_course_tag() );
		define( 'LEARNINGONLINE_IS_CATEGORY', learning_online_is_course_category() );
		define( 'LEARNINGONLINE_IS_TAX', is_tax( get_object_taxonomies( 'lp_course' ) ) );
		define( 'LEARNINGONLINE_IS_SEARCH', learning_online_is_search() );
		if ( LEARNINGONLINE_IS_COURSES || LEARNINGONLINE_IS_TAG || LEARNINGONLINE_IS_CATEGORY || LEARNINGONLINE_IS_SEARCH || LEARNINGONLINE_IS_TAX ) {
			 ...
			 * single page.
			 */
			if ( ( $course_page_id = learning_online_get_page_id( 'courses' ) ) && ( $course_page_slug = get_post_field( 'post_name', $course_page_id ) ) ) {
			...
			if ( ! preg_match( '/\[learning_online_archive_course\s?(.*)\]/', $content ) ) {
				$content = $content . '[learning_online_archive_course]';//$this->archive_content();
			}
			...
			if ( learning_online_is_course_category() ) {
				$wp_query->post->post_title = single_term_title( '', false );//__( 'Course Category', 'learningonline' );
			}
	...
	public function single_content( $content ) {
		...
		ob_start();
		learning_online_get_template( 'content-single-course.php' );
	...
	public function pre_get_posts( $q ) {
		...
		if ( $q->get( 'post_type' ) == 'lp_course' && is_single() ) {
			...
			if ( ! empty( $q->query_vars['p'] ) && LP_COURSE_CPT == get_post_type( $q->query_vars['p'] ) ) {
				...
				$post        = learning_online_get_post_by_name( $course_name, 'lp_course', true );
			}
			...
			if ( $course ) {
				...
				if ( $item_name = $q->get( 'lesson' ) ) {
					$item = learning_online_get_post_by_name( $item_name, 'lp_lesson', true );
					if ( $item ) {
						$item_object = LP_Lesson::get_lesson( $item->ID );
					}
				} elseif ( $item_name = $q->get( 'quiz' ) ) {
					$item = learning_online_get_post_by_name( $item_name, 'lp_quiz', true );
					if ( $item ) {
						$quiz = LP_Quiz::get_quiz( $item->ID );
						if ( $question_name = $q->get( 'question' ) ) {
							$question = learning_online_get_post_by_name( $question_name, 'lp_question', true );
							if ( ! $question ) {
								LP_Debug::instance()->add( sprintf( '%s: File %s, line #%d', '404', __FILE__, __LINE__ ) );
								learning_online_is_404();
		 ...
		 */
		if ( $q->is_main_query() && $q->is_page() && ( $q->queried_object_id == ( $page_id = learning_online_get_page_id( 'courses' ) ) && $page_id ) ) {
		...
		if ( $q->is_home() && 'page' == get_option( 'show_on_front' ) && get_option( 'page_on_front' ) == learning_online_get_page_id( 'courses' ) ) {
		 ...
		 */
		if ( $q->is_page() && 'page' == get_option( 'show_on_front' ) && $q->get( 'page_id' ) == learning_online_get_page_id( 'courses' ) && learning_online_get_page_id( 'courses' ) ) {
			...
			$course_page                            = get_post( learning_online_get_page_id( 'courses' ) );
		...
		if ( ( learning_online_is_courses() || learning_online_is_course_category() ) ) {
