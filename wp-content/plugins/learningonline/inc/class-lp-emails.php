...
class LP_Emails {
	...
	public function __clone () {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'learningonline' ), '1.0' );
	}
	...
	public function __construct () {
		if ( did_action( 'learning_online_emails_init' ) ) {
			return;
		}
		...
		add_action( 'learning_online_course_submit_for_reviewer_notification', array( $this, 'review_course' ), 10, 2 );
		add_action( 'learning_online_course_submit_rejected_notification', array( $this, 'course_rejected' ), 10, 2 );
		add_action( 'learning_online_course_submit_approved_notification', array( $this, 'course_approved' ), 10, 2 );
		add_action( 'learning_online_user_finish_course_notification', array( $this, 'finish_course' ), 10, 3 );
		// Send email customer when order created
		add_filter( 'learning_online_checkout_success_result_notification', array( $this, 'customer_new_order' ), 10, 2 );
		add_action( 'set_user_role_notification', array( $this, 'become_an_teacher' ), 10, 3 );

		add_action( 'learning_online_email_header', array( $this, 'email_header' ) );
		add_action( 'learning_online_email_footer', array( $this, 'email_footer' ) );

		do_action( 'learning_online_emails_init', $this );
	}

	public function email_header ( $heading, $return = false ) {
		ob_start();
		learning_online_get_template( 'emails/email-header.php', array( 'email_heading' => $heading ) );
		$header = ob_get_clean();
		if ( ! $return ) {
			echo $header;
		} else {
	...
	public function email_footer ( $footer_text, $return = false ) {
		ob_start();
		learning_online_get_template( 'emails/email-footer.php', array( 'footer_text' => $footer_text ) );
		$footer = ob_get_clean();
		if ( ! $return ) {
			echo $footer;
		} else {
	...
	public function course_rejected ( $course_id ) {
		$course_user = learning_online_get_user( get_post_field( 'post_author', $course_id ) );
	...
	public static function init_email_notifications () {
		$actions = apply_filters(
			'learning_online_email_actions',
			array(
				'learning_online_course_submit_rejected',
				'learning_online_course_submit_approved',
				'learning_online_course_submit_for_reviewer',
				'learning_online_user_enrolled_course',
				// new order to admin
				'learning_online_order_status_pending_to_processing',
				'learning_online_order_status_pending_to_completed',
				'learning_online_order_status_pending_to_on-hold',
				'learning_online_order_status_failed_to_processing',
				'learning_online_order_status_failed_to_completed',
				'learning_online_order_status_failed_to_on-hold',
				'learning_online_order_status_completed',
				// admin create new order
				'learning_online_order_status_draft_to_pending',
				'learning_online_order_status_draft_to_processing',
				'learning_online_order_status_draft_to_on-hold',
				// Create order
				'learning_online_checkout_success_result',
				'learning_online_user_finish_course',
