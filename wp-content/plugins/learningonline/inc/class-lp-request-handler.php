...
class LP_Request_Handler {
	...
	public static function init() {
		...
		LP_Request_Handler::register( 'purchase-course', 'learning_online_purchase_course_handler', 20 );
		LP_Request_Handler::register( 'enroll-course', 'learning_online_purchase_course_handler', 20 );
	}
	...
	public static function process_request() {
		...
		if ( !empty( $_REQUEST ) ) foreach ( $_REQUEST as $key => $value ) {
			do_action( 'learning_online_request_handler_' . $key, $value, $_REQUEST );
		}
	}
	...
	public static function register( $action, $function = '', $priority = 5 ) {
		if ( is_array( $action ) ) {
			foreach ( $action as $item ) {
				...
				add_action( 'learning_online_request_handler_' . $action, $callback, $priority );
			}
		} else {
			add_action( 'learning_online_request_handler_' . $action, $function, $priority );
		}
	}

	public static function register_ajax( $action, $function, $priority = 5 ) {
		//$action, $function, $priority = 5
		add_action( 'learning_online_ajax_handler_' . $action, $function, $priority );
