...
class LP_Query {
	...
	public function parse_request( $q ) {
		if ( did_action( 'learning_online_parse_query' ) ) {
			return $q;
		}
		...
		$course_id                  = learning_online_setup_course_data( $this->query_vars['course'] );
		$quiz_name                  = '';
		$question_name              = '';
		$lesson_name                = '';
		...
		if ( $quiz_name && $question_name ) {
			if ( $quiz = learning_online_get_post_by_name( $quiz_name, LP_QUIZ_CPT ) ) {
				...
				if ( $question = learning_online_get_post_by_name( $question_name, 'lp_question' ) ) {
					global $wpdb;
					$query = $wpdb->prepare( "
					...
					", learning_online_get_current_user_id(), $quiz->ID, 'lp_quiz', 'completed' );
					if ( $history_id = $wpdb->get_var( $query ) ) {
						learning_online_update_user_item_meta( $history_id, 'current_question', $question->ID );
					}
				}
			}
		}
		$this->query_vars['course_id'] = $course_id;
		do_action_ref_array( 'learning_online_parse_query', array( &$this ) );
	}
	...
	function add_rewrite_tags() {
		...
		do_action( 'learning_online_add_rewrite_tags' );
	}
	...
	function add_rewrite_rules() {
		...
		$rewrite_prefix = get_option( 'learning_online_permalink_structure' );
		// lesson
		$course_type  = 'lp_course';
		$post_types   = get_post_types( '', 'objects' );
		...
		if ( $profile_id = learning_online_get_page_id( 'profile' ) ) {
			add_rewrite_rule(
		...
		if ( $course_page_id = learning_online_get_page_id( 'courses' ) ) {
			add_rewrite_rule(
		...
		do_action( 'learning_online_add_rewrite_rules' );
	}
	...
	public function get_course( $return = 'id' ) {
		...
		if ( $course && $return == 'object' ) {
			$course = learning_online_get_course( $course );
		}
		return $course;
	}
	...
	public function remove_query_tax() {
		remove_filter( 'posts_where', 'learning_online_add_tax_search' );
		remove_filter( 'posts_join', 'learning_online_join_term' );
		remove_filter( 'posts_groupby', 'learning_online_tax_groupby' );
	}
}