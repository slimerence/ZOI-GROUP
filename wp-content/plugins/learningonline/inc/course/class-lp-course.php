...
class LP_Course extends LP_Abstract_Course {
	...
	public function frontend_assets() {
		if ( learning_online_is_course() ) {
			...
			LP_Assets::add_localize( $translate, false, 'learning-online-single-course' );
		}
	}

	private function _get_localize() {
		return apply_filters(
			'learning_online_single_course_js_params',
			array(
				'confirm_finish_course' => array(
					'message' => sprintf( __( 'Are you sure you want to finish course %s?', 'learningonline' ), get_the_title() ),
					'title'   => __( 'Finish course', 'learningonline' )
				),
				'confirm_retake_course' => array(
					'message' => sprintf( __( 'Are you sure you want to retake course %s', 'learningonline' ), get_the_title() ),
					'title'   => __( 'Retake course', 'learningonline' )
				),
		...
		if ( empty( $courses[$item_id] ) ) {
			...
			$query = $wpdb->prepare( "
				SELECT lsi.item_id, ls.section_course_id as course_id
					FROM {$wpdb->prefix}learningonline_section_items lsi
					INNER JOIN {$wpdb->prefix}learningonline_sections ls ON ls.section_id = lsi.section_id
	...
	private static function get_course_class( $the_course, $args = array() ) {
		...
		return apply_filters( 'learning_online_course_object_class', $class_name, $course_type, $post_type, $course_id );
	}
	...
	private static function get_course_object( $the_course ) {
		...
		return apply_filters( 'learning_online_course_object', $the_course );
	}
