...
abstract class LP_Abstract_Course {
		...
		if ( empty( $this->{$key} ) ) {
			...
			switch ( $key ) {
				case 'current_item':
					...
					break;
				case 'current_lesson':
					$lesson_id = ( ( $lesson_id = learning_online_get_request( "lesson_id" ) ) && $this->has( 'item', $lesson_id ) ) ? $lesson_id : null;
					if ( $lesson_id ) {
	...
	public function get_image( $size = 'course_thumbnail', $attr = array() ) {
		...
		return apply_filters( 'learning_online_course_image', $image, $this->id, $size, $attr );
	}
	...
	public function is_enrollable() {
		...
		return apply_filters( 'learning_online_is_enrollable', $enrollable, $this );
	}
	...
	public function is_required_enroll() {
		...
		return apply_filters( 'learning_online_course_required_enroll', $required, $this );
	}
	...
	public function get_title() {
		return apply_filters( 'learning_online_course_title', $this->post ? $this->post->post_title : '', $this );
	}
	...
	public function get_curriculum( $section_id = 0, $force = false ) {
		...
		$curriculum = _learning_online_get_course_curriculum( $this->id, $force );
		$return     = false;
		if ( $section_id ) {
		...
		return apply_filters( 'learning_online_course_curriculum', $return, $this->id, $section_id );
	}
	...
	public function count_users_enrolled( $count_db = '' ) {
		...
		return apply_filters( 'learning_online_count_users_enrolled', $enrolled, $this );
	}
	...
	public function get_users_enrolled( $force = false ) {
		...
		if ( ( $this->_count_users === null && !array_key_exists( $this->id, self::$course_users ) ) || $force ) {
			self::$course_users = _learning_online_count_users_enrolled_courses( array( $this->id ) );
		}
	...
	public function get_students_html( $user_id = null ) {
		...
		if ( $count = $this->count_users_enrolled( 'append' ) ):
			...
			if ( $course_info['status'] ):
				if ( $count == 1 ):
					$output .= __( 'You enrolled', 'learningonline' );
				else:
					$output .= sprintf( _nx( 'You and one student enrolled', 'You and <span class="course-students-number">%1$s</span> students enrolled', intval( $count - 1 ), '', 'learningonline' ), $count - 1 );
				endif;
				$output = apply_filters( 'learning_online_students_enrolled_html', $output, $this );
			else:
				$output = sprintf( _nx( 'One student enrolled', '<span class="course-students-number">%1$s</span> students enrolled', $count, '', 'learningonline' ), $count );
				$output = apply_filters( 'learning_online_one_student_enrolled_html', $output, $this );
			endif;
		else:
			$output = apply_filters( 'learning_online_no_student_enrolled_html', __( 'No student enrolled', 'learningonline' ), $this );
		endif;
		return $output;
	}
	...
	public function get_instructor() {
		...
		return apply_filters( 'learning_online_course_instructor', $author_name, $this->id );
	}
	...
	public function get_instructor_html() {
		...
		$html       = sprintf(
			'<a href="%s">%s</a>',
			learning_online_user_profile_link( $this->post->post_author ),
			$instructor
		);
		return apply_filters( 'learning_online_course_instructor_html', $html, $this->post->post_author, $this->id );
	}
	...
	public function is_free() {
		...
		return apply_filters( 'learning_online_is_free_course', $is_free, $this );
	}
	...
	public function get_price() {
		...
		return apply_filters( 'learning_online_course_price', $price, $this );
	}
	...
	public function get_price_html() {
		...
		if ( $this->is_free() ) {
			$price_html = apply_filters( 'learning_online_course_price_html_free', __( 'Free', 'learningonline' ), $this );
		} else {
			$price      = $this->get_price();
			$price      = learning_online_format_price( $price, true );
			$price_html = apply_filters( 'learning_online_course_price_html', $price, $this );
		}
		return $price_html;
	}
	...
	public function get_origin_price_html() {
		...
		if ( $origin_price = $this->get_origin_price() ) {
			$origin_price      = learning_online_format_price( $origin_price, true );
			$origin_price_html = apply_filters( 'learning_online_course_origin_price_html', $origin_price, $this );
		}
		//}
	...
	public function get_students_list( $force = false, $limit = - 1 ) {
		...
		if ( $this->_students_list === null || $force ) {
			$query                = $wpdb->prepare( "
				SELECT DISTINCT u.*
				FROM {$wpdb->users} u
				INNER JOIN {$wpdb->prefix}learningonline_user_items ui ON ui.user_id = u.ID
				WHERE ui.item_id = %d
				AND ui.item_type = %s
				LIMIT %d
	...
	public function get_quizzes( $field = null ) {
		...
		return apply_filters( 'learning_online_course_quizzes', $quizzes, $this );
	}
	...
	public function get_lessons( $args = null ) {
		...
		return apply_filters( 'learning_online_course_lessons', $lessons, $this );
	}
	...
	public function get_curriculum_items( $args = array() ) {
		...
		return apply_filters( 'learning_online_get_course_items', $items, $this );
	}

	public function is_viewing( $content = '' ) {

		$item_type = !empty( $_REQUEST['course-item'] ) ? $_REQUEST['course-item'] : '';
		$viewing   = apply_filters( 'learning_online_course_is_viewing', $item_type ? $item_type : 'course' );
		if ( $content ) {
	...
	public function is_viewing_item( $item_id = false ) {
		...
		return apply_filters( 'learning_online_viewing_course_item', $return, $item_id, $this->id );
	}
	...
	public function is_current_item( $item_id ) {
		...
		return apply_filters( 'learning_online_is_current_course_item', $view_id == $item_id, $item_id, $view_id, $this->id );
	}
	...
	public function has( $tag ) {
		...
		if ( is_callable( $callback ) ) {
			...
			throw new Exception( sprintf( __( 'The function %s doesn\'t exists', 'learningonline' ), $tag ) );
		}
	}
	...
	public function is_purchasable() {
		...
		return apply_filters( 'learning_online_item_is_purchasable', $is_purchasable, $this->id );
	}
	...
	public function count_in_order( $statuses = 'completed' ) {
		...
		if ( !array_key_exists( $key, $data ) ) {
			...
			$query      = $wpdb->prepare( "
				...
				FROM {$wpdb->learningonline_order_itemmeta} oim
				INNER JOIN {$wpdb->learningonline_order_items} oi ON oi.order_item_id = oim.learningonline_order_item_id
					AND oim.meta_key = %s
	...
	public function get_item( $thing = '' ) {
		...
		return apply_filters( 'learning_online_course_item', $return, $item, $this->id );
	}
	...
	public function get_item_link( $item_id ) {
		...
		return apply_filters( 'learning_online_course_item_link', $item_links[ $key ], $item_id, $this );
	}
	...
	public function get_next_item( $args = null ) {
		...
		$items_types = apply_filters( 'learning_online_course_' . $args['dir'] . '_item_types', false, $this->id );
		...
		return apply_filters( 'learning_online_course_' . $args['dir'] . '_item', $next_item, $current_item, $this );
	}
	...
	public function get_next_item_html( $args = null ) {
		...
		if ( $next_item = $this->get_next_item( $args ) ) {
			ob_start();
			learning_online_get_template( 'content-lesson/next-button.php', array(
				'item'   => $next_item,
				'course' => $this
			) );
	...
	public function get_prev_item_html( $args = null ) {
		...
		if ( $next_item = $this->get_next_item( $args ) ) {
			ob_start();
			learning_online_get_template( 'content-lesson/prev-button.php', array( 'item' => $next_item, 'course' => $this ) );
			return ob_get_clean();
		}
		return false;
	}
	...
	public function get_course_result_html( $user_id = 0, $force = false ) {
		...
		if ( ( $this->course_result == 'evaluate_lesson' ) || !$quizzes ) {
			...
			$html = sprintf( __( '%d of %d items completed', 'learningonline' ), $this->count_completed_items( $user_id, $force ), $total_items );
		} else {
			if ( $this->course_result == 'evaluate_final_quiz' ) {
				$html = sprintf( __( '%d%% completed', 'learningonline' ), $this->_evaluate_course_by_quiz( $user_id, $force ) * 100 );
			} else {
				$html = sprintf( __( '%d%% completed', 'learningonline' ), $this->_evaluate_course_by_quizzes( $user_id, $force ) * 100 );
			}
		}
		return apply_filters( 'learning_online_course_result_html', $html, $this->id, $user_id );
	}
	...
	protected function _evaluate_course_by_items( $user_id = 0, $force = false, $type = '' ) {
		...
		return apply_filters( 'learning_online_course_results_by_items', $result, $this->id, $user_id );
	}
	...
	public function evaluate_course_results( $user_id = 0, $force = false ) {
		...
		return apply_filters( 'learning_online_evaluation_course_results', $results );
	}
	...
	public function _evaluate_course_by_quizzes_results( $user_id, $force = false ) {
		...
		return apply_filters( 'learning_online_evaluate_course_by_quizzes_results', $result, $this->id, $user_id );
	}
	...
	public function enable_evaluate_item( $item_id, $user_id = 0 ) {
		...
		return apply_filters( 'learning_online_enable_evaluate_course_item', true, $item_id, $user_id, $this->id );
	}
	...
	public function _evaluate_course_by_passed_quizzes_results( $user_id, $force = false ) {
		...
		return apply_filters( 'learning_online_evaluate_course_by_passed_quizzes_results', $result, $this->id, $user_id );
	}
	...
	public function _get_total_question( $quizzes_ids = array() ) {
		...
		if ( !empty( $quizzes_ids ) ) {
			...
			$sql = $wpdb->prepare( "
				SELECT COUNT(*)
				FROM {$wpdb->prefix}learningonline_quiz_questions lqq
	...
	public function get_completed_lessons( $user_id, $force = false ) {
		...
		if ( !array_key_exists( $key, $completed_lessons ) || $force ) {
			...
			$query = $wpdb->prepare( "
					...
					FROM {$wpdb->prefix}learningonline_user_items ul
		...
		return apply_filters( 'learning_online_user_completed_lessons', $completed_lessons[$key], $this->id, $user_id );
	}
	...
	public function _evaluate_course_by_lesson( $user_id, $force = false ) {
		...
		return apply_filters( 'learning_online_evaluation_course_lesson', $evaluate_course_by_lesson[$key], $this->id, $user_id );
	}
	...
	public function get_completed_items( $user_id = 0, $force = false, $type = '' ) {
		...
		_learning_online_parse_user_item_statuses( $user_id, $this->id );
		...
		return apply_filters( 'learning_online_user_completed_items', $completed_items, $this->id, $user_id );
	}
	...
	public function count_completed_items( $user_id = 0, $force = false, $type = '' ) {
		...
		return apply_filters( 'learning_online_count_user_completed_items', $count, $this->id, $user_id );
	}
	...
	public function is_final_quiz( $quiz_id ) {
		return apply_filters( 'learning_online_is_final_quiz', $this->final_quiz == $quiz_id, $quiz_id, $this->id );
	}
	...
	public function _evaluate_course_by_quiz( $user_id, $force = false ) {
		...
		return apply_filters( 'learning_online_evaluation_course_quiz', $result, $this->id, $user_id );
	}
	...
	public function _evaluate_course_by_quizzes( $user_id, $force = false ) {
		...
		return apply_filters( 'learning_online_evaluation_course_quizzes', $result, $this->id, $user_id );
	}
	...
	public function get_user_duration_html( $user_id = 0, $time = null ) {
		...
		if ( $course_info ) {
			...
			if ( $start_time + $duration > $now ) {
				...
				$html   = sprintf( __( 'This course will end within %s next', 'learningonline' ), $remain );
			}
		}
		return $html;
	}
	...
	public function get_user_expired_time( $user_id = 0, $args = array() ) {
		...
		return apply_filters( 'learning_online_user_course_expired_time', $expired, $user_id, $this->id );
	}
	...
	public function is_expired( $user_id = 0, $args = array() ) {
		...
		return apply_filters( 'learning_online_user_course_expired', $this->get_user_expired_time( $user_id, $args ) - current_time( 'timestamp' ) );
	}
	...
	public function output_args( $args = null ) {
		...
		$output = array(
			...
			'grade'        => $course_grade,
			'grade_html'   => learning_online_course_grade_html( $course_grade, false ),
			'current_item' => $this->is_viewing_item(),
			'items'        => $this->get_items_params()
		);

		$output = apply_filters( 'learning_online_single_course_params', $output, $this->id );
		LP_Assets::add_var( 'LP_Course_Params', wp_json_encode( $output ), 'learning-online-single-course' );
	...
	public function get_external_link() {
		return apply_filters( 'learning_online_external_link_buy_course', $this->external_link_buy_course, $this->id );
	}
