<?php
$attributes = learning_online_get_attributes();
...
$attribute_values = learning_online_get_attribute_terms( $attribute_ids );
...
<ul class="lp-course-attributes course-filters" data-ajax="<?php echo !empty( $this->instance['ajax_filter'] ) ? 'yes' : 'no'; ?>">
	<?php foreach ( $attributes as $attribute ) {
		...
		$values = learning_online_get_attribute_terms( $attribute->term_id );
		if ( !$values ) {
			continue;
		}
...
<?php if ( !empty( $this->instance['button_filter'] ) ) { ?>
	<button type="button" class="lp-button-filter" disabled="disabled"><?php _e( 'Filter', 'learningonline' ); ?></button>
<?php } ?>
<button type="button" class="lp-button-reset-filter" <?php echo $has_filtered ? '' : 'disabled="disabled"'; ?>"><?php _e( 'Reset', 'learningonline' ); ?></button>
