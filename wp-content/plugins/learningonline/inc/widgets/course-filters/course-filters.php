...
if ( !class_exists( 'LP_Widget_Course_Filters' ) ) {
	...
	class LP_Widget_Course_Filters extends LP_Widget {
		public function __construct() {
			...
			$this->options = array(
				'title'               => array(
					...
					'id'   => "{$prefix}title",
					'type' => 'text',
					'std'  => __( '', 'learningonline' )
				),
				'filter_by'           => array(
					'name'    => __( 'Filter by', 'learningonline' ),
					'id'      => "{$prefix}filter_by",
					'type'    => 'checkbox_list',
					'std'     => '',
					'options' => ''
				...
				'attribute_operation' => array(
					'name'    => __( 'Attribute operation', 'learningonline' ),
					'id'      => "{$prefix}attribute_operator",
					'type'    => 'select',
					'std'     => 'and',
					'options' => array(
						'and' => __( 'And', 'learningonline' ),
						'or'  => __( 'Or', 'learningonline' )
					)
				),
				'value_operation'     => array(
					'name'    => __( 'Value operation', 'learningonline' ),
				...
				'ajax_filter'         => array(
					'name' => __( 'Ajax filter', 'learningonline' ),
					'id'   => "{$prefix}ajax_filter",
					'type' => 'checkbox',
					'std'  => '0',
					'desc' => __( 'Use ajax to fetch content while filtering', 'learningonline' )
				),
				'button_filter'       => array(
					'name' => __( 'Button filter', 'learningonline' ),
					'id'   => "{$prefix}button_filter",
					'type' => 'checkbox',
					'std'  => '0',
					'desc' => __( 'If checked, user has to click this button to start filtering', 'learningonline' )
				)
			);
			parent::__construct();
			add_filter( 'learning_online_widget_display_content-' . $this->id_base, 'learning_online_is_courses' );
			if ( !is_admin() ) {
		...
		protected function get_filtered_term_course_counts( $term_ids, $taxonomy, $query_type ) {
			...
			$query             = apply_filters( 'learning_online_get_filtered_term_course_counts_query', $query );
			$query             = implode( ' ', $query );
