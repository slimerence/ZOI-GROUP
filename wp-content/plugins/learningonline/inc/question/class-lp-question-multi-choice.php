...
class LP_Question_Multi_Choice extends LP_Abstract_Question {
	...
	public function submit_answer( $quiz_id, $answer ) {
		$questions = learning_online_get_question_answers( null, $quiz_id );
		if ( !is_array( $questions ) ) $questions = array();
		$questions[$quiz_id][$this->id] = is_array( $answer ) ? reset( $answer ) : $answer;
		learning_online_save_question_answer( null, $quiz_id, $this->id, is_array( $answer ) ? reset( $answer ) : $answer );
	}
	...
	public function get_default_answers( $answers = false ) {
		if ( !$answers ) {
			if ( $this->id && $this->post->post_status !== 'auto-draft' ) {
				global $wpdb;
				$sql              = $wpdb->prepare( "SELECT * FROM $wpdb->learningonline_question_answers "
					. " WHERE question_id = %d"
	...
	public function admin_interface( $args = array() ) {
		ob_start();
		$view = learning_online_get_admin_view( 'meta-boxes/question/multi-choice-options.php' );
		include $view;
		$output = ob_get_clean();
	...
	public function get_icon() {
		return '<img src="' . apply_filters( 'learning_online_question_icon', LP()->plugin_url( 'assets/images/multiple-choice.png' ), $this ) . '">';
	}
	...
	public function admin_interfaces( $args = array() ) {
		...
		<p>
			<button type="button" class="button lpr-button-add-answer"><?php _e( 'Add answer', 'learningonline' ); ?></button>
		</p>
	...
	private function _admin_enqueue_script( $enqueue = true ) {
		...
		<script type="text/javascript">
			(function ($) {
				...
				$form.unbind('learning_online_question_before_update.<?php echo $key;?>').bind('learning_online_question_before_update.<?php echo $key;?>', function () {
					...
					if ($question.length) {
						...
						if (0 == $input.length) {
							...
							message += ": " + '<?php _e( 'No answer added to question or you must select at least one the answer is correct!', 'learningonline' );?>';
							window.learning_online_before_update_quiz_message.push(message);
	...
	public function render( $args = null ) {
		...
		$view = learning_online_locate_template( 'content-question/multi-choice/answer-options.php' );
		include $view;
	}
	...
	public static function admin_js_template() {
		...
		<tr class="lp-list-option lp-list-option-empty <# if(data.id){ #>lp-list-option-{{data.id}}<# } #>" data-id="{{data.id}}">
			...
			<td>
				<input class="lp-answer-text no-submit key-nav" type="text" name="learning_online_question[{{data.question_id}}][answer][text][]" value="{{data.text}}" />
			</td>
			<th class="lp-answer-check">
				<input type="hidden" name="learning_online_question[{{data.question_id}}][answer][value][]" value="{{data.value}}" />
				<input type="checkbox" name="learning_online_question[{{data.question_id}}][checked][]" {{data.checked}} value="{{data.value}}" />
			</th>
		...
		<?php
		return apply_filters( 'learning_online_question_multi_choice_answer_option_template', ob_get_clean(), __CLASS__ );
	}
}