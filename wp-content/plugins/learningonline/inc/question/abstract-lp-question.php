...
class LP_Abstract_Question {
	...
	protected function _init() {
		...
		add_filter( 'learning_online_question_answers', array( $this, '_get_default_answers' ), 10, 2 );
	}
	...
	public function empty_answers() {
		global $wpdb;
		$query = $wpdb->prepare( "
			DELETE FROM {$wpdb->learningonline_question_answers}
			WHERE question_id = %d
		", $this->id );
		do_action( 'learning_online_before_delete_question_answers', $this->id );
		$wpdb->query( $query );
		do_action( 'learning_online_delete_question_answers', $this->id );
	...
	public function get_default_answers( $answers = false ) {
		if ( !$answers ) {
			$answers = array(
				array(
					'is_true' => 'yes',
					'value'   => learning_online_uniqid(),
					'text'    => __( 'Option First', 'learningonline' )
				),
				array(
					'is_true' => 'no',
					'value'   => learning_online_uniqid(),
					'text'    => __( 'Option Seconds', 'learningonline' )
				),
				array(
					'is_true' => 'no',
					'value'   => learning_online_uniqid(),
					'text'    => __( 'Option Third', 'learningonline' )
				)
			);
		}
		return $answers;
	...
	public function save( $post_data = null ) {
		 ...
		 */
		$types = apply_filters( 'learning_online_save_default_question_types', array( 'true_or_false', 'multi_choice', 'single_choice' ) );
		//echo $this->type;
		...
		if ( in_array( $this->type, $types ) ) {
			...
			if ( !empty( $post_data['answer'] ) ) {
				...
				foreach ( $post_data['answer']['text'] as $index => $text ) {
					...
					$answers[] = apply_filters( 'learning_online_question_answer_data', $data, $post_data['answer'], $this );
				}

				if ( $answers = apply_filters( 'learning_online_question_answers_data', $answers, $post_data['answer'], $this ) ) {
		...
		do_action( 'learning_online_update_question_answer', $this, $post_data );
	}
	...
	public function get_answers( $field = null, $exclude = null ) {
		...
		/*	if ( !empty( $GLOBALS['learningonline_question_answers'][$this->id] ) ) {
				if ( array_key_exists( $this->id, $GLOBALS['learningonline_question_answers'] ) ) {
					$answers = $GLOBALS['learningonline_question_answers'][$this->id];
				}
			}
			if ( empty( $answers ) ) {
				$query = $wpdb->prepare( "
				...
				$GLOBALS['learningonline_question_answers'][$this->id] = $answers;
			}*/
		...
		return apply_filters( 'learning_online_question_answers', $answers, $this );
	}
	...
	public function admin_interface_head( $args = array() ) {
		$view = learning_online_get_admin_view( 'meta-boxes/question/header.php' );
		include $view;
	}
	...
	public function admin_interface_foot( $args = array() ) {
		$view = learning_online_get_admin_view( 'meta-boxes/question/footer.php' );
		include $view;
	}
	...
	public function admin_interface( $args = array() ) {
		printf( __( 'Function %s should override from its child', 'learningonline' ), __FUNCTION__ );
	}
	...
	public function get_icon() {
		return '<img src="' . apply_filters( 'learning_online_question_icon', LP()->plugin_url( 'assets/images/question.png' ), $this ) . '">';
	}
	...
	public function is_selected_option( $answer, $answered = false ) {
		...
		return apply_filters( 'learning_online_is_selected_option', $is_selected, $answer, $answered, $this );
	}
	...
	public function save_user_answer( $answer, $quiz_id, $user_id = null ) {
		...
		if ( $progress = $user->get_quiz_progress( $quiz_id ) ) {
			...
			$question_answers = apply_filters( 'learning_online_update_user_question_answers', $question_answers, $progress->history_id, $user_id, $this, $quiz_id );

			learning_online_update_user_quiz_meta( $progress->history_id, 'question_answers', $question_answers );
		}
		//do_action( 'learning_online_update_user_answer', $progress, $user_id, $this, $quiz_id );
	}
...
class LP_Question extends LP_Abstract_Question {
	...
	public function admin_interface_head( $args = array() ) {
		...
		$questions = learning_online_get_question_types();
		?>
		<div class="lpr-question lpr-question-<?php echo preg_replace( '!_!', '-', $this->get_type() ); ?>" data-type="<?php echo preg_replace( '!_!', '-', $this->get_type() ); ?>" data-id="<?php echo $this->get( 'ID' ); ?>" id="learning-online-question-<?php echo $this->id; ?>">
		<div class="lpr-question-head">
			<p>
				<a href="<?php echo get_edit_post_link( $post_id ); ?>"><?php _e( 'Edit', 'learningonline' ); ?></a>
		...
		<?php //do_action( 'learning_online_admin_before_question_answer', $this );
		?>
		<p class="lpr-question-option-label"><?php _e( 'Answer', 'learningonline' ); ?></p>
		<?php
	}
	...
	public function admin_interface_foot( $args = array() ) {
		...
		<p class="lpr-change-question-type">
			<span><?php _e( 'Change type of this question to', 'learningonline' ); ?></span>
			<select class="lpr-question-types" name="lpr_question[type]" id="lpr_question-type" data-type="<?php echo $question_type; ?>">
				<option value=""><?php _e( '--Select type--', 'learningonline' ); ?></option>
				<?php if ( $question_types ): foreach ( $question_types as $type ): ?>
					<option value="<?php echo $type; ?>" <?php selected( $type == $question_type ); ?>><?php echo learning_online_question_slug_to_title( $type ); ?></option>
