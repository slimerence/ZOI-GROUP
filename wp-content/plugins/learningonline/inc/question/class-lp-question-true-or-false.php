...
class LP_Question_True_Or_False extends LP_Abstract_Question {
	...
	public function __construct( $the_question = null, $args = null ) {
		...
		add_filter( 'learning_online_question_answers', array( $this, 'limit_answers' ), 10, 2 );
	}
		...
		if ( !$answers ) {
			...
			$answers = array(
				array(
					'is_true' => 'yes',
					'value'   => 'true',
					'text'    => __( 'True', 'learningonline' )
				),
				array(
					'is_true' => 'no',
					'value'   => 'false',
					'text'    => __( 'False', 'learningonline' )
				)
			);
		}
		return $answers;
	...
	public function admin_js_template() {
		...
		<?php
		return apply_filters( 'learning_online_question_true_or_false_answer_option_template', ob_get_clean(), __CLASS__ );
	}
}