...
class LP_Question_Single_Choice extends LP_Abstract_Question {
	...
	public static function admin_js_template() {
		...
		<tr class="lp-list-option lp-list-option-new lp-list-option-empty <# if(data.id){ #>lp-list-option-{{data.id}}<# } #>" data-id="{{data.id}}">
			...
			<th class="lp-answer-check">
				...
				<input type="radio" name="learning_online_question[{{data.question_id}}][checked][]" {{data.checked}} value="{{data.value}}" />
			</th>
		...
		<?php
		return apply_filters( 'learning_online_question_single_choice_answer_option_template', ob_get_clean(), __CLASS__ );
	}
	...
	public function submit_answer( $quiz_id, $answer ) {
		...
		learning_online_save_question_answer( null, $quiz_id, $this->get( 'ID' ), is_array( $answer ) ? reset( $answer ) : $answer );
		//print_r($answer);
	}

	public function get_icon() {
		return '<img src="' . apply_filters( 'learning_online_question_icon', LP()->plugin_url( 'assets/images/single-choice.png' ), $this ) . '">';
	}
	...
	private function _admin_enqueue_script( $enqueue = true ) {
		...
		<script type="text/javascript">
			(function ($) {
				var $form = $('#post');
				$form.unbind('learning_online_question_before_update.<?php echo $key;?>').on('learning_online_question_before_update.<?php echo $key;?>', function () {
					...
					if ($question.length) {
						...
						if (0 == $input.length) {
							...
							message += ": " + '<?php _e( 'No answer added to question or you must set an answer is correct!', 'learningonline' );?>'
	...
	public function admin_interface( $args = array() ) {
		ob_start();
		$view = learning_online_get_admin_view( 'meta-boxes/question/single-choice-options.php' );
		include $view;
		$output = ob_get_clean();
	...
	public function render( $args = null ) {
		...
		$view = learning_online_locate_template( 'content-question/single-choice/answer-options.php' );
		include $view;
	}
