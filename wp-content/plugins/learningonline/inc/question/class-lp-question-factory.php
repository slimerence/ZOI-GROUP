...
class LP_Question_Factory {
	...
	public static function get_question_class( $the_question, $args = array() ) {
		...
		return apply_filters( 'learning_online_question_class', $classname, $question_type, $question_id );
	}
	...
	public static function get_question_object( $the_question ) {
		...
		return apply_filters( 'learning_online_question_object', $the_question );
	}
	...
	public static function init() {
		...
		if ( is_admin() ) {
			...
			add_action( 'learning_online_convert_question_type', array( __CLASS__, 'convert_question' ), 5, 4 );
			add_filter( 'learning_online_question_answers_data', array( __CLASS__, 'sanitize_answers' ), 10, 3 );

		} else {

		}
		add_action( 'learning_online_load_quiz_question', array( __CLASS__, 'save_question_if_needed' ), 100, 3 );
		add_action( 'learning_online_user_finish_quiz', array( __CLASS__, 'save_question' ), 100, 2 );
		add_action( 'learning_online_after_quiz_question_title', array( __CLASS__, 'show_answer' ), 100, 2 );
		add_action( 'learning_online_after_question_wrap', array( __CLASS__, 'show_hint' ), 100, 2 );
		add_action( 'learning_online_after_question_wrap', array( __CLASS__, 'show_explanation' ), 110, 2 );
		...
		do_action( 'learning_online_question_factory_init', __CLASS__ );
	}
	...
	public static function delete_question( $post_id, $force=false ) {
		...
		if( 'lp_question' === get_post_type($post_id) ) {
			...
			$sql = 'DELETE FROM `'.$wpdb->prefix.'learningonline_question_answers` WHERE `question_id` = '.$post_id;
			$wpdb->query($sql);
			// remove question in quiz
			$sql = 'DELETE FROM `'.$wpdb->prefix.'learningonline_quiz_questions` WHERE `question_id` = '.$post_id;
			$wpdb->query($sql);
		}
	}
	...
	public static function show_hint( $id, $quiz_id ) {
		learning_online_get_template( 'content-question/hint.php' );
	}

	public static function show_explanation( $id, $quiz_id ) {
		learning_online_get_template( 'content-question/explanation.php' );
	}
	...
	public static function save_question_if_needed( $question_id, $quiz_id, $user_id ) {
		...
		$save_id         = learning_online_get_request( 'save_id' );
		...
		if ( $question && !$user->has_checked_answer( $save_id, $quiz_id ) && $user->get_quiz_status( $quiz_id ) == 'started' ) {
			...
			$question_answer = array_key_exists( 'learning-online-question-' . $save_id, $question_answer ) ? $question_answer['learning-online-question-' . $save_id] : '';
			$question->save_user_answer( $question_answer, $quiz_id );
			do_action( 'learning_online_save_user_question_answer', $question_answer, $save_id, $quiz_id, $user_id, true );
		}
	...
	public static function admin_assets() {
		//LP_Assets::enqueue_style( 'learning-online-meta-box-question' );
		//LP_Assets::enqueue_script( 'learning-online-meta-box-question', false, array( 'learning-online-admin' ) );
	}
	...
	public static function get_types() {
		$types = array(
			'true_or_false' => __( 'True Or False', 'learningonline' ),
			'multi_choice'  => __( 'Multi Choice', 'learningonline' ),
			'single_choice' => __( 'Single Choice', 'learningonline' )
		);
		return apply_filters( 'learning_online_question_types', $types );
	}
	...
	public static function save( $post_id ) {
		...
		if ( !empty( $_POST['learning_online_question'] ) ) {
			foreach ( $_POST['learning_online_question'] as $the_id => $post_data ) {
	...
	public static function convert_question( $id, $from, $to, $data ) {
		if ( !empty( $data['learning_online_question'] ) && !empty( $data['learning_online_question'][$id] ) ) {
			$post_data = $data['learning_online_question'][$id];
		} else {
			$post_data = array();
		}
